...
#Christian_lhcbstyle.C
#from Christian_lhcbstyle import *
...
def plotMassFit(w,dataName,massVar,opts={}):
        ...
        pdf.plotOn(fr,RooFit.Components('exp'),RooFit.LineStyle(kDashed),RooFit.LineColor(kBlue))
        ...
        pdf.plotOn(fr,RooFit.Components('sig'),RooFit.LineColor(kRed))
        ...
        #fr_h.SetLineColor(1)
        #fr_h.SetFillColor(2)
        #fr_h.SetFillColor(kBlue-5) #Christians pull band
        #fr_h.SetLineColor(kBlue-5)
        #fr_h.SetFillColor(kBlue-5) #Christians pull band
       ...
       #fr.GetYaxis().SetTitleSize(0.15)
        fr.GetYaxis().SetLabelSize(0.07)
        fr.GetXaxis().SetTitleSize(0)
        fr.GetXaxis().SetLabelSize(0)



       #Changes requested by Marco
        #----------------------------------------------------------------------#
        #1. The Y-axis
        #of each mass distribution should show candidates and not
        #events. Please us the following to that extend:

        fr.GetYaxis().SetTitle("Candidates / (%4.0f %s)" % (m.getBinWidth(1), str(m.getUnit())))
        #Done!

        #2. In MeV/c the c needs to be italicised, use #it{c}. This affects the
        #X-axis titles, but also the RooRealVar units when defining the RooRealVar.

        #Done!

        #3. The X-axis title should show the mass as m_daughters. m is to be
        #italicised. In case of mumu daughters, the charges are to be explicit.
        #The minus sign needs to be replaced with #font[122]{-}. Hence the m_mumu is:
        #'#it{m}_{#mu^{+}#mu^{#font[122]{-}}}'

        #Done!

        #4. units are to be enclosed in square brackets []. (not for the Y-axis
        #        candidates/ (2 MeV))
        #Done!

        #- use italicised c in X-axis and RooRealVar.
        #Done!

        #- use squared parentheses around X-axis units.
        #Done!

        #- Add LHCb
        #- Add 3fb-1 below LHCb

        #- increase the size of the label and title for both axis, specially X-axis.


        #- harmonise with Christian's plots:
        #-- remove Pull
        #-- use same grey as Christian for the pull
        #-- use color coding and linestyle as Christian.o




        #5. Save all the plot in the following formats (at least): .C, .root,
        #.pdf, .png, .eps
        ...
        lhcbName.AddText("#intLdt = 3fb^{-1}")

        #lhcbName.AddText("#intLdt=3fb^{-1}")
        #lhcbName.AddText("3fb^{-1}")
        lhcbName.Draw()
        ...
        fr_h.GetYaxis().SetTitleSize(0.15)
        fr_h.GetYaxis().SetLabelSize(0.13)
        fr_h.GetYaxis().SetNdivisions(7)

        fr_h.GetXaxis().SetTitle('#it{m}_{#mu^{+}#mu^{#font[122]{-}}K^{+}} [MeV/#it{c}^{2}]')
        fr_h.GetXaxis().SetTitleOffset(1.05)
        #fr_h.GetXaxis().SetTitleSize(0.2)
        fr_h.GetXaxis().SetTitleSize(0.18)
        fr_h.GetXaxis().SetLabelSize(0.15)
        ...
        canv.SaveAs(directory+opts.get('plotName','MassFit')+'_param_'+str(param)+'.pdf')
        canv.SaveAs(directory+opts.get('plotName','MassFit')+'_param_'+str(param)+'.png')
        canv.SaveAs(directory+opts.get('plotName','MassFit')+'_param_'+str(param)+'.C')
        canv.SaveAs(directory+opts.get('plotName','MassFit')+'_param_'+str(param)+'.ps')
        canv.SaveAs(directory+opts.get('plotName','MassFit')+'_param_'+str(param)+'.eps')
...
def fitBu2JpsiKplus(data, massVar, cutName, cut, log = True, dirName = False, param = False):
        ...
        mass = RooRealVar(massVar,'#it{m}_{#mu^{+}#mu^{#font[122]{-}}K^{+}}',Bu_pdg-100.,Bu_pdg+100.,'MeV/#it{c}^{2}')
        obs = RooArgList(mass)
...
def fitBs2JpsiPhi(data, massVar, cutName, cut, log = True, dirName = False, param = False):
        ...
        mass = RooRealVar(massVar,'J/#psi constrained B^{0}_{s} mass',Bs_pdg-100.,Bs_pdg+100.,'MeV/#it{c}^{2}')
        obs = RooArgList(mass)
