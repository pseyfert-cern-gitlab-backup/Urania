...
def getTrigHistsInPeriod(nTupleName):
	...
	for t in trig:
		...
		p = plotTrigHist( {'nTupleName':str(nTupleName), 'channel':channel, 'topLines':topLines, 'nTop': nTop, 'base':base, 'trig':t, 'directory':dir} )
		del p
...
class plotTrigHist():
	...
	def __init__(self, opts={'channel':'Bu' , 'topLines':False, 'base':'Bplus', 'trig':'L0'}):
		...
		self.dictData = readData(self.nTupleName,{'channel':self.channel})
...
#-------------------------------------------------------------------------------------------------------------------#	
	...
	def drawHistogram(self,varDict, hists):
		...
		self.canv.Print(self.dir + 'TrigHist_'+self.nTupleName+self.suf+'.ps')	
		return 1
