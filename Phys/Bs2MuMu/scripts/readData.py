...
def readData(nTupleName, opts={}):
	...
	#---------------------------------------------------------------#
        if nTupleName=='2011_Strip20r1':
		#2011 data
                print '    o) Importing reprocessed: ', nTupleName
                lumi_repro = 71 + 132 + 120 + 164 + 41.5 + 54.7 + 64.7 + 70.7 + 67.3 + 74.7 + 73.7 + 72.4 + 35.0 # 1041.7pb
        ...
        if nTupleName=='2011_Strip20r1_a0':
		#2011 data
                print '    o) Importing reprocessed: ', nTupleName
                lumi_repro = 71
                dir = '/castor/cern.ch/user/f/fsoomro/Bs2MuMuStrip20/'
	...
	#---------------------------------------------------------------#
	if nTupleName=='2012_Strip20' or nTupleName=='2012_Strip20_BadITRuns' or nTupleName=='2012_Strip20_GoodITRuns':
	...
	if nTupleName=='2012_Strip19c_f':
        ...
        if opts.get('cut',0) and opts.get('applyCut',False):
        	...
        	print '    ...be patient...'
                ...
                t = tree.CopyTree(opts.get('cut')+'1')
                tree = t
                print '    ..after cut tree contains:', tree.GetEntries()
               	# ---------------------------------------------------------------- #

        return {'tree':tree, 'name':nTupleName, 'lumi':lumi, 'lumiUnc':lumiUnc, 'cut':opts.get('cut')}
