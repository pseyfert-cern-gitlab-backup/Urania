...
#opts['RESULTDIR'] = 'Profile_Results/TestFit/'
#opts['RESULTDIR'] = 'Profile_Results/TestFit2/'
#opts['RESULTDIR'] = 'Profile_Results/Fit_AddBdPiMuNu/'
#opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_LaPMuNu_Fixed_DoubleExp/'
opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_AleCommentsRooKeysMisID/'
#opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_AleCommentsCbMisid/'
#opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_AleCommentsFixAlphaFdFs/
#opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_AleCommentsFixAlphaConstrFdFsCBMisId/'
#opts['RESULTDIR'] = 'Profile_Results/Fit_PiMuNu_PiMuMu_AleCommentsCBMisidDoubleExp/'
# --------------------------------------------------------------------------------- #
...
opts['ADD_B0PIMUNU']= 1
opts['FIX_B0PIMUNU']= 1
opts['ADD_B0PIMUNU_CONST']= 0

opts['ADD_B0UPIMUMU'] = 1
opts['FIX_B0UPIMUMU'] = 1
...
print '     MODEL:  -> Done! \n'
print '     MODEL:   o) Import the misid phace space(->BDT) dependence correction factors...'
...
#alpha_KPi , alpha_KPi_cons = createConst(TheTable.NormKPi_fullmass*1e9,TheTable.NormKPi_fullmassErr*1e9,'alpha_KPi')
#alpha_Bu , alpha_Bu_cons = createConst(TheTable.NormBu_fullmass*1e9,TheTable.NormBu_fullmassErr*1e9,'alpha_Bu')
...
alpha_Bu_err = RooRealVar('eBu','eBu',TheTable.NormBu_fullmassErr*1e12) #for combination in weighted av
alpha_Bu_err.setConstant()
alpha_KPi_err = RooRealVar('eKPi','eKPi',TheTable.NormKPi_fullmassErr*1e12) #for combination in weighted av
...
#alpha_d = RooFormulaVar("alpha_d","alpha_d", "0.5*alpha_Bu+0.5*alpha_KPi", RooArgList(alpha_Bu,alpha_KPi)) # ok!
#alpha_d = RooFormulaVar("alpha_d","alpha_d", "1*alpha_Bu+0.*alpha_KPi", RooArgList(alpha_Bu,alpha_KPi)) #Works!
nbd = RooRealVar("NBd","NBd", 5,-20.,30)
...
if opts.get('INCLUDE_BD'):
    ...
    nbd = RooFormulaVar("NBd","NBd", "BRd*timeAccCorrBd/alpha_d", RooArgList(alpha_d,timeAccCorrBd,BR_d))
...
nbs = RooFormulaVar("NBs","NBs", "BRs*timeAccCorrBs/alpha_s", RooArgList(alpha_s,timeAccCorrBs,BR_s))
print '     MODEL:  -> Done! \n'
...
n , n_cons = createConst(TheTable.CBExpo,TheTable.CBExpo,'CBExpo')


print '     MODEL:  -> Done! \n'
print '     MODEL:   o) Import the signal distribution in BDT from hh (TIS) distribution in the BDT...'

f2 ,f2_cons = createConst(TheTable.BDT_sig2,TheTable.BDT_sigErr2, "f2")
f3 ,f3_cons = createConst(TheTable.BDT_sig3,TheTable.BDT_sigErr3, "f3")
f4 ,f4_cons = createConst(TheTable.BDT_sig4,TheTable.BDT_sigErr4, "f4")
f5 ,f5_cons = createConst(TheTable.BDT_sig5,TheTable.BDT_sigErr5, "f5")
f6 ,f6_cons = createConst(TheTable.BDT_sig6,TheTable.BDT_sigErr6, "f6")
f7 ,f7_cons = createConst(TheTable.BDT_sig7,TheTable.BDT_sigErr7, "f7")
f8 ,f8_cons = createConst(TheTable.BDT_sig8,TheTable.BDT_sigErr8, "f8")

#f1 ,f1_cons = createConst(TheTable.BDT_sig1,TheTable.BDT_sigErr1, "f1")
...
class BsMuMuModel:
    ...
    def __init__(self, glbin):
        ...
        print '         -> N_Bs_bin = N_Bs * BDT_f/j1 * 1/timeAcc '
        self.nbs = RooFormulaVar("NBs" + i ,"NBs" + i, "NBs*1/j"+i+"*f"+i+"*1/timeAcc"+i, RooArgList(v["f"+i],v["nbs"],v["j"+i],v["timeAcc"+i]))
        print '         -> N_Bd_bin = N_Bs * BDT_f/j1 '
...
print '  -> Done! \n'
print '     o) Add the constraints to the model...\n'
...
summaryConstraints.add(RooArgSet(f2_cons, f3_cons, f4_cons, f5_cons, f6_cons, f7_cons, f8_cons))
...
if not opts.get('FIXFITMODEL'):
    fiter.fitTo(allData,RooFit.Minos(kTRUE), RooFit.ExternalConstraints(RooArgSet(summaryConstraints)),RooFit.Strategy(2), RooFit.NumCPU(8), RooFit.Verbose(kTRUE))
