...
def DoSigModel(BR, dc = dcCMS):
    ...
    dc[str(BR) + "barrel"] = csm_template(2)  ####### 12 nuissance parameters:   (c_u, c_phi, b_kpi, fdfs, Ntis_hh), (mean, sigma, trans), ( N_GL :2,3,4) , CB model
    dc[str(BR) + "barrel_no_syst"] = csm_template(0)
    dc[str(BR) + "EC"] = csm_template(2)  ####### 12 nuissance parameters:   (c_u, c_phi, b_kpi, fdfs, Ntis_hh), (mean, sigma, trans), ( N_GL :2,3,4) , CB model
