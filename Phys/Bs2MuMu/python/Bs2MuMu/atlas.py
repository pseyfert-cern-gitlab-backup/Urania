...
def DoSigModel(BR, dc = dcATLAS):
    ...
    dc[str(BR) + "r1"] = csm_template(3)  ####### 12 nuissance parameters:   (c_u, c_phi, b_kpi, fdfs, Ntis_hh), (mean, sigma, trans), ( N_GL :2,3,4) , CB model
    dc[str(BR) + "r1_no_syst"] = csm_template(0)
    dc[str(BR) + "r2"] = csm_template(3)  ####### 12 nuissance parameters:   (c_u, c_phi, b_kpi, fdfs, Ntis_hh), (mean, sigma, trans), ( N_GL :2,3,4) , CB model
    dc[str(BR) + "r2_no_syst"] = csm_template(0)

    dc[str(BR) + "r3"] = csm_template(3)  ####### 12 nuissance parameters:   (c_u, c_phi, b_kpi, fdfs, Ntis_hh), (mean, sigma, trans), ( N_GL :2,3,4) , CB model
