...
#from Bs2MuMuParams
from Bs2MuMuParams import Table_1fb as TheTable
import sys, os
...
if CHANNEL == "Bs":
    ...
    s_MISIDn_asym_rel = MisIDfErrNBd/TheTable.MisIDfBd
    s_MISID_sym_rel = TheTable.MisIDGlobalFactorErr / TheTable.MisIDGlobalFactor
...
Nb, hbpdf = fillB(Nsb,idx,"hbpdf 2012")
...
for i in range(len(N_sig_gl)):
    ...
    Nb_sb_p[i], hbpdf_sb_p[i] = fillB(nnsbp, idx, "hbpdf_sb"+str(i)+"_p_2012")
    Nb_sb_n[i], hbpdf_sb_n[i] = fillB(nnsbn, idx, "hbpdf_sb"+str(i)+"_n_2012")
...
for i in range(len(N_sig_gl)-1):
    Nbp_i[i], hbpdfp_i[i] = fillB(Nsb, idxp[i], "hbpdfp_i"+str(i)+"_2012")
    Nbn_i[i], hbpdfn_i[i] = fillB(Nsb, idxn[i], "hbpdfn_i"+str(i)+"_2012")

### Create Bkg hypothesis

bkg_no_syst = csm_template(0)#8)
bkg = csm_template(2*int(len(N_sig_gl)) -1 )
for i in range(len(N_sig_gl)):
