...
opts['RESULTDIR'] = 'Profile_Results/2013_CombinationFit_v1/'
...
#f1 = RooFormulaVar("f1", "f1", "(1-f2-f3-f4-f5-f6-f7-f8)", RooArgList(f2,f3,f4,f5,f6,f7,f8)) #wrong, the renormalization needs to be done at the end, for all the corrections together!
...
f_bd_raw_sum = 0
for i in range(0,8):
	f_bs_raw[i] = RooFormulaVar("f_bs_raw_"+str(i),"Unnormalized fr. of events in bdt bin"+str(i), "(@0*(1/@1)*@2)", RooArgList(v['BDT_f'+str(i)],v['j'+str(i)],v['timeAcc'+str(i)]))
	f_bs_raw_sum += f_bs_raw[i]	
	f_bd_raw[i] = RooFormulaVar("f_bd_raw_"+str(i),"Unnormalized fr. of events in bdt bin"+str(i), "(@0*(1/@1))", RooArgList(v['BDT_f'+str(i)],v['j'+str(i)]))
	f_bd_raw_sum += f_bd_raw[i]	

sum_bs = RooRealVar('f_bs_raw_sum','Sum of all the raw fractions',f_bs_raw_sum)
sum_bd = RooRealVar('f_bd_raw_sum','Sum of all the raw fractions',f_bd_raw_sum)
...
f_bd = []
for i in range(0,8):
	f_bs[i] = RooFormulaVar("f_bs_"+str(i),"Fr. of events in bdt bin"+str(i), "(@0/@1)", RooArgList(['f_bd_raw_'+str(i)],v[''+str(i)],v['f_bs_raw_sum']))
	f_bd[i] = RooFormulaVar("f_bd_"+str(i),"Fr. of events in bdt bin"+str(i), "(@0/@1)", RooArgList(['f_bd_raw_'+str(i)],v[''+str(i)],v['f_bd_raw_sum']))
...
class BsMuMuModel:
	...
	self.nbs = RooFormulaVar("NBs" + i ,"NBs" + i, "NBs*f_bs_"+i, RooArgList(v["nbs"],v["f_bs_"+i]))

	print '         -> N_Bd_bin = N_Bs * f_bd_i'
        self.nbd = RooFormulaVar("NBd" + i ,"NBd" + i, "NBd*f_bd_"+i, RooArgList(v["nbd"],v["f_bd_"+i]))
