...
def sigmas(offset = 1, fixShapePar = 0):


    if offset and fixShapePar: #Case where the RooPhysBkg shape parameters are fixxed
        #This is the -log(L) of the fitted model:
        # a) The null model
        nll_null        =  -39415.6855263
        # b) BRs free
        nll_brs         =  -39422.5573216
        getSignificance(nll_null,nll_brs,1,' BRs fit w.r.t. BRs=BRd=0')
        # c) BRs free
        nll_brd         =  -39416.8635326
        getSignificance(nll_null,nll_brd,1,' BRd fit w.r.t. BRs=BRd=0')
        # d) BRs,BRd free
        nll_brs_brd     =  -39424.0895137
        getSignificance(nll_null,nll_brs_brd,2,' BRs,BRd fit w.r.t. BRs=BRd=0')
        print '  RESULTS WITH OFFSET AND RooPhysBkg shape fixed! '



    if offset and not fixShapePar:
        #This is the -log(L) of the fitted model:
        # a) The null model
