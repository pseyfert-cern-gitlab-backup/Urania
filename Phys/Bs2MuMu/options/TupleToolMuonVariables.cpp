  ...
  , m_dist(0)
{
  ...
  declareProperty("isoBDT_xmlFilePath",m_isoBDT_xmlFilePath="./");
  
  declareProperty("dumpVeloTracks", m_dumpVeloTracks = false); 
  declareProperty("dumpUpstreamTracks", m_dumpUpstreamTracks = false); 
  //end MR
...
StatusCode TupleToolMuonVariables::initialize() {
  ...
  //pFinder = tool<IRelatedPVFinder>
  //("GenericParticle2PVRelator__p2PVWithIPChi2_OfflineDistanceCalculatorName_",this);
  ...
  //MR: initialize isoBDTs
  for (int i=0;i<4;i++) isobdt_reader[i] = new TMVA::Reader("!Color:!Silent");
  //
  // Legend set of vars: isobdt1: set1; isobdt2: set1+3; isobdt3: set1+3+4; isobdt4: set1+2+3+4
  //
...
StatusCode TupleToolMuonVariables::OtherB(const LHCb::Particle *part, 
  ...
  LHCb::Particles* m_allparts = getIfExists<LHCb::Particles>(m_ParticlePath);
  if (! m_allparts )
  {
  ...
  if ( otherBtracks>0 )
    {
      ...
      //const Gaudi::XYZVector otherB2 = otherB(&OTHERB_parts);
      //const Gaudi::XYZVector Bs2MuMuAnalysis::otherB(LHCb::Particles* otherb){
...
//   for (LHCb::Particle::ConstVector::const_iterator ipart= 
...
//     }
  ...
  test &= tuple ->column ("Jpsi_dist_chi2", TMath::Sqrt(chi2final));
  ...
  if (! c1) distChi = -100;
  test &= tuple ->column ("Jpsi_dist_chi2_r", TMath::Sqrt(distChi) );
  
  
  const LoKi::Types::Fun BPVVDCHI2 = LoKi::Cuts::VDCHI2(goodPV);
  double temp_dist = BPVVDCHI2(myJpsi); 
  test &= tuple ->column ("Another_Jpsi_dist_chi2_r", TMath::Sqrt(temp_dist) );
...
StatusCode TupleToolMuonVariables::fillIsolation(const LHCb::Particle *part,
  ...
  //std::cout<<"attention Hannah, filled ntuple (mup) "<<m_count_mup_iso1<<" "<<m_count_mup_iso2<<" "<<m_count_mup_isoD0<<std::endl;
  //std::cout<<"attention Hannah, filled ntuple (mum) "<<m_count_mum_iso1<<" "<<m_count_mum_iso2<<" "<<m_count_mum_isoD0<<std::endl;
...
std::vector<int>  TupleToolMuonVariables::getIso(const LHCb::Particle* B, int isotype, bool cutOnTracks){
  ...
  //std::cout<<"1  part size"<< parts.size()<<std::endl;
  if(parts.size() !=2) {
    return iso;
  }
  ...
  for ( int i2 = 0; i2 < 2; i2++) {
    ...
    for ( ip = allparts->begin(); ip != allparts->end() ; ++ip) {
      //std::cout<< "Loop over all particles L924"<< std::endl;
      ...
      j++; 
      //if not DTF stops between O printout and here
      ...
      verts = getIfExists<LHCb::RecVertex::Container>(m_PVInputLocation);
      for ( iv = verts->begin(); iv != verts->end(); iv++) {
...
StatusCode TupleToolMuonVariables::fillCosNK(const LHCb::Particle *part, 
  ...
  //  std::cout<<"End of Fill  cosnk "<<std::endl;
}
...
StatusCode TupleToolMuonVariables::ConstrMass(const LHCb::Particle* B, 
  ...
  //std::cout<<"In B mass "<< parts <<  std::endl;
  ...
  //std::cout<<"B mass, by constraining the Jpsi mass. For 3/4 body decays. "<<std::endl;
  if(!test) return StatusCode::FAILURE;
  else return StatusCode::SUCCESS;
...
StatusCode TupleToolMuonVariables::CDFIsolation_2(const LHCb::Particle* B,
  ...
  // std::cout<<"To match the CERN definition... should implement this in CDFisolation and remove this"<<std::endl;
...
StatusCode TupleToolMuonVariables::CDFIsolation(const LHCb::Particle* B,
  ...
  //std::cout<<"compute CDF isolation variable"<<std::endl;
  if (test)   return StatusCode::SUCCESS;
  else return StatusCode::FAILURE;
...
StatusCode TupleToolMuonVariables::ATLASIsolation(const LHCb::Particle* B,
  ...
  //std::cout<<" ATLAS isolation "<<std::endl;
...
StatusCode TupleToolMuonVariables::fillMCTruth(const LHCb::Particle *part, 
  ...
  const LHCb::GenHeader* gh = getIfExists<LHCb::GenHeader>(LHCb::GenHeaderLocation::Default);
  ...
  //std::cout<<"Fill MC quantities  "<<std::endl;
  if(test) return StatusCode::SUCCESS;
  else return StatusCode::FAILURE; 
...
//
// MR: Recursive function which put in std::vectors the info of particles belonging to decay Tree
...
StatusCode TupleToolMuonVariables::fillUpstreamTracksInfo(Tuples::Tuple& tuple){
  ...
  test &= tuple->farray( "US_par_fmpos_x", par_fmpos_x.begin(), par_fmpos_x.end(), "US_num", US_maxnum );//position at 'firstMeasurement'
  test &= tuple->farray( "US_par_fmpos_y", par_fmpos_y.begin(), par_fmpos_y.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_fmpos_z", par_fmpos_z.begin(), par_fmpos_z.end(), "US_num", US_maxnum );

  test &= tuple->farray( "US_par_xp_cov00", par_px_cov00.begin(), par_px_cov00.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov11", par_px_cov11.begin(), par_px_cov11.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov22", par_px_cov22.begin(), par_px_cov22.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov33", par_px_cov33.begin(), par_px_cov33.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov44", par_px_cov44.begin(), par_px_cov44.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov55", par_px_cov55.begin(), par_px_cov55.end(), "US_num", US_maxnum );

  test &= tuple->farray( "US_par_xp_cov01", par_px_cov01.begin(), par_px_cov01.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov02", par_px_cov02.begin(), par_px_cov02.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov12", par_px_cov12.begin(), par_px_cov12.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov34", par_px_cov34.begin(), par_px_cov34.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov35", par_px_cov35.begin(), par_px_cov35.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov45", par_px_cov45.begin(), par_px_cov45.end(), "US_num", US_maxnum );

  test &= tuple->farray( "US_par_xp_cov03", par_px_cov03.begin(), par_px_cov03.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov04", par_px_cov04.begin(), par_px_cov04.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov05", par_px_cov05.begin(), par_px_cov05.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov13", par_px_cov13.begin(), par_px_cov13.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov14", par_px_cov14.begin(), par_px_cov14.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov15", par_px_cov15.begin(), par_px_cov15.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov23", par_px_cov23.begin(), par_px_cov23.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov24", par_px_cov24.begin(), par_px_cov24.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_xp_cov25", par_px_cov25.begin(), par_px_cov25.end(), "US_num", US_maxnum );
  ...
  test &= tuple->farray( "US_par_slope_covxx", par_slope_covxx.begin(), par_slope_covxx.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_slope_covyy", par_slope_covyy.begin(), par_slope_covyy.end(), "US_num", US_maxnum );
  test &= tuple->farray( "US_par_slope_covxy", par_slope_covxy.begin(), par_slope_covxy.end(), "US_num", US_maxnum );
  ...
  test &= tuple->farray( "US_par_clo", par_clo.begin(), par_clo.end(), "US_num", US_maxnum );
  ...
  //fill the truth-match info in the ntuple 
  if (m_isMC){
    fillTruthMatchedNtuple(tuple,"US",US_maxnum);
  }
  if(test) return StatusCode::SUCCESS;
  else return StatusCode::FAILURE; 
...
void TupleToolMuonVariables::fillTruthMatchedInfo(const LHCb::Track* track,LHCb::Track2MC2D* table2d){
  bool matched = kFALSE;
  bool hasMother = kFALSE;
  bool hasGranny = kFALSE;
  bool hasMotGranny = kFALSE;
  ...
  if (!mcp) {
    _MC_ori_vtx_x.push_back(-9999);
    _MC_ori_vtx_y.push_back(-9999);
    _MC_ori_vtx_z.push_back(-9999);
    _MC_ori_type.push_back(-9999);
    _MC_par_trueID.push_back(-9999);
    _MC_par_isnoniso_mup.push_back(-9999);
    _MC_par_isnoniso_mum.push_back(-9999);
    _MC_par_px.push_back(-9999);
    _MC_par_py.push_back(-9999);
    _MC_par_pz.push_back(-9999);
    _MC_par_M.push_back(-9999);
    _MC_par_E.push_back(-9999);
    _MC_origin_id.push_back(-9999);
  }
  else {
    matched = kTRUE;
    _MC_ori_vtx_x.push_back(mcp->originVertex()->position().x());
    _MC_ori_vtx_y.push_back(mcp->originVertex()->position().y());
    _MC_ori_vtx_z.push_back(mcp->originVertex()->position().z());
    _MC_ori_type.push_back(mcp->originVertex()->type());
    
    _MC_par_trueID.push_back(mcp->particleID().pid());
    _MC_par_isnoniso_mup.push_back(isNonIso(mcp,"mu+"));
    _MC_par_isnoniso_mum.push_back(isNonIso(mcp,"mu-"));
    _MC_par_px.push_back(mcp->momentum().Px());
    _MC_par_py.push_back(mcp->momentum().Py());
    _MC_par_pz.push_back(mcp->momentum().Pz());
    _MC_par_M.push_back(mcp->momentum().M());
    _MC_par_E.push_back(mcp->momentum().E());
    
    //Ancestor info
    const LHCb::MCParticle* ancestor = originof(mcp) ;
    _MC_origin_id.push_back(ancestor->particleID().pid());
    
    //MC mother info
    const LHCb::MCParticle* mother = mcp->mother();
    if (mother) {
      hasMother = kTRUE;
      _MC_par_hasMother.push_back(kTRUE);
      _MC_mot_ori_vtx_x.push_back(mother->originVertex()->position().x());
      _MC_mot_ori_vtx_y.push_back(mother->originVertex()->position().y());
      _MC_mot_ori_vtx_z.push_back(mother->originVertex()->position().z());
      
      _MC_mot_trueID.push_back(mother->particleID().pid());
      _MC_mot_px.push_back(mother->momentum().Px());
      _MC_mot_py.push_back(mother->momentum().Py());
      _MC_mot_pz.push_back(mother->momentum().Pz());
      _MC_mot_M.push_back(mother->momentum().M());
      _MC_mot_E.push_back(mother->momentum().E());
      
      //MC grandmother info
      const LHCb::MCParticle* granny = mother->mother();
      if (granny) {
	hasGranny = kTRUE;
	_MC_mot_hasMother.push_back(kTRUE);
	
	_MC_gra_ori_vtx_x.push_back(granny->originVertex()->position().x());
	_MC_gra_ori_vtx_y.push_back(granny->originVertex()->position().y());
	_MC_gra_ori_vtx_z.push_back(granny->originVertex()->position().z());
	
	_MC_gra_trueID.push_back(granny->particleID().pid());
	_MC_gra_px.push_back(granny->momentum().Px());
	_MC_gra_py.push_back(granny->momentum().Py());
	_MC_gra_pz.push_back(granny->momentum().Pz());
	_MC_gra_M.push_back(granny->momentum().M());
	_MC_gra_E.push_back(granny->momentum().E());
	
	//MC mother of grandmother info
	const LHCb::MCParticle* motgranny = granny->mother();
	if (motgranny) {
	  hasMotGranny = kTRUE;
	  _MC_gra_hasMother.push_back(kTRUE);
	  _MC_mogra_ori_vtx_x.push_back(motgranny->originVertex()->position().x());
	  _MC_mogra_ori_vtx_y.push_back(motgranny->originVertex()->position().y());
	  _MC_mogra_ori_vtx_z.push_back(motgranny->originVertex()->position().z());
	  
	  _MC_mogra_trueID.push_back(motgranny->particleID().pid());
	  _MC_mogra_px.push_back(motgranny->momentum().Px());
	  _MC_mogra_py.push_back(motgranny->momentum().Py());
	  _MC_mogra_pz.push_back(motgranny->momentum().Pz());
	  _MC_mogra_M.push_back(motgranny->momentum().M());
	  _MC_mogra_E.push_back(motgranny->momentum().E());
	  
	  //MC grandmother of grandmother info
	  const LHCb::MCParticle* gragranny = motgranny->mother();
	  if (gragranny) {
	    hasGraGranny = kTRUE;
	    _MC_mogra_hasMother.push_back(kTRUE);
	    _MC_grgra_ori_vtx_x.push_back(gragranny->originVertex()->position().x());
	    _MC_grgra_ori_vtx_y.push_back(gragranny->originVertex()->position().y());
	    _MC_grgra_ori_vtx_z.push_back(gragranny->originVertex()->position().z());
	    
	    _MC_grgra_trueID.push_back(gragranny->particleID().pid());
	    _MC_grgra_px.push_back(gragranny->momentum().Px());
	    _MC_grgra_py.push_back(gragranny->momentum().Py());
	    _MC_grgra_pz.push_back(gragranny->momentum().Pz());
	    _MC_grgra_M.push_back(gragranny->momentum().M());
	    _MC_grgra_E.push_back(gragranny->momentum().E());
	  } else hasGraGranny = kFALSE;//Granny of granny
	} else hasMotGranny = kFALSE;//Mother of granny
      } else hasGranny = kFALSE;//Granny
    } else hasMother = kFALSE;//Mother
  }// end mcp!=0 condition
  if (!hasMother) {      
    _MC_par_hasMother.push_back(kFALSE);
    _MC_mot_ori_vtx_x.push_back(-9999);
    _MC_mot_ori_vtx_y.push_back(-9999);
    _MC_mot_ori_vtx_z.push_back(-9999);
    
    _MC_mot_trueID.push_back(-9999);
    _MC_mot_px.push_back(-9999);
    _MC_mot_py.push_back(-9999);
    _MC_mot_pz.push_back(-9999);
    _MC_mot_M.push_back(-9999);
    _MC_mot_E.push_back(-9999);
  }
  if(!hasGranny) {
    _MC_mot_hasMother.push_back(kFALSE);
    _MC_gra_ori_vtx_x.push_back(-9999);
    _MC_gra_ori_vtx_y.push_back(-9999);
    _MC_gra_ori_vtx_z.push_back(-9999);
    
    _MC_gra_trueID.push_back(-9999);
    _MC_gra_px.push_back(-9999);
    _MC_gra_py.push_back(-9999);
    _MC_gra_pz.push_back(-9999);
    _MC_gra_M.push_back(-9999);
    _MC_gra_E.push_back(-9999);
  }
  if(!hasMotGranny) {
    _MC_gra_hasMother.push_back(kFALSE);
    _MC_mogra_ori_vtx_x.push_back(-9999);
    _MC_mogra_ori_vtx_y.push_back(-9999);
    _MC_mogra_ori_vtx_z.push_back(-9999);
    
    _MC_mogra_trueID.push_back(-9999);
    _MC_mogra_px.push_back(-9999);
    _MC_mogra_py.push_back(-9999);
    _MC_mogra_pz.push_back(-9999);
    _MC_mogra_M.push_back(-9999);
    _MC_mogra_E.push_back(-9999);
  }
  
  if(!hasGraGranny) {
    _MC_mogra_hasMother.push_back(kFALSE);
    _MC_grgra_ori_vtx_x.push_back(-9999);
    _MC_grgra_ori_vtx_y.push_back(-9999);
    _MC_grgra_ori_vtx_z.push_back(-9999);
    
    _MC_grgra_trueID.push_back(-9999);
    _MC_grgra_px.push_back(-9999);
    _MC_grgra_py.push_back(-9999);
    _MC_grgra_pz.push_back(-9999);
    _MC_grgra_M.push_back(-9999);
    _MC_grgra_E.push_back(-9999);
  }
  _MC_match.push_back(matched);
  
  return;
}//end fillTruthMatchedInfo

void TupleToolMuonVariables::ClearTruthMatchedVectors(){
  _MC_match.clear();
  _MC_par_hasMother.clear();
  _MC_mot_hasMother.clear();
  _MC_gra_hasMother.clear();
  _MC_mogra_hasMother.clear();
  ...
  _MC_par_M.clear(); 
  _MC_par_E.clear();
  _MC_par_trueID.clear();
  _MC_par_isnoniso_mup.clear();
  _MC_par_isnoniso_mum.clear();  
  _MC_mot_ori_vtx_x.clear();
  _MC_mot_ori_vtx_y.clear(); 
  _MC_mot_ori_vtx_z.clear(); 
  _MC_mot_px.clear(); 
  _MC_mot_py.clear(); 
  _MC_mot_pz.clear(); 
  _MC_mot_M.clear(); 
  _MC_mot_E.clear();
  _MC_mot_trueID.clear();
  
  _MC_gra_ori_vtx_x.clear();
  _MC_gra_ori_vtx_y.clear(); 
  _MC_gra_ori_vtx_z.clear(); 
  _MC_gra_px.clear(); 
  _MC_gra_py.clear(); 
  _MC_gra_pz.clear(); 
  _MC_gra_M.clear(); 
  _MC_gra_E.clear();
  _MC_gra_trueID.clear();
  
  _MC_mogra_ori_vtx_x.clear();
  _MC_mogra_ori_vtx_y.clear(); 
  _MC_mogra_ori_vtx_z.clear(); 
  _MC_mogra_px.clear(); 
  _MC_mogra_py.clear(); 
  _MC_mogra_pz.clear(); 
  _MC_mogra_M.clear(); 
  _MC_mogra_E.clear();
  _MC_mogra_trueID.clear();
  
  _MC_grgra_ori_vtx_x.clear();
  _MC_grgra_ori_vtx_y.clear(); 
  _MC_grgra_ori_vtx_z.clear(); 
  _MC_grgra_px.clear(); 
  _MC_grgra_py.clear(); 
  _MC_grgra_pz.clear(); 
  _MC_grgra_M.clear(); 
  _MC_grgra_E.clear();
  _MC_grgra_trueID.clear();
  
  return;
}//end ClearTruthMatchedVectors

void TupleToolMuonVariables::fillTruthMatchedNtuple(Tuples::Tuple& tuple,TString block_name,const int max_num=250){
  bool test=true;
  TString num_flag=block_name+"_num";

  test &= tuple->farray((block_name+"_isMCMatched").Data(),_MC_match.begin(),_MC_match.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_Origin_ID").Data(),_MC_origin_id.begin(),_MC_origin_id.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Ori_Vtx_x").Data(),_MC_ori_vtx_x.begin(),_MC_ori_vtx_x.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Ori_Vtx_y").Data(),_MC_ori_vtx_y.begin(),_MC_ori_vtx_y.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Ori_Vtx_z").Data(),_MC_ori_vtx_z.begin(),_MC_ori_vtx_z.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Ori_type").Data(),_MC_ori_type.begin(),_MC_ori_type.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_ID").Data(),_MC_par_trueID.begin(),_MC_par_trueID.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_isnoniso_mup").Data(),_MC_par_isnoniso_mup.begin(),_MC_par_isnoniso_mup.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_isnoniso_mum").Data(),_MC_par_isnoniso_mum.begin(),_MC_par_isnoniso_mum.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Px").Data(),_MC_par_px.begin(),_MC_par_px.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Py").Data(),_MC_par_py.begin(),_MC_par_py.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_Pz").Data(),_MC_par_pz.begin(),_MC_par_pz.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_E").Data(),_MC_par_E.begin(),_MC_par_E.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_part_M").Data(),_MC_par_M.begin(),_MC_par_M.end(),num_flag.Data(), max_num );
  
  test &= tuple->farray((block_name+"_true_part_hasMother").Data(),_MC_par_hasMother.begin(),_MC_par_hasMother.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Ori_Vtx_x").Data(),_MC_mot_ori_vtx_x.begin(),_MC_mot_ori_vtx_x.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Ori_Vtx_y").Data(),_MC_mot_ori_vtx_y.begin(),_MC_mot_ori_vtx_y.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Ori_Vtx_z").Data(),_MC_mot_ori_vtx_z.begin(),_MC_mot_ori_vtx_z.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_ID").Data(),_MC_mot_trueID.begin(),_MC_mot_trueID.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Px").Data(),_MC_mot_px.begin(),_MC_mot_px.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Py").Data(),_MC_mot_py.begin(),_MC_mot_py.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_Pz").Data(),_MC_mot_pz.begin(),_MC_mot_pz.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_E").Data(),_MC_mot_E.begin(),_MC_mot_E.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_moth_M").Data(),_MC_mot_M.begin(),_MC_mot_M.end(),num_flag.Data(), max_num );
  
  test &= tuple->farray((block_name+"_true_moth_hasMother").Data(),_MC_mot_hasMother.begin(),_MC_mot_hasMother.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Ori_Vtx_x").Data(),_MC_gra_ori_vtx_x.begin(),_MC_gra_ori_vtx_x.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Ori_Vtx_y").Data(),_MC_gra_ori_vtx_y.begin(),_MC_gra_ori_vtx_y.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Ori_Vtx_z").Data(),_MC_gra_ori_vtx_z.begin(),_MC_gra_ori_vtx_z.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_ID").Data(),_MC_gra_trueID.begin(),_MC_gra_trueID.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Px").Data(),_MC_gra_px.begin(),_MC_gra_px.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Py").Data(),_MC_gra_py.begin(),_MC_gra_py.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_Pz").Data(),_MC_gra_pz.begin(),_MC_gra_pz.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_E").Data(),_MC_gra_E.begin(),_MC_gra_E.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_gran_M").Data(),_MC_gra_M.begin(),_MC_gra_M.end(),num_flag.Data(), max_num );
  
  test &= tuple->farray((block_name+"_true_gran_hasMother").Data(),_MC_gra_hasMother.begin(),_MC_gra_hasMother.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Ori_Vtx_x").Data(),_MC_mogra_ori_vtx_x.begin(),_MC_mogra_ori_vtx_x.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Ori_Vtx_y").Data(),_MC_mogra_ori_vtx_y.begin(),_MC_mogra_ori_vtx_y.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Ori_Vtx_z").Data(),_MC_mogra_ori_vtx_z.begin(),_MC_mogra_ori_vtx_z.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_ID").Data(),_MC_mogra_trueID.begin(),_MC_mogra_trueID.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Px").Data(),_MC_mogra_px.begin(),_MC_mogra_px.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Py").Data(),_MC_mogra_py.begin(),_MC_mogra_py.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_Pz").Data(),_MC_mogra_pz.begin(),_MC_mogra_pz.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_E").Data(),_MC_mogra_E.begin(),_MC_mogra_E.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_mogr_M").Data(),_MC_mogra_M.begin(),_MC_mogra_M.end(),num_flag.Data(), max_num );
  
  test &= tuple->farray((block_name+"_true_mogr_hasMother").Data(),_MC_mogra_hasMother.begin(),_MC_mogra_hasMother.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Ori_Vtx_x").Data(),_MC_grgra_ori_vtx_x.begin(),_MC_grgra_ori_vtx_x.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Ori_Vtx_y").Data(),_MC_grgra_ori_vtx_y.begin(),_MC_grgra_ori_vtx_y.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Ori_Vtx_z").Data(),_MC_grgra_ori_vtx_z.begin(),_MC_grgra_ori_vtx_z.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_ID").Data(),_MC_grgra_trueID.begin(),_MC_grgra_trueID.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Px").Data(),_MC_grgra_px.begin(),_MC_grgra_px.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Py").Data(),_MC_grgra_py.begin(),_MC_grgra_py.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_Pz").Data(),_MC_grgra_pz.begin(),_MC_grgra_pz.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_E").Data(),_MC_grgra_E.begin(),_MC_grgra_E.end(),num_flag.Data(), max_num );
  test &= tuple->farray((block_name+"_true_grgr_M").Data(),_MC_grgra_M.begin(),_MC_grgra_M.end(),num_flag.Data(), max_num );

  //clear the content of the _MC_ vectors (for next use)
  ClearTruthMatchedVectors();
  
  return;

}//end fillTruthMatchedNtuple
...
void TupleToolMuonVariables::init_muonsMCDecayTree(const LHCb::Particle* part){
  if (!m_isMC) return;

  //clear the mup/mum mc decaytree vectors
  _mup_mcdecaytree_vec.clear();
  _mum_mcdecaytree_vec.clear();
  //get the MCParticle* of the two muons
  const LHCb::Particle* vdau1 = part->daughtersVector().at(0);
  const LHCb::Particle* vdau2 = part->daughtersVector().at(1);
  const LHCb::MCParticle* mc_mup;
  const LHCb::MCParticle* mc_mum;
  if (vdau1->charge()>0){
    mc_mup = m_p2mcAssoc->relatedMCP(vdau1);
    mc_mum = m_p2mcAssoc->relatedMCP(vdau2);
  }								
  else{
    mc_mum = m_p2mcAssoc->relatedMCP(vdau1);
    mc_mup = m_p2mcAssoc->relatedMCP(vdau2);
  }
  //fill the decaytree vectors
  if (mc_mup) get_decayTree(mc_mup,_mup_mcdecaytree_vec); 
  if (mc_mum) get_decayTree(mc_mum,_mum_mcdecaytree_vec); 
  return;
  
}//end init_muonsMCDecayTree
