...
void JpsiKsPdf::setTails(){
  if(m_setup=="Optimal"){
    ...
    if(m_alpha) m_alpha->setVal(m_ttype==3 ? 1.375 : (m_ttype==5 ? 2.326 : 2.305));
    if(m_n) m_n->setVal(m_ttype==3 ? 3.32 : (m_ttype==5 ? 1.72 : 1.75));
    if(m_alpha2) m_alpha2->setVal(m_ttype==3 ? -1.470 : (m_ttype==5 ? -2.240 : -2.207));
    if(m_n2) m_n2->setVal(m_ttype==3 ? 125.2 : (m_ttype==5 ? 6.8 : 7.4));
    setConstant("Tails");
  }
...
void JpsiKsPdf::plotLegend(double x1,double y1,double x2,double y2, TString what) const {
  ...
  if (m_Bs) {
    if (sWhat.find("mass")!=std::string::npos) {
      HBs->SetLineColor(0);  
      HBs->SetFillColor(m_BsCol);
      leg->AddEntry(HBs,"B_{s}^{0}#rightarrow J/#psiK_{S}^{0}","f");
    }
    else if (sWhat.find("time")!=std::string::npos) {
      HBs->SetLineColor(1);  
      HBs->SetLineStyle(4);
      if (!(sWhat.find("Bkg")!=std::string::npos)) {
  ...
  if (m_Bd) {
    if (sWhat.find("mass")!=std::string::npos) {
      ...
      HBd->SetLineColor(1);  
      HBd->SetLineStyle(6);
      if (sWhat.find("All")!=std::string::npos) {
  ...
  if (m_Kstar) leg->AddEntry(HKstar,"B^{0}#rightarrow J/#psiK^{*0}","f");
