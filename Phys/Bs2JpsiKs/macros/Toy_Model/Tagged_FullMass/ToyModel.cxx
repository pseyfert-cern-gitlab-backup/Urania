...
void ToyModel::DefiningConstantsAndVariables(int tracktype, double theLumi){
  ...
  epsilon = new RooRealVar("epsilon","CKM suppression",pow(0.2252,2)/(1-pow(0.2252,2)));

  // *** Lifetime Constants ***
  tau_Bd = new RooRealVar("tau_Bd","Average lifetime for Bd",1.519,"ps");// HFAG - Fall 2012
  dG_Bd  = new RooRealVar("dG_Bd","Lifetime difference for Bs",0.0,"ps^-1"); // Assumption
  dM_Bd  = new RooRealVar("dM_Bd","Mass difference between light and heavy weak-eigenstate for Bs",0.507,"ps^-1");// HFAG - Fall 2012
  ...
  // *** CP Observables *** 
  Robs_Bd_input = new RooFormulaVar("Robs_Bd_input","R observable for Bs", "1+2*@3*@0*cos(@1)*cos(@2)+pow(@3*@0,2)",RooArgList(*a_penguin,*theta_penguin,*gamma,*epsilon));
  Adir_Bd_input = new RooFormulaVar("Adir_Bd_input","Direct CP asymmetry for Bs", "(-2*@3*@0*sin(@1)*sin(@2))/(1+2*@3*@0*cos(@1)*cos(@2)+pow(@3*@0,2))",RooArgList(*a_penguin,*theta_penguin,*gamma,*epsilon));
  Amix_Bd_input = new RooFormulaVar("Amix_Bd_input","Mixing induced CP asymmetry for Bs", "@4*(sin(@3)+2*@5*@0*cos(@1)*sin(@3+@2)+pow(@5*@0,2)*sin(@3+2*@2))/(1+2*@5*@0*cos(@1)*cos(@2)+pow(@5*@0,2))",RooArgList(*a_penguin,*theta_penguin,*gamma,*phi_Bs,*eta,*epsilon));
  ADG_Bd_input  = new RooFormulaVar("ADG_Bd_input" ,"Delta Gamma CP asymmetry for Bs", "-1*@4*(cos(@3)+2*@5*@0*cos(@1)*cos(@3+@2)+pow(@5*@0,2)*cos(@3+2*@2))/(1+2*@5*@0*cos(@1)*cos(@2)+pow(@5*@0,2))",RooArgList(*a_penguin,*theta_penguin,*gamma,*phi_Bs,*eta,*epsilon));  

  Adir_Bd = new RooRealVar("Adir_Bd","Direct CP asymmetry for Bs",Adir_Bd_input->getVal() ,-3.,3.);
  Amix_Bd = new RooRealVar("Amix_Bd","Mixing induced CP asymmetry for Bs",Amix_Bd_input->getVal(),-3.,3.);
  ADG_Bd  = new RooRealVar("ADG_Bd" ,"Delta Gamma CP asymmetry for Bs", ADG_Bd_input->getVal());// not accessible with DG=0
  ...
  mistag_Bd = new RooRealVar("mistag_Bd","Mistag fraction of original B and Bbar",0.3650);// LHCB-PAPER-2012-035
  tageff_Bd = new RooRealVar("tageff_Bd","Tagging efficiency",0.3265);// LHCB-PAPER-2012-035
  mistag_Bs = new RooRealVar("mistag_Bs","Mistag fraction of original B and Bbar",(0.367*(0.332-0.332*0.121) + 0.344*0.121)/(0.332 + 0.121 - 0.332*0.121));// LHCB-CONF-2012-026 && LHCB-CONF-2012-050
  tageff_Bs = new RooRealVar("tageff_Bs","Tagging efficiency",0.332 + 0.121 - 0.332*0.121);// LHCB-CONF-2012-026 && LHCB-CONF-2012-050
  ...
  tagNorm_Bd = new RooFormulaVar("tagNorm_Bd","Normalisation using tagging eff","@0!=0 ? 0.5*@1 : 1-@1",RooArgList(*tag,*tageff_Bd));
  ACosh_Bd = new RooFormulaVar("ACosh_Bd","Parameter of the Cosh in decay equation for Bd","@0",RooArgList(*tagNorm_Bd));
  DSinh_Bd = new RooFormulaVar("DSinh_Bd","Parameter of the Sinh in decay equation for Bd","@0*@1",RooArgList(*tagNorm_Bd,*ADG_Bd));
  CCos_Bd = new RooFormulaVar("CCos_Bd","Parameter of the Cos in decay equation for Bd","@0*@3*@1*(1-2*@2)",RooArgList(*tagNorm_Bd,*Adir_Bd,*mistag_Bd,*tag));
  SSin_Bd = new RooFormulaVar("SSin_Bd","Parameter of the Sin in decay equation for Bd","@0*@3*@1*(1-2*@2)",RooArgList(*tagNorm_Bd,*Amix_Bd,*mistag_Bd,*tag));
  
  tagNorm_Bs = new RooFormulaVar("tagNorm_Bs","Normalisation using tagging eff","@0!=0 ? 0.5*@1 : 1-@1",RooArgList(*tag,*tageff_Bs));
  ACosh_Bs = new RooFormulaVar("ACosh_Bs","Parameter of the Cosh in decay equation for Bs","@0",RooArgList(*tagNorm_Bs));
  DSinh_Bs = new RooFormulaVar("DSinh_Bs","Parameter of the Sinh in decay equation for Bs","@0*@1",RooArgList(*tagNorm_Bs,*ADG_Bs));
  CCos_Bs = new RooFormulaVar("CCos_Bs","Parameter of the Cos in decay equation for Bs","@0*@3*@1*(1-2*@2)",RooArgList(*tagNorm_Bs,*Adir_Bs,*mistag_Bs,*tag));
  SSin_Bs = new RooFormulaVar("SSin_Bs","Parameter of the Sin in decay equation for Bs","@0*@3*@1*(1-2*@2)",RooArgList(*tagNorm_Bs,*Amix_Bs,*mistag_Bs,*tag));
...
void ToyModel::BuildPdf(){
  ...
  // *** Binned Acceptance ***
  Btime->setBinning(RooUniformBinning(Btime->getMin(), Btime->getMax(), 200, "acceptanceBinning"), "acceptanceBinning");
  sigAccBinned = new RooBinnedPdf("sigAccBinned", "sigAccBinned", *Btime, "acceptanceBinning", *acceptance);
  sigAccBinned->setForceUnitIntegral(true);
  bkgAccBinned = new RooBinnedPdf("bkgAccBinned", "bkgAccBinned", *Btime, "acceptanceBinning", *bkgAcc);
  bkgAccBinned->setForceUnitIntegral(true);
  // Add Acceptance to resolution model
  sigResModel = new RooEffResModel("sigResModel", "sigResModel", *resModel, *sigAccBinned);
  bkgResModel = new RooEffResModel("bkgResModel", "bkgResModel", *resModel, *bkgAccBinned);
  
  // *** Signal ***
  decay_Bd = new RooBDecay("decay_Bd","Fitted Decay",*Btime,*tau_Bd,*dG_Bd,*ACosh_Bd,*DSinh_Bd,*CCos_Bd,*SSin_Bd,*dM_Bd,*sigResModel,RooBDecay::SingleSided);
  decay_Bs = new RooBDecay("decay_Bs","Fitted Decay",*Btime,*tau_Bs,*dG_Bs,*ACosh_Bs,*DSinh_Bs,*CCos_Bs,*SSin_Bs,*dM_Bs,*sigResModel,RooBDecay::SingleSided);
  ...
  decay_acc_bkg = new RooDecay("decay_acc_bkg","Decay with acceptance",*Btime,*tau_long,*bkgResModel,RooDecay::SingleSided);
  if(TrackType==3){
  ...
  total_Bd = new RooProdPdf("total_Bd","Total PDF - Signal Bd",RooArgList(*Bmass_Sig_Bd,*decay_Bd));
  total_Bs = new RooProdPdf("total_Bs","Total PDF - Signal Bs",RooArgList(*Bmass_Sig_Bs,*decay_Bs));  
  total_Bkg = new RooProdPdf("total_Bkg","Total PDF - Background",RooArgList(*Bmass_Exp_bkg,*decay_background));  

  // *** Composite PDFs ***
  total_pdf = new RooAddPdf("total_pdf","Total PDF",RooArgList(*total_Bd,*total_Bs,*total_Bkg),RooArgList(*nBd_fit,*nBs_fit,*nBkg_fit),false);
...
void ToyModel::Plotting(TString label){
  ...
  total_pdf->plotOn(tagFrame,Slice(*tag,"B"),Components("total_Bd"),LineWidth(2),LineColor(2));
  ...
  std::cout << "*** Plotting PDF components: Tagged Decay Time in Bs mass window ***" << std::endl;

  // *** Datapoints and distribution ***
  RooPlot *tagBsFrame = Btime->frame(Bins(74)); 
  tagBsFrame->SetYTitle("# Events/(0.2 ps)");
  tagBsFrame->SetXTitle("B decay time (ps)");

  tagBsFrame->SetTitleOffset(1.3,"y");
  tagBsFrame->SetTitleSize(0.07,"x");
  tagBsFrame->SetTitleSize(0.07,"y");
  tagBsFrame->SetTitleFont(132,"y");
  tagBsFrame->SetTitleFont(132,"x");
  tagBsFrame->SetLabelSize(0.05,"x");
  tagBsFrame->SetLabelSize(0.05,"y");
  tagBsFrame->SetLabelFont(132,"x");
  tagBsFrame->SetLabelFont(132,"y");
  
  // *** Full Tagged Sample ***
  //Bmass->setRange("BsOnly",5340,5390);
  Bmass->setRange("BsOnly",5340,5520);
  ToyData->plotOn(tagBsFrame,Cut("tag==1"),CutRange("BsOnly"),MarkerStyle(21),MarkerSize(0.8));
  total_pdf->plotOn(tagBsFrame,ProjectionRange("BsOnly"),Slice(*tag,"B"),Components("total_Bd"),LineWidth(2),LineColor(2));
  total_pdf->plotOn(tagBsFrame,ProjectionRange("BsOnly"),Slice(*tag,"B"),Components("total_Bs"),LineWidth(2),LineColor(3));
  total_pdf->plotOn(tagBsFrame,ProjectionRange("BsOnly"),Slice(*tag,"B"),Components("total_Bkg"),LineWidth(2),LineColor(1),LineStyle(kDashed));
  total_pdf->plotOn(tagBsFrame,ProjectionRange("BsOnly"),Slice(*tag,"B"),LineWidth(2),LineColor(4));
  
  // General Settings
  tagBsFrame->SetMinimum(1);
  tagBsFrame->SetMaximum(100);
  
  // Add Pull Distribution
  PullPlot("ToyModel_Btime_BsOnly_Tagged_"+track+"_"+label+".pdf",tagBsFrame,1);
...
void ToyModel::FittingParameters(TString setup){
  ...
  // Systematics Fits
  mistag_Bd->setVal(setup=="mistag+" ? 0.3650+0.0075 : (setup=="mistag-" ? 0.3650-0.0075 : 0.3650));
  tageff_Bd->setVal(setup=="tageff+" ? 0.3265+0.0032 : (setup=="tageff-" ? 0.3265-0.0032 : 0.3265));
  Adir_Bd->setVal(setup=="Adir+" ? Adir_Bd_input->getVal()+0.026 : (setup=="Adir-" ? Adir_Bd_input->getVal()-0.026 : Adir_Bd_input->getVal()));
  Amix_Bd->setVal(setup=="Amix+" ? Amix_Bd_input->getVal()+0.024 : (setup=="Amix-" ? Amix_Bd_input->getVal()-0.024 : Amix_Bd_input->getVal()));
  
  std::cout << "Systematics: Fit Values" << std::endl;
  std::cout << "Mistag: " << mistag_Bd->getVal() << std::endl;
  std::cout << "Tagging Efficiency: " << tageff_Bd->getVal() << std::endl;
  std::cout << "Adir: " << Adir_Bd->getVal() << std::endl;
  std::cout << "Amix: " << Amix_Bd->getVal() << std::endl;
...
void ToyModel::Saving(TString setup){
