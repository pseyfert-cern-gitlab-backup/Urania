...
void toyFit(int tracktype,TString lumi, TString param){
  ...
  for(Long64_t i=0; i<nEntries_All; i++){
    ...
    if(covQual==3 && ADG_Err>0.2 && ADG_Err<2.){
    ...
    else if(covQual==3 && count<1000) bad_Err++;
    count++;
  }
  ...
  FitPlotParam(Val,data,"SepFullMass_"+param+"_Val"+track+".pdf",Results_Val,lumi);
  
  double Results_Err[] = {-999,-999,-999,-999};
  FitPlotParam(Err,data,"SepFullMass_"+param+"_Err"+track+".pdf",Results_Err,lumi);
  
  double Results_Pull[] = {-999,-999,-999,-999};
  FitPlotParam(Pull,data,"SepFullMass_"+param+"_Pull"+track+".pdf",Results_Pull,lumi);
  ...
  std::cout << "Due to unrealistic error estimate: " << bad_Err << "/1000" << std::endl;
  std::cout << param << " & Val & " << nEntries_Good << "/1000" << " & ";
  std::cout << Results_Val[0] << " ! " << Results_Val[1] << " & ";
  std::cout << Results_Val[2] << " ! " << Results_Val[3] << std::endl;
  std::cout << param << " & Err & " << nEntries_Good << "/1000" << " & ";
  std::cout << Results_Err[0] << " ! " << Results_Err[1] << " & ";
  std::cout << Results_Err[2] << " ! " << Results_Err[3] << std::endl;
  std::cout << param << " & Pull & " << nEntries_Good << "/1000" << " & ";
