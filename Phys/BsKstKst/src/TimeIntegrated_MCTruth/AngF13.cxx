...
ClassImp(AngF13) 
 ...
 Double_t AngF13::evaluate() const 
 { 
...
Double_t AngF13::analyticalIntegral(Int_t code, const char* rangeName) const
{
   ...
   if (code==1 || code==2 || code==3 || code==5){
     ...
     double x1 = hic1 + (c1_th*pow(hic1,2))/2. + (c2_th*pow(hic1,3))/3. + (c3_th*pow(hic1,4))/4. + (c4_th*pow(hic1,5))/5. + (c5_th*pow(hic1,6))/6.;
     double y1 = loc1 + (c1_th*pow(loc1,2))/2. + (c2_th*pow(loc1,3))/3. + (c3_th*pow(loc1,4))/4. + (c4_th*pow(loc1,5))/5. + (c5_th*pow(loc1,6))/6.;
