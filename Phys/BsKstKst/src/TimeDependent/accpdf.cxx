...
Double_t accpdf::accCos(Double_t x) const
 { 
	...
	return 1.+k1*x+k2*(2.*x*x-1.)+k3*(4.*x*x*x-3.*x)+k4*(8.*x*x*x*x-8.*x*x+1.);
...
Double_t accpdf::accTime(Double_t x) const 
 { 
	...
	return x*x*x/(a_acc+x*x*x)*(1.-b_acc*x);
...
Double_t accpdf::evaluate() const 
 {
	...
	if (option == 1) {return accMass(m1)*accMass(m2);}
	if (option == 2) {return accCos(cos1)*accCos(cos2);}
