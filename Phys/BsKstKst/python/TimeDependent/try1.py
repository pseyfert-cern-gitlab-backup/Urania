...
def MCsetcut(year_aux,set_aux):
	if year_aux == 0:
		if set_aux == 0: return "itype==-73"
		elif set_aux == 1: return "itype==-70"
		else: return "(itype==-73 || itype==-70)"
	else:
...
def LoadDataVis(data_file, data_tree, datatype, evnum_limit = 0):
	...
	tree_2011_wide = tree_full.CopyTree(t_name_MC+">=0. && "+t_name_MC+"<=12. && "+MCsetcut(0,datatype))
	tree_2012_wide = tree_full.CopyTree(t_name_MC+">=0. && "+t_name_MC+"<=12. && "+MCsetcut(1,datatype))
	tree_2011_narrow = tree_full.CopyTree("abs("+m1_name_MC+"-900.)<150. && abs("+m2_name_MC+"-900.)<150. && "+t_name_MC+">=0. && "+t_name_MC+"<=12. && "+MCsetcut(0,datatype))
	tree_2012_narrow = tree_full.CopyTree("abs("+m1_name_MC+"-900.)<150. && abs("+m2_name_MC+"-900.)<150. && "+t_name_MC+">=0. && "+t_name_MC+"<=12. && "+MCsetcut(1,datatype))
	...
	for i in tree_2011_wide:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))) and MCtruth(i):
			...
			MCweight1.setVal(invN_2011_wide/PDF_gen.evaluate())
	...
	for i in tree_2012_wide:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))) and MCtruth(i):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			t_MC.setVal(eval("i."+t_name_MC))
			MCweight1.setVal(invN_2012_wide/PDF_gen.evaluate())
	...
	for i in tree_2011_narrow:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))) and MCtruth(i):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			t_MC.setVal(eval("i."+t_name_MC))
			MCweight1.setVal(invN_2011_narrow/PDF_gen.evaluate())
	...
	for i in tree_2012_narrow:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))) and MCtruth(i):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			t_MC.setVal(eval("i."+t_name_MC))
			MCweight1.setVal(invN_2012_narrow/PDF_gen.evaluate())
...
def LoadDataNW(evnum_limit = 0):

	data_file = 'DVntuple_PhSp_AllBranches.root'
	data_tree = 'MCDecayTree'
	datatype = 0
	...
	tree_full = file_in.Get(data_tree)
	file_out = TFile(NTUPLE_PATH + "trash.root","RECREATE")
	tree_2011_wide = tree_full.CopyTree(m1_name_MC+">=750. && "+m2_name_MC+">=750. && "+m1_name_MC+"<=1700. && "+m2_name_MC+"<=1700.")
	tree_2012_wide = tree_full.CopyTree(m1_name_MC+">=750. && "+m2_name_MC+">=750. && "+m1_name_MC+"<=1700. && "+m2_name_MC+"<=1700.")
	tree_2011_narrow = tree_full.CopyTree("abs("+m1_name_MC+"-900.)<150. && abs("+m2_name_MC+"-900.)<150.")
	tree_2012_narrow = tree_full.CopyTree("abs("+m1_name_MC+"-900.)<150. && abs("+m2_name_MC+"-900.)<150.")

	PDF_gen = CreateGenPDF()

	data0_2011_wide_ = RooDataSet("data0_2011_wide_","data0_2011_wide_",RooArgSet(mKp1_MC,mKp2_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
	data0_2012_wide_ = RooDataSet("data0_2012_wide_","data0_2012_wide_",RooArgSet(mKp1_MC,mKp2_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
	data0_2011_narrow_ = RooDataSet("data0_2011_narrow_","data0_2011_narrow_",RooArgSet(mKp1_narrow_MC,mKp2_narrow_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
	data0_2012_narrow_ = RooDataSet("data0_2012_narrow_","data0_2012_narrow_",RooArgSet(mKp1_narrow_MC,mKp2_narrow_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
	...
	for i in tree_2011_wide:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))):
			...
			data0_2011_wide_.add(RooArgSet(mKp1_MC,mKp2_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
			ev_counter += 1
	year_MC.setVal(1)
	ev_counter = 0
	for i in tree_2012_wide:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			MCweight0.setVal(1./PDF_gen.evaluate())
			data0_2012_wide_.add(RooArgSet(mKp1_MC,mKp2_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
			ev_counter += 1
	wide_window_MC.setVal(0)
	year_MC.setVal(0)
	ev_counter = 0
	for i in tree_2011_narrow:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			MCweight0.setVal(1./PDF_gen.evaluate())
			data0_2011_narrow_.add(RooArgSet(mKp1_narrow_MC,mKp2_narrow_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
			ev_counter += 1
	year_MC.setVal(1)
	ev_counter = 0
	for i in tree_2012_narrow:
		if ((evnum_limit == 0) or ((evnum_limit != 0) and (ev_counter < evnum_limit))):
			...
			phi_MC.setVal(eval("i."+phi_name_MC))
			MCweight0.setVal(1./PDF_gen.evaluate())
			data0_2012_narrow_.add(RooArgSet(mKp1_narrow_MC,mKp2_narrow_MC,cos1_MC,cos2_MC,phi_MC,MCweight0))
			ev_counter += 1

	print 'Data loaded.'

	data_wide = RooDataSet("data_wide","data_wide",data0_2012_wide_,data0_2012_wide_.get(),"",MCweight0.GetName())
	data_narrow = RooDataSet("data_narrow","data_narrow",data0_2012_narrow_,data0_2012_narrow_.get(),"",MCweight0.GetName())

	return data_wide,data_narrow,PDF_gen


data_wide,data_narrow,PDF_gen = LoadDataNW()
wide_window_MC.setVal(1)
fr1 = mKp1_MC.frame()
fr2 = mKp2_MC.frame()
fr3 = cos1_MC.frame()
fr4 = cos2_MC.frame()
fr5 = phi_MC.frame()
data_wide.plotOn(fr1,RooFit.Binning(12))
data_wide.plotOn(fr2,RooFit.Binning(12))
data_wide.plotOn(fr3,RooFit.Binning(12))
data_wide.plotOn(fr4,RooFit.Binning(12))
data_wide.plotOn(fr5,RooFit.Binning(12))
cn = TCanvas("cn","cn",800,1200)
cn.Divide(2,3)
cn.cd(1)
fr1.Draw()
cn.cd(2)
