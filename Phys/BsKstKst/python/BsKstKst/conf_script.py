...
if TUP:
    tuplesv=["T DATAFILE='"+os.environ["BSKSTKSTROOT"]+"/NTuples/Bs2KstKst"+string+".root' TYP='ROOT' OPT='NEW'"]
...
selection = "$B2VV_V2HHSELECTIONS/DVSelLooseBs2Kst0Kst0.py"  ### Now also in Installarea

datacards =  os.environ["BSKSTKSTROOT"] + "/datacards/BsKstKst.py"

from BenderBsKstKst import *
...
def configure():
    ...
    DaVinci().HistogramFile =os.environ["BSKSTKSTROOT"] + "/NTuples/MCtestrbH"+string+".root"
    
    #importOptions("$DAVINCIROOT/options/PreloadUnits.opts")
    importOptions("$STDOPTS/PreloadUnits.opts")
    
    importOptions( selection )
    DaVinci().UserAlgorithms += [GaudiSequencer("Bs2KstKstDet")]  ### Check you use the right name (or automate it)!
    ...
    if DST:
        ...
        dstW.selectionLocations += ['/Event/Phys/Bs2KstKst']
        dstW.COUNTER = {}
        dstW.COUNTER["EVT"] = 0
        for sel in dstW.selectionLocations: dstW.COUNTER[sel] = 0
  
    bs= BsKst0Kst0("BsKst0Kst0")      
    bs.COUNTER = {}
    bs.decayname = "[B_s0 -> K*(892)0 K*(892)~0]cc "
    bs.COUNTER["reco"]=0 
    bs.COUNTER["EVT"] = 0
    bs.COUNTER["Sel"] = 0
    bs.LookIn ="Bs2KstKstDet"#  "Bs2KstKst"
