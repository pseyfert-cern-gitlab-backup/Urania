...
cuts = "GLK2012sb.>0.14 && (Kplus_PIDp-Kplus_PIDK)<0. && (Kminus_PIDp-Kminus_PIDK)<0. && Piplus_PIDK<-5. && Piminus_PIDK<-5. && Kplus_PIDK>5. && Kminus_PIDK>5. && Piplus_PIDp<10. && Piminus_PIDp<10. && abs(Kstb_M-895.94)<150. && abs(Kst_M-895.94)<150. "
...
if SWEIGHTS:
    ...
    t_Bs = t.CopyTree("abs(B_s0_M-5371.7764871746)<30")
    t_Bd = t.CopyTree("abs(B_s0_M-5284.6464871746)<30")
...
Ang_Bs.modParam("dsm",pi,pi,kFALSE)
...
Ang_Bs.modParam("normP",1.,0,kTRUE)
Ang_Bs.modParam("normS",1,0,kTRUE)
Ang_Bs.modParam("normSS",1,0,kTRUE)
Ang_Bs.modParam("norm_int",0.162916712884,0.,kTRUE)
Ang_Bs.modParam("norm_int_ss",1.,0.,kTRUE)
Ang_Bs.modParam("norm_int_sv_ss",1.,0.,kTRUE)


# mhi = 896.+150
# mlo = 896.-150

# limhi = TComplex(mhi - Ang_Bs.m_kst.getVal(),Ang_Bs.g_kst.getVal()/2.)
# limlo = TComplex(mlo - Ang_Bs.m_kst.getVal(),Ang_Bs.g_kst.getVal()/2.)

# eta = limhi.Theta()-limlo.Theta()
# Z = sqrt((pow(mhi - Ang_Bs.m_kst.getVal(),2) + pow(Ang_Bs.g_kst.getVal()/2.,2))/(pow(mlo - Ang_Bs.m_kst.getVal(),2) + pow(Ang_Bs.g_kst.getVal()/2.,2)))
# DL = atan(2*(mhi-Ang_Bs.m_kst.getVal())/Ang_Bs.g_kst.getVal()) - atan(2*(mlo-Ang_Bs.m_kst.getVal())/Ang_Bs.g_kst.getVal())
# Dm = mhi-mlo

# Csp = sqrt(Ang_Bs.g_kst.getVal()/(2*DL*Dm))*sqrt(pow(log(Z),2) + pow(eta,2))
...
Ang_Bs.modParam("norm_int",Csp_mod,0.,kTRUE)
Ang_Bs.modParam("Csp_arg",Csp_arg,0.,kTRUE)

# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Bs fL:  0.286546739724  +  0.0683987627738   -0.0840205320961
# Bs fpa:  0.286643038298  +  0.0423781207782   -0.0363468301896
# Bs Asp2:  0.235907030878  +  0.0314090403826   -0.0311107996947
# Bs Asm2:  0.341014400973  +  0.031250990042   -0.0315742496457
# Bs dpa:  -1.30856521296  +  0.241225003326   -0.193876628238
# Bs dsp:  3.75740318454  +  0.466611958973   -0.34027163373
# Bs dsm:  2.39528656168  +  0.235779252532   -0.240444979863
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Ang_Bd.OwnPhys(), Ang_Bd.CopyNorm(Ang_Bs)
...
Ang_Bd.modParam("dsp",0.,pi,kFALSE)
...
fitres_Bs = Ang_Bs.sigmodel.fitTo(dataset_Bs,RooFit.Save(),RooFit.NumCPU(4),RooFit.SumW2Error(1),RooFit.Minos(1))
