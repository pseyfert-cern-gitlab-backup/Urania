...
from PDF_builder_self_onlyangles_v2 import *
...
file_data = TFile("${WHOME}/NTuples_Bs2KstKst_strip17/public/Bs2KstKst_Real2011_strip17_cuts_v11r1_GL2012.root")
tree2 = file_data.Get("T")

cuts = "GLK2012sb.>0.24"
cuts += "& ((P11_richPID_p-P11_richPID_k)<15) & ((P21_richPID_p-P21_richPID_k)<15) "
cuts += "& (P21_richPID_k>4) & (P11_richPID_k>4) &(P22_richPID_k<-2) & (P12_richPID_k<-2) "
cuts += "& ! ( (abs(M_prpiKpi-5620)<50 && P11_richPID_p-P11_richPID_k>0 ) || (abs(M_Kpiprpi-5620)<50 && P21_richPID_p-P21_richPID_k>0 )) "
cuts += "& abs(P1_mass-896)<30"
cuts += "& L0Decision & Hlt1Decision & Hlt2Decision"

coef_name = "acceptance_2011_30.dct"

# cuts = "GLK2012sb.>0.14 "
# cuts += "& ((P11_richPID_p-P11_richPID_k)<0) & ((P21_richPID_p-P21_richPID_k)<0) "
# cuts += "& (P21_richPID_k>5) & (P11_richPID_k>5) &(P22_richPID_k<-5) & (P12_richPID_k<-5) "
# cuts += "& ((P12_richPID_p-P12_richPID_k)<10) & ((P22_richPID_p-P22_richPID_k)<10) "
# cuts += "& abs(P1_mass-896)<50"
# cuts += "& P2_mass<1700"

# coef_name = "acceptance_v6.dct"
...
# Mass_bins = [744.,1700]
# Mass_bins = [744.,856.,936.,1000.,1200,1450,1700]
Mass_bins = [744.,856.,936.,1050.,1200,1350,1500,1700]


import cPickle, os
acc_coef_file = open(os.environ["PWD"] + "/"+coef_name,'r')
...
for i in range(len(Mass_bins)-1):
    ...
    t = t3.CopyTree("P2_mass>"+str(Mass_bins[i])+" && "+"P2_mass<"+str(Mass_bins[i+1]))
    ...
    if FIT_MASS:
        ...
        Bmass.SetNameTitle("B_mass","B_mass")
        ...
        nentries = n_s.getVal()
        nentries_err = n_s.getError()


    else:

        t_Bs = t.CopyTree("abs(B_mass-5372.33)<30")
        t_Bd = t.CopyTree("abs(B_mass-5285.20)<30")

        nentries = t_Bs.GetEntries()
        nentries_err = sqrt(t_Bs.GetEntries())

        dataset_Bs = RooDataSet("data_Bs","data_Bs",t_Bs,RooArgSet(CTh1,CTh2,Phi))
        dataset_Bd = RooDataSet("data_Bd","data_Bd",t_Bd,RooArgSet(CTh1,CTh2,Phi))

    Ang_Bs = AngularPDF("Bs",acceptance = True)
    Ang_Bs.OwnPhys()


    acc_coefs = acc_coef_list[i]["coefs"]
    
    Ang_Bs.modParam("c1_th1",acc_coefs["c1_th1"],0.,kTRUE)
    Ang_Bs.modParam("c2_th1",acc_coefs["c2_th1"],0.,kTRUE)
    Ang_Bs.modParam("c3_th1",acc_coefs["c3_th1"],0.,kTRUE)
    Ang_Bs.modParam("c4_th1",acc_coefs["c4_th1"],0.,kTRUE)
    Ang_Bs.modParam("y_th1",acc_coefs["cy_th1"],0.,kTRUE)
    
    Ang_Bs.modParam("c1_th2",acc_coefs["c1_th2"],0.,kTRUE)
    Ang_Bs.modParam("c2_th2",acc_coefs["c2_th2"],0.,kTRUE)
    Ang_Bs.modParam("c3_th2",acc_coefs["c3_th2"],0.,kTRUE)
    Ang_Bs.modParam("c4_th2",acc_coefs["c4_th2"],0.,kTRUE)
    Ang_Bs.modParam("y_th2",acc_coefs["cy_th2"],0.,kTRUE)
...
#     swave_err_ = sqrt((nentries*Ang_Bs.As2.getError())**2 + (nentries_err*Ang_Bs.As2.getVal())**2)
    swave_err_ = sqrt((nentries*Ang_Bs.As2.getPropagatedError(fitres_Bs))**2 + (nentries_err*Ang_Bs.As2.getVal())**2)
...
#     pwave_err_ = sqrt((nentries*Ap2.getPropagatedError(fitres_Bs))**2 + (nentries_err*Ap2.getVal())**2)
    pwave_ = nentries*Ang_Bs.Ap2.getVal()
    pwave_err_ = sqrt((nentries*Ang_Bs.Ap2.getError())**2 + (nentries_err*Ang_Bs.Ap2.getVal())**2)
    ...
    result["params"]["fpa_Bs"] = Ang_Bs.fpa.getVal()
    result["params"]["fpa_Bs_errhi"] = Ang_Bs.fpa.getPropagatedError(fitres_Bs)
    result["params"]["fpa_Bs_errlo"] = -1.*Ang_Bs.fpa.getPropagatedError(fitres_Bs)
    result["params"]["fpa_Bs_errsim"] = Ang_Bs.fpa.getPropagatedError(fitres_Bs)
...
foutl = TFile("results_2011.root","recreate")
...
for bin in results:
    ...
    if "mass_plot" in bin.keys(): plot["mass_plot"] = bin["mass_plot"]
...
import cPickle, os
cPickle.dump(fit_results,file(os.environ["PWD"] + "/results_2011.dct",'w'))


for plot in plots:
    name = str(int(plot["mass_mean"]-2*plot["mass_err"]))+"_"+str(int(plot["mass_mean"]+2*plot["mass_err"]))
    if "mass_plot" in plot.keys():
