...
acc_coef_file_50 = open(os.environ["PWD"] + "/acceptance_50_MeV.dct",'r')
acc_coef_list_50 = cPickle.load(acc_coef_file_50)

results = []

#BREAK

for i in range(len(Mass_bins)-2):
    ...
    acc_coefs_c1 = acc_coef_list_50[0]["coefs"]
    acc_coefs_c2 = acc_coef_list[i]["coefs"]
    
    Ang_Bs.modParam("c1_th1",acc_coefs_c1["c1_th"],0.,kTRUE)
    Ang_Bs.modParam("c2_th1",acc_coefs_c1["c2_th"],0.,kTRUE)
    Ang_Bs.modParam("c3_th1",acc_coefs_c1["c3_th"],0.,kTRUE)
    Ang_Bs.modParam("c4_th1",acc_coefs_c1["c4_th"],0.,kTRUE)
    Ang_Bs.modParam("y_th1",acc_coefs_c1["cy_th"],0.,kTRUE)
    
    Ang_Bs.modParam("c1_th2",acc_coefs_c2["c1_th"],0.,kTRUE)
    Ang_Bs.modParam("c2_th2",acc_coefs_c2["c2_th"],0.,kTRUE)
    Ang_Bs.modParam("c3_th2",acc_coefs_c2["c3_th"],0.,kTRUE)
    Ang_Bs.modParam("c4_th2",acc_coefs_c2["c4_th"],0.,kTRUE)
    Ang_Bs.modParam("y_th2",acc_coefs_c2["cy_th"],0.,kTRUE)
...
#     pwave_err_ = sqrt((nentries*Ap2.getPropagatedError(fitres_Bs))**2 + (nentries_err*Ap2.getVal())**2)
    ...
    params_set = Ang_Bs.sigmodel.getParameters(dataset_all)
...
foutl = TFile("results_Bs_acc_anal.root","recreate")
...
import cPickle, os
cPickle.dump(fit_results,file(os.environ["PWD"] + "/results_Bs_acc_anal.dct",'w'))
