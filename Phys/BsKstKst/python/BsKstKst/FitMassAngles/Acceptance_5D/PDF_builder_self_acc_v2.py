...
gROOT.ProcessLine(".x ${BSKSTKSTROOT}/src/TimeIntegratedAcc5D/AngMassTerm.cxx+")
...
class AngularPDF:
    # def __init__(self,name,acc_filename,acc_name):
    ...
    def make(self):
        ...
        for index in self.indices:
            ...
            term_index = AngMassTerm("KFM"+str(index)+"_"+name,"KFM"+str(index)+"_"+name,index,CTh1,CTh2,Phi,m1,m2,
                                     ...
                                     self.Gs,self.DGs,self.norm,self.normP,self.normS,self.normSS,self.acc_tree,self.acc_name)
                               
                               
            self.products.append(term_index)

            self.dummyParamList.add(self.een)
            self.finalFuncList.add(term_index)
