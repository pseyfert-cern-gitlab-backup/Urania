...
def makeHist(**kwargs):
    ...
    recognisedKeyList = ['xKey', 'yKey', 'chain', 'name', 'scaleX', 'scaleY', 'yKey',
                         ...
                         'zeroYaxis', 'normalise', 'decPlaces', 'lineColour']
    ...
    if dim == 1:
        drawString = '%s*%f>>%s(%i, %f, %f)' %(xKey, scaleX, histName, numBinsX, xMin, xMax)
                                            
    elif dim == 2:
        # NOTE!!! The x and y keys are switched in the drawstring because ROOT is stupid
        # and treats the variables in this order.
        drawString = '%s*%f:%s*%f>>%s(%i, %f, %f, %i, %f, %f)' %(yKey, scaleY, xKey, scaleX, histName,
                                                                 numBinsX, xMin, xMax,
                                                                 numBinsY, yMin, yMax)
...
def plotHist(**kwargs):
    ...
    recognisedKeyList = ['hist', 'cx', 'cy', 'cName', 'setLog', 'drawOption', 'gfxType',
                         'plotNameOpts', 'setLegend', 'nameOverride']
    for theKey in validKeys:
        if theKey not in recognisedKeyList:
