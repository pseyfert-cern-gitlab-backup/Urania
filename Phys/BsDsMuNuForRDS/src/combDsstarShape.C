...
void AddData(RooWorkspace*, float);
...
void AddModel(RooWorkspace* ws, float nentries, TString resonance){
  ...
  RooRealVar sigma("sigma", "Width of signal", 15.0, 3.0, 40.0);
  RooRealVar alpha1("alpha1", "alpha1", 0.8, 0., 3.0);
  RooRealVar alpha2("alpha2", "alpha2", -0.8, -3.0, 0.);
  RooRealVar n1 = RooRealVar("n1", "n1", 10., 0.2, 50.);
  RooRealVar n2 = RooRealVar("n2", "n2", 10., 0.2, 50.);
  ...
  RooRealVar c = RooRealVar("c", "c", -0.0001, -1., 1.);
  ...
  // They will be fitted.
  RooRealVar SigYield("SigYield","fitted yield for Signal", nentries*0.45, 0., nentries) ; 
  RooRealVar BkgYield("BkgYield","fitted yield for Background", nentries*0.55, 0., nentries) ; 
  ...
  RooAddPdf model("model","signal+background models", RooArgList(*BkgModel), RooArgList(BkgYield));
...
//____________________________________
void AddData(RooWorkspace* ws, float nentries){
  ...
  /*
    ...
    std::cout << "My sample is " << sample << std::endl;
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("/eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_BdDstDs_iso.root");
    std::cout << fChain->GetEntries() << std::endl;
  } else if (sample == "BuDD"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_BuD0Ds_iso.root");
  } else if (sample == "BsDD"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_BsDsDs_iso.root");
  } else if (sample == "LbDD"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_LbLcDs_iso.root");
  } else if (sample == "sigmu"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/MC09/MC09_DsMuNu.root");
  } else if (sample == "sigtau"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/MC09/MC09_DsTauNu.root");
  }
  */
...
/*
  RooRealVar *fitMass;
  if(resonance == "Dsst"){
    fitMass = new RooRealVar("resMass", "resMass", 1900., 2300.);
  }
  if(resonance == "Ds0"){
...
*/  
  ...
  /*
  RooArgSet vars(*fitMass, *triggerTOS, *kplp, *kmip, *pip, *kplpt, *kmipt, *pipt, *dspt);
  ...
  vars.add(RooArgSet(*bsorigx,*bsorigy,*bsorigz,*bsendx,*bsendy,*bsendz));
  ...
  vars.add(RooArgSet(*sq,*McorrDsst,*McorrDs,*bSinFlightAng,*bFlightLen));
  ...
  */
  ...
  TH1F *invMassPlot = new TH1F("invMassPlot", "invMassPlot", 100, 1950, 2250);
  TH1F *combMassPlot = new TH1F("combMassPlot", "combMassPlot", 100, 1950, 2250);
  Double_t regularMass, combMass;
  // Fill for Dsst
  //if(resonance == "Dsst"){
    for(int i = 0; i!=nentries-310; ++i){
      if (i%100000 == 0) std::cout << "--- ... Processing event: " << i << std::endl;
      fChain->GetEntry(i);
      // get the usual Ds* 
      bool cutsData = Bs_0_Hlt2XcMuXForTauB2XcMuDecision_TOS==1 && Bs_0_MCORR<8000 && Bs_0_MCORR>3000 && mu_PIDmu>0.0 && Ds_MM>1940 && Ds_MM<2000 && mu_iso_MinBDT_Long>-0.5 && Ds_0_40_nc_mult>0 && sqrt(pow(Ds_0_40_nc_maxPt_PX,2)+pow(Ds_0_40_nc_maxPt_PY,2))>0 && Ds_0_40_nc_CL > 0.6;
      if (cutsData) {
        if(sqrt( pow((Ds_0_40_nc_maxPt_PE+Ds_PE),2) - ( pow(Ds_0_40_nc_maxPt_PX+Ds_PX,2)+pow(Ds_0_40_nc_maxPt_PY+Ds_PY,2)+pow(Ds_0_40_nc_maxPt_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30 < 1950. || sqrt( pow((Ds_0_40_nc_maxPt_PE+Ds_PE),2) - ( pow(Ds_0_40_nc_maxPt_PX+Ds_PX,2)+pow(Ds_0_40_nc_maxPt_PY+Ds_PY,2)+pow(Ds_0_40_nc_maxPt_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30 > 2250.) continue;
        regularMass = sqrt( pow((Ds_0_40_nc_maxPt_PE+Ds_PE),2) - ( pow(Ds_0_40_nc_maxPt_PX+Ds_PX,2)+pow(Ds_0_40_nc_maxPt_PY+Ds_PY,2)+pow(Ds_0_40_nc_maxPt_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30;
        invMassPlot->Fill(regularMass);
      //}

      // look for a Ds with similar direction and get the corresponding photon
      Double_t eta = Ds_eta;
      Double_t phi = Ds_phi; 
      Double_t gamma_PX = Ds_0_40_nc_maxPt_PX;
      Double_t gamma_PY = Ds_0_40_nc_maxPt_PY;
      Double_t gamma_PZ = Ds_0_40_nc_maxPt_PZ;
      Double_t gamma_PE = Ds_0_40_nc_maxPt_PE;
      Double_t gamma_CL = Ds_0_40_nc_CL;
      Double_t gamma_mult = Ds_0_40_nc_mult;
      for (int j = i+2; j!=i+300; ++j) { 
        fChain->GetEntry(j);
        // apply the cuts to add only good data
        // remove mu_pT> 1000
        bool cuts;
        double deltaPhi = fabs(phi-Ds_phi);
        double deltaEta = fabs(eta-Ds_eta);
        if ( deltaPhi > M_PI ) deltaPhi = 2 * M_PI - deltaPhi;
        double deltaR = sqrt( deltaPhi * deltaPhi + deltaEta * deltaEta );

        if (deltaR < 0.01 ){
          std::cout << "--- ... Processing event: " << i << ", " << j << std::endl;
          //if (sample == "data" || sample == "SS" || sample == "sigmu" || sample == "sigtau"){
          cuts = Bs_0_Hlt2XcMuXForTauB2XcMuDecision_TOS==1 && Bs_0_MCORR<8000 && Bs_0_MCORR>3000 && mu_PIDmu>0.0 && Ds_MM>1940 && Ds_MM<2000 && mu_iso_MinBDT_Long>-0.5 && gamma_mult>0 && sqrt(pow(gamma_PX,2)+pow(gamma_PY,2))>0 && gamma_CL > 0.6;
          //}
          //else cuts = Bs_0_MCORR<8000 && Bs_0_MCORR>3000 && mu_PIDmu>0.0 && Ds_MM>1940 && Ds_MM<2000 && mu_iso_MinBDT_Long>-0.5 && gamma_mult>0 && sqrt(pow(gamma_PX,2)+pow(gamma_PY,2))>0 && gamma_CL > 0.6; 
          if (cuts) {
            if(sqrt( pow((gamma_PE+Ds_PE),2) - ( pow(gamma_PX+Ds_PX,2)+pow(gamma_PY+Ds_PY,2)+pow(gamma_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30 < 1950. || sqrt( pow((gamma_PE+Ds_PE),2) - ( pow(gamma_PX+Ds_PX,2)+pow(gamma_PY+Ds_PY,2)+pow(gamma_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30 > 2250.) continue;
            combMass = sqrt( pow((gamma_PE+Ds_PE),2) - ( pow(gamma_PX+Ds_PX,2)+pow(gamma_PY+Ds_PY,2)+pow(gamma_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30;
            combMassPlot->Fill(combMass);
            /*
            *fitMass = sqrt( pow((gamma_PE+Ds_PE),2) - ( pow(gamma_PX+Ds_PX,2)+pow(gamma_PY+Ds_PY,2)+pow(gamma_PZ+Ds_PZ,2) ) )-Ds_MM+1968.30;
            ...
            *bsendz = Bs_0_ENDVERTEX_Z;
            // stuff from Marcello
            *MassDsG = sqrt(((Ds_PE+gamma_PE)*(Ds_PE+gamma_PE)-
                           (Ds_PX+gamma_PX)*(Ds_PX+gamma_PX)-
                           (Ds_PY+gamma_PY)*(Ds_PY+gamma_PY)-
                           (Ds_PZ+gamma_PZ)*(Ds_PZ+gamma_PZ))/1000000)-
                           Ds_MM/1000+DsMassPDG;
            TLorentzVector Bs4V(Bs_0_PX,Bs_0_PY,Bs_0_PZ,Bs_0_PE);
            TLorentzVector Mu4V(mu_PX,mu_PY,mu_PZ,mu_PE);
            TLorentzVector Dsst4V(Ds_PX+gamma_PX,Ds_PY+gamma_PY,Ds_PZ+gamma_PZ,Ds_PE+gamma_PE);
            ...
            *McorrDsst = (sqrt( Bs_0_MM*Bs_0_MM+pTcorrss*pTcorrss ) + pTcorrss);
            ...
            upvars.add(RooArgSet(*sq,*McorrDsst,*McorrDs,*bSinFlightAng,*bFlightLen));
            data->add(upvars);     
            */
            break;
          }
  ...
  // do the super imposed plot
  TCanvas *cPlot = new TCanvas("superimposed","superimposed",800,600); 
  Double_t norm1 = invMassPlot->Integral(80,100);
  Double_t norm2 = combMassPlot->Integral(80,100);
  Double_t norm = norm1/norm2;
  std::cout << norm1 << ", " << norm2 << ", " << norm << std::endl;
  invMassPlot->Draw();
  combMassPlot->SetFillColor(kRed);
  //combMassPlot->SetLineColor(kRed);
  combMassPlot->Scale(norm);
  combMassPlot->Draw("same");
  cPlot->SaveAs("ShapeComb.pdf");
...
*/
  ...
  TFile *file = new TFile("DsMass_WithSWeighted_"+resonance+"_"+sample+"_CB_CombDsstar.root","RECREATE");
  ...
  Double_t bsorigx, bsorigy, bsorigz, bsendx, bsendy, bsendz;
  Double_t sigw, bkgw;
  ...
  Double_t sq; 
  Double_t McorrDsst, McorrDs, bSinFlightAng, bFlightLen;
  ...
  finalTree->Branch("McorrDs",&McorrDs,"McorrDs/D");
  ...
  for (int i = 0; i!=nen; ++i){
    ...
    McorrDs = Dset->getRealValue("McorrDs");
...
void MakePlots(RooWorkspace* ws, TString resonance){
...
//  RooAbsPdf* SigModel = ws->pdf("sig");
  ...
  RooPlot *frame_1;
  if(resonance=="Dsst"){
    frame_1 = invMass->frame(1900.,2300.,100); 
  }
  if(resonance=="Ds0"){
...
//  model->plotOn(frame_1,Components(*SigModel),LineStyle(kDashed),LineColor(kRed)) ;   
  ...
  RooPlot* frame2_1 = mcorr->frame(3000.,8000.,80);
  ...
  RooPlot* frame3_1 = mcorr->frame(3000.,8000.,80);
  ...
  frame3_1->Draw() ;
  cdata1->SaveAs("SPlot_MCORR_"+resonance+".pdf");
  ...
  RooPlot *frame_2;
  if(resonance=="Dsst"){
    frame_2 = invMass->frame(1900.,2300.,80);
  }
  if(resonance=="Ds0"){
...
//  model->plotOn(frame_2,Components(*SigModel),LineStyle(kDashed),LineColor(kRed)) ;
  ...
  RooPlot* frame2_2 = q2->frame(-5000000.,14000000.,80);
  ...
  RooPlot *frame_3;
  if(resonance=="Dsst"){
    frame_3 = invMass->frame(1900.,2300.,80);
  }
  if(resonance=="Ds0"){
...
//  model->plotOn(frame_3,Components(*SigModel),LineStyle(kDashed),LineColor(kRed)) ;
  ...
  frame3_3->Draw() ;
  cdata3->SaveAs("SPlot_Emu_"+resonance+".pdf");
...
float getEntries(){
  ...
  /*
    ...
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_BdDstDs_iso.root");
  } else if (sample == "BuDD"){
    fChain = new TChain("B2DsMuNuTuple/DecayTree");
    fChain->Add("root://eoslhcb.cern.ch//eos/lhcb/user/r/rvazquez/RDS/WithDsVetoes/TupleRDS_BuD0Ds_iso.root");
  } else if (sample == "BsDD"){
