...
#include "TupleToolMuonIsolation.h"
#include "TMath.h"
#include "TFile.h"
#include "TString.h"
#include <TROOT.h>
...
//-----------------------------------------------------------------------------
// Implementation file for class : TupleToolMuonIsolation
//
// 2011-08-08 : Fatima Soomro 
//-----------------------------------------------------------------------------

// Declaration of the Algorithm Factory
DECLARE_TOOL_FACTORY( TupleToolMuonIsolation )
...
//=============================================================================
TupleToolMuonIsolation::TupleToolMuonIsolation( const std::string& type,
  ...
  , m_dist(0)
{
  ...
  declareProperty("isoBDT_xmlFilePath",m_isoBDT_xmlFilePath="/afs/cern.ch/user/r/rvazquez/cmtuser/DaVinci_v38r0/Phys/BsDsMuNuForRDS/BDTfiles/");
...
// Destructor
//=============================================================================
TupleToolMuonIsolation::~TupleToolMuonIsolation() {} 

//=============================================================================
// Initialization
//=============================================================================
StatusCode TupleToolMuonIsolation::initialize() {
  ...
  isobdt_reader[0]->BookMVA(isobdt_methodName,m_isoBDT1_xmlFile);
  isobdt_reader[1]->BookMVA(isobdt_methodName,m_isoBDT2_xmlFile);
  isobdt_reader[2]->BookMVA(isobdt_methodName,m_isoBDT3_xmlFile);
  isobdt_reader[3]->BookMVA(isobdt_methodName,m_isoBDT4_xmlFile); 
...
// Fill the tuple
//=============================================================================
StatusCode TupleToolMuonIsolation::fill( const LHCb::Particle *, const LHCb::Particle *part,
  ...
  for( SmartRefVector< LHCb::Particle >::const_iterator idau = Bdaughters.begin(); 
    ...
    if (NULL == (*idau)->proto() ){
      ...
      if( abs ((*idau)->particleID().pid() ) == 411) { 
        JpsiInDau = true; 
	      myJpsi = (*idau);
	    }
    }
    else if ( abs((*idau)->particleID().pid()) == dauIds[0] 
           || abs((*idau)->particleID().pid()) == dauIds[1] ) {
      debug() << "daughter no " << countDaughters << " is " << abs((*idau)->particleID().pid()) << endmsg;  
      countDaughters++;  
  ...
  // ------------------------------------------//
  // === Fill these only for muon daughters ===//
  // ------------------------------------------//
  if(checkDau && countDaughters == 1 && abs(part->particleID().pid()) >500 ){
...
//  Fill Eta
//=============================================================================
StatusCode TupleToolMuonIsolation::fillEta(const LHCb::Particle *part,
...
//=============================================================================
StatusCode TupleToolMuonIsolation::fillTrackHitInfo(const LHCb::Particle *part,
						 const std::string prefix,
						 Tuples::Tuple& tuple ){
  if( !part->isBasicParticle() ) return StatusCode::SUCCESS;
  
  if( abs(part->particleID().pid()) == 13 || abs(part->particleID().pid()) == 11 ) {
    debug() << "FillTrackHitInfo::particle is " << part->particleID().pid() << endmsg; 
  }
...
//=============================================================================
StatusCode TupleToolMuonIsolation::fillLoKiTau(const LHCb::Particle *part,
					       const std::string prefix,
					       Tuples::Tuple& tuple ){
...
//=============================================================================
StatusCode TupleToolMuonIsolation::fillBDTS(const LHCb::Particle *part,
...
StatusCode TupleToolMuonIsolation::OtherB(const LHCb::Particle *part, 
...
//=============================================================================
// Jpsi vertex distance chi2 wrt the PV which minimizes the B IPchi2
//=============================================================================
StatusCode TupleToolMuonIsolation::fillFakeVtx(const LHCb::Particle *part,
					       const LHCb::Particle *myJpsi,
					       const std::string ,
					       Tuples::Tuple& tuple ){
...
//
StatusCode TupleToolMuonIsolation::fillIsolation(const LHCb::Particle *part,
						 const std::string prefix,
						 Tuples::Tuple& tuple ){
  m_count_mum = 0;
  m_count_mup = 0;
  ...
  debug() << "fillIsolation::After checking daughters" << endmsg;
...
//=============================================================================
std::vector<int> TupleToolMuonIsolation::getIso(const LHCb::Particle* B, int isotype, bool cutOnTracks){
  ...
  for ( ip_part = parts.begin(); ip_part != parts.end(); ++ip_part) {    
    if ( NULL==(*ip_part)->proto() ) continue;
  ...
  for ( int i2 = 0; i2 < 1; ++i2) {
    ...
    for ( ip = allparts->begin(); ip != allparts->end() ; ++ip) {
      ...
      if (isotype >100 && angle < iso_angle && fc < iso_fc && doca < iso_doca && ips > iso_ips 
    	 && svdis > iso_svdis && svdis < iso_svdis_h && pvdis > iso_pvdis && pvdis < iso_pvdis_h 
	     && mytrack_clone <  iso_track_clonedist && mytrack_gp < iso_track_ghost && mytrack_trchi2 < iso_track_trchi2 ) {
        iso5[i2] += 1;
      } 

      if(false){
        debug()<< " isotype was "<<isotype<<"   "<< angle <<" - "<< iso_angle<<" "<<
	      fc <<" - "<< iso_fc<< " \n "<<
	      doca <<" -  "<< iso_doca<<" \n "<<
	      ips <<" - "<< iso_ips<<" \n"<<
	      svdis <<" - "<< iso_svdis<<" \n"<<
...
//=============================================================================
void  TupleToolMuonIsolation::IsHltGood(Gaudi::XYZPoint o,Gaudi::XYZVector p,
...
double TupleToolMuonIsolation::pointer (Gaudi::XYZVector vertex,
                                        Gaudi::XYZVector p, 
...
StatusCode TupleToolMuonIsolation::fillTrackIsoBDT(const LHCb::Particle *B,
	                                    					   const std::string prefix,
						                                       Tuples::Tuple& tuple ){
  ...
  double mu_isobdt_1st_vec[4];
  double mu_isobdt_2nd_vec[4];
  double mu_isobdt_3rd_vec[4];
  ...
  else DsDaug = parts[0]->daughtersVector(); 
  if ( NULL!=(parts[1]->proto()) ) {
    mu_trk=parts[1]->proto()->track();
  }
  else DsDaug = parts[1]->daughtersVector();
  if (DsDaug.size() != 3){
    error() << "Number of Ds daughters != 3: " << DsDaug.size() << endmsg;
  }
  ...
  for ( ip_part = parts.begin(); ip_part != parts.end(); ++ip_part) {  
    ...
    for ( part_it = particles->begin() ; part_it != part_it_end ; ++part_it ) {
      ...
      //if the track is equal to the muon, skip the track
      if (track==mu_trk) continue;
      counterLong++;
      // if the track is equal to the daughters of the Ds, skip the track
      bool isInDecay = false;
      LHCb::Particle::ConstVector::const_iterator itDaug;
      for( itDaug = DsDaug.begin(); itDaug != DsDaug.end(); ++itDaug) {
        const LHCb::Particle* partDaug = *itDaug;
        if (track==partDaug->proto()->track()){
          isInDecay = true;
        }
      }
      if(isInDecay){
        debug() << "Removing track from isolation computation" << endmsg;
        continue;
      }
      counterNoDs++;
      ...
      if(PV) trk_fc = calcFC(trackMomentum, partMomentum, vertex_mu_track, PV);
      ...
      if (msgLevel(MSG::DEBUG)){
	      debug()<<"Traccia num :"<<counter++<<endmsg;
	      debug()<<"log(trk_ips) = "<<log_trk_ips<<endmsg;
       	debug()<<"trk_pvdis = "<<trk_pvdis<<endmsg;
	      debug()<<"trk_svdis = "<<trk_svdis<<endmsg;
	      debug()<<"log(trk_doca) = "<<log(trk_doca)<<endmsg;
	      debug()<<"trk_angle = "<<trk_angle<<endmsg;
	      debug()<<"trk_fc = "<<trk_fc<<endmsg;
	      debug()<<"trk_dphi = "<<trk_dphi<<endmsg;
	      debug()<<"trk_deta = "<<trk_deta<<endmsg;
	      debug()<<"trk_pt = "<<trk_pt<<endmsg;
	      debug()<<"trk_ch = "<<trk_ch<<endmsg;
	      debug()<<"trk_nnmu = "<<trk_nnmu<<endmsg;
	      debug()<<"trk_nnpi = "<<trk_nnpi<<endmsg;
	      debug()<<"trk_nnk = "<<trk_nnk<<endmsg;
      }
      //
    ...
    for (int i=0;i<4;++i){
      if (abs(part->particleID().pid())==13){
	      mu_isobdt_1st_vec[i]=isobdt_1st_vec[i];
	      mu_isobdt_2nd_vec[i]=isobdt_2nd_vec[i];
	      mu_isobdt_3rd_vec[i]=isobdt_3rd_vec[i];
      }
      else {
	      error()<<"ERROR: B dau is not a muon"<<endmsg;
      }
    }//end i<4 loop
    debug()<< "Number of NoDs = " << counterNoDs << endmsg;
  ...
  for (int i=0;i<4;++i){
    ...
    tuple->column((flag+"_1st").Data(),mu_isobdt_1st_vec[i]);
    tuple->column((flag+"_2nd").Data(),mu_isobdt_2nd_vec[i]);
    tuple->column((flag+"_3rd").Data(),mu_isobdt_3rd_vec[i]);
  }//end i<4 loop
...
StatusCode TupleToolMuonIsolation::fillPVList(Tuples::Tuple& tuple){
...
StatusCode TupleToolMuonIsolation::fillPVrefitList(const LHCb::Particle* part,Tuples::Tuple& tuple){
  //Get the list of primary vertices and store their vtx info in ntuple
  LHCb::RecVertex::Container* verts = NULL;
  if(exist<LHCb::RecVertex::Container>(m_PVInputLocation)) verts = get<LHCb::RecVertex::Container>(m_PVInputLocation);
  LHCb::RecVertex::Container::const_iterator iv;
...
StatusCode TupleToolMuonIsolation::fillTrkDocaPV(Tuples::Tuple& tuple){
...
// Fill DOCA:
//=============================================================================
StatusCode TupleToolMuonIsolation::fillDoca(const LHCb::Particle *part, 
					     const std::string prefix, 
					     Tuples::Tuple& tuple ){
  bool test = true;
  ...
  debug() << "Filled the doca variables" << endmsg;
  if(test) return StatusCode::SUCCESS;
  else return StatusCode::FAILURE;
...
StatusCode TupleToolMuonIsolation::fillCosNK(const LHCb::Particle *part, 
					      const std::string prefix, 
					      Tuples::Tuple& tuple ){
  ...
  if(idx != 1) return StatusCode::SUCCESS;
...
double TupleToolMuonIsolation::getphi(const LHCb::Particle* vdau1, const LHCb::Particle* vdau2){
  double dphi = vdau1->momentum().Phi() - vdau2->momentum().Phi();
  return dphi;
}

//=============================================================================
double TupleToolMuonIsolation::gettheta(const LHCb::Particle* vdau1, const LHCb::Particle* vdau2){
...
//=============================================================================
double TupleToolMuonIsolation::ratio( double p1, double p2){  
  return fabs(p1 -p2)*(1./fabs(p1+p2)); 
}

//=============================================================================
double TupleToolMuonIsolation::IsClose(const LHCb::Particle* p1,const LHCb::Particle* p2) {
...
//=============================================================================
void TupleToolMuonIsolation::closest_point(Gaudi::XYZPoint o,Gaudi::XYZVector p,
...
double TupleToolMuonIsolation::arcosine(Gaudi::XYZVector p1,Gaudi::XYZVector p2) {
...
//=============================================================================
void TupleToolMuonIsolation::InCone(Gaudi::XYZPoint o1, 
...
//=============================================================================
StatusCode TupleToolMuonIsolation::ConstrMass(const LHCb::Particle* B, 
                                              const std::string prefix, Tuples::Tuple& tuple ){
  
  const LoKi::Types::Fun constrainedMass = LoKi::Cuts::DTF_FUN (LoKi::Cuts::M, true, "J/psi(1S)" );
...
//=============================================================================
StatusCode TupleToolMuonIsolation::CDFIsolation_2(const LHCb::Particle* B,
...
//=============================================================================
StatusCode TupleToolMuonIsolation::CDFIsolation(const LHCb::Particle* B,
                                                const LHCb::Particle* vdau1,
                                                const LHCb::Particle* vdau2,
                                                const std::string prefix, 
                                                Tuples::Tuple& tuple){
...
StatusCode TupleToolMuonIsolation::CDFIsolation_Yuri(const LHCb::Particle* B,
						      const LHCb::Particle* vdau1,
						      const LHCb::Particle* vdau2,
						      const std::string prefix, 
						      Tuples::Tuple& tuple ){
...
StatusCode TupleToolMuonIsolation::ATLASIsolation(const LHCb::Particle* B,
						   const std::string prefix, 
						   Tuples::Tuple& tuple ){
  ...
  for(ipp= parts->begin();ipp!=parts->end();++ipp){
    ...
    if(proto) {
      ...
      if(atrack) {
	      ...
	      if (ips > 0. and doca < 0.2 and angle < 0.2618) n_incone+=1;
...
//==============================
const LHCb::MCParticle* TupleToolMuonIsolation::originof( const LHCb::MCParticle* product ) {
...
// Fill MC quantities
//=============================================================================
StatusCode TupleToolMuonIsolation::fillTrackInfo(Tuples::Tuple& tuple ){
  ...
  for(ipp= parts->begin();ipp!=parts->end();++ipp){
    ...
    if (m_isMC){
      if (!MCp) {
	      ...
	      else  m_endVertices = MCp->endVertices().size();
...
// Fill MC quantities
//=============================================================================
StatusCode TupleToolMuonIsolation::fillMCTruth(const LHCb::Particle *part, 
                                               const std::string prefix, 
                                               Tuples::Tuple& tuple ){
...
void  TupleToolMuonIsolation::dumpIsoVariables(const LHCb::Particle* B, Tuples::Tuple& tuple ){
  ...
  if(PV) PosPV = PV->position();
  if(SV) PosSV = SV->position();
  ...
  for (int i=0;i<1;++i){
	...
	cout<<"ATTENTION: (*ip) = "<<(*ip)<<" mucand_vec[0] = "<<mucand_vec[0]<<" mucand_vec[1] = "<<mucand_vec[1]<<" mucand_vec[0]->proto()->track() = "<<mucand_vec[0]->proto()->track()<<" mucand_vec[1]->proto()->track() = "<<mucand_vec[1]->proto()->track()<<" track = "<<track<<" ratio(pt,ptmu[0]) = "<<ratio(pt,ptmu[0])<<" ratio(pt,ptmu[1]) = "<<ratio(pt,ptmu[1])<<" Questa particella non e' uno dei due mu candidates"<<endl;
      ...
      */
      ...
      //if (ratio(pt, ptmu[0]) < 0.0001 || ratio(pt,ptmu[1]) <0.0001) {
      if (track==mutrack_vec[0]) {
	      if (!(ratio(pt, ptmu[0]) < 0.0001)) {
	        info()<<"ATTENTION!!: ratio(pt,ptmu[0]) = "<<ratio(pt,ptmu[0])<<endmsg;
	      }
      ...
      if(PV) IsHltGood(o, p, omu, pmu ,PosPV, hltgood, fc); 
...
// Finalize
//=============================================================================
StatusCode TupleToolMuonIsolation::finalize(){
  
  return TupleToolBase::finalize();
  
}

double TupleToolMuonIsolation::calcIPToAnyPV( const LHCb::Particle * cand )
{
  double imp=0.;
  double impchi2=0.;
  double ips=100000;
  ...
  if (exist<LHCb::RecVertex::Container>(m_PVInputLocation)){
    ...
    debug()<<"calcIPToAnyPV: verts = NULL. return -1"<<endmsg; 
    return -1; 
  } 
  for ( iv = verts->begin(); iv != verts->end(); ++iv) {
    counter++;
...
double TupleToolMuonIsolation::enclosedAngle(Gaudi::XYZVector p1,Gaudi::XYZVector p2) {
  double den      = p1.R()*p2.R();
...
void TupleToolMuonIsolation::getPerpFeet(Gaudi::XYZPoint track_pos,
...
double TupleToolMuonIsolation::calcFC( Gaudi::XYZVector track_mom,
...
double TupleToolMuonIsolation::calcVertexDist(Gaudi::XYZPoint muTrack, const LHCb::VertexBase* v){
  if (v==NULL) return 0 ;
...
//
StatusCode TupleToolMuonIsolation::fillMCDecayTree(Tuples::Tuple& tuple){
  if(! m_isMC){
...
//
void  TupleToolMuonIsolation::fillMCInfo(const LHCb::MCParticle* part,int mot_index,int depth,int flag){
  mc_partindex++;
...
StatusCode TupleToolMuonIsolation::fillBposmomCov(const LHCb::Particle* B, std::string prefix, Tuples::Tuple& tuple ){
...
StatusCode TupleToolMuonIsolation::fillVeloTracksInfo(Tuples::Tuple& tuple){
  bool test=true;
...
StatusCode TupleToolMuonIsolation::fillUpstreamTracksInfo(Tuples::Tuple& tuple){
  bool test=true;
  vector<double> par_pos_x, par_pos_y, par_pos_z;
  vector<double> par_pos_covxx,par_pos_covyy,par_pos_covxy;
...
void TupleToolMuonIsolation::trkPVassociator(std::vector<double>& trkPV_pz, std::vector<double>& trkPV_gho,std::vector<int>& trkPV_num,std::string trk_type){
  if (trkPV_pz.size()!=trkPV_gho.size() || trkPV_pz.size()!=trkPV_num.size()){
    error()<<"ERROR in TupleToolMuonIsolation::trkPVassociator: trkPV_pz, trkPV_gho, trkPV_num siz ="<<trkPV_pz.size()<<" "<<trkPV_gho.size()<<" " <<trkPV_num.size()<<endmsg; 
    return; 
  }
...
int TupleToolMuonIsolation::getAssociatedPV(const LHCb::Track* track, const std::vector<double>& trkPV_pz, const std::vector<double>& trkPV_gho, const std::vector<int>& trkPV_num){
  int output=0;
  if (trkPV_pz.size()!=trkPV_gho.size() || trkPV_pz.size()!=trkPV_num.size()) { 
    error()<<"ERROR in TupleToolMuonIsolation::trkPVassociator: trkPV_pz, trkPV_gho, trkPV_num siz ="<<trkPV_pz.size()<<" "<<trkPV_gho.size()<<" " <<trkPV_num.size()<<endmsg; 
    return output; 
