...
# - Fit for Re[a'] and Im[a'] from the CP & H observables in Bs->JpsiK*
...
filename = "Bs2JpsiKstarFit_wSU3.shelve"
...
def funcSU3_Re(RePar, ImPar, xiPar, deltaPar):
    # Express Re[a] and Im[a] in terms of Re[a']=RePar and Im[a']=ImPar
    a = xiPar * math.cos(deltaPar)
    b = xiPar * math.sin(deltaPar)
    return (a * RePar + b * ImPar)/(pow(a,2) + pow(b,2))

def funcSU3_Im(RePar, ImPar, xiPar, deltaPar):
    # Express Re[a] and Im[a] in terms of Re[a']=RePar and Im[a']=ImPar
    a = xiPar * math.cos(deltaPar)
    b = xiPar * math.sin(deltaPar)
    return (-b * RePar + a * ImPar)/(pow(a,2) + pow(b,2))
...
def theoHobs(RePar, ImPar, GPar, xiPar, deltaPar):
    return Norm(funcSU3_Re(RePar, ImPar, xiPar, deltaPar), funcSU3_Im(RePar, ImPar, xiPar, deltaPar), GPar)/Norm(-epsilon[0] * RePar, -epsilon[0] * ImPar, GPar)

# Easy accessors
def theoAdirCA(RePar, ImPar, GPar, xiPar, deltaPar):
    return theoAdir(funcSU3_Re(RePar, ImPar, xiPar, deltaPar), funcSU3_Im(RePar, ImPar, xiPar, deltaPar), GPar)

def theoAdirCS(RePar, ImPar, GPar):
    return theoAdir(-epsilon[0] * RePar, -epsilon[0] * ImPar, GPar)

def theoAmixCA(RePar, ImPar, GPar, FPar, xiPar, deltaPar, eta):
    return theoAmix(funcSU3_Re(RePar, ImPar, xiPar, deltaPar), funcSU3_Im(RePar, ImPar, xiPar, deltaPar), GPar, FPar, eta)

def theoAmixCS(RePar, ImPar, GPar, FPar, xiPar, deltaPar, eta):
    return theoAmix(-epsilon[0] * RePar, -epsilon[0] * ImPar, GPar, FPar, eta)

# Chi2 Minimizer
def chi2(RePar, ImPar, GPar, xiPar, deltaPar, model):
    chi2 = 0
    # Gamma Constraint
    if (GPar>=gamma[0]):
    ...
    chi2 = chi2 + pow(xiPar - xi_su3[0],2)/pow(xi_su3[1],2)\
           + pow(deltaPar - delta_su3[0],2)/pow(delta_su3[1],2)
    # Longitudinal Polarisation
    if ("Long" in model):
      chi2 = chi2 + pow(theoAdirCA(RePar, ImPar, GPar, xiPar, deltaPar) - AdirBs2JpsiKstar_Long[0],2)/pow(AdirBs2JpsiKstar_Long[1],2)\
             + pow(theoHobs(RePar, ImPar, GPar, xiPar, deltaPar) - HBsKstarBsPhi_Long[0],2)/pow(HBsKstarBsPhi_Long[1],2)
    # Parallel Polarisation
    elif ("Para" in model):
      chi2 = chi2 + pow(theoAdirCA(RePar, ImPar, GPar, xiPar, deltaPar) - AdirBs2JpsiKstar_Para[0],2)/pow(AdirBs2JpsiKstar_Para[1],2)\
             + pow(theoHobs(RePar, ImPar, GPar, xiPar, deltaPar) - HBsKstarBsPhi_Para[0],2)/pow(HBsKstarBsPhi_Para[1],2)
    # Perpendicular Polarisation
    elif ("Perp" in model):
      chi2 = chi2 + pow(theoAdirCA(RePar, ImPar, GPar, xiPar, deltaPar) - AdirBs2JpsiKstar_Perp[0],2)/pow(AdirBs2JpsiKstar_Perp[1],2)\
             + pow(theoHobs(RePar, ImPar, GPar, xiPar, deltaPar) - HBsKstarBsPhi_Perp[0],2)/pow(HBsKstarBsPhi_Perp[1],2)
    return chi2
...
def doMinuit(myChi2, tag, scan):
  ...
  m.values["xiPar"] = xi_su3[0]
  m.values["deltaPar"] = delta_su3[0]

  m.errors["RePar"] = 0.25
  m.errors["ImPar"] = 0.25
  m.errors["GPar"] = gamma[1]
  m.errors["xiPar"] = xi_su3[1]
  m.errors["deltaPar"] = delta_su3[1]
  ...
  xiFit = m.values["xiPar"]
  xiSym = m.errors["xiPar"]
  deFit = m.values["deltaPar"]
  deSym = m.errors["deltaPar"]
  bestchi2 = m.fval
  ...
  print ""
  print "     xi = %.3f +/- %.3f" % (xiFit, xiSym)
  print "  delta = [%.3f +/- %.3f] Deg" % (deFit * R2D, deSym * R2D)
...
if (False):
  ...
  def myChi2(RePar, ImPar, GPar, xiPar, deltaPar):
    return chi2(RePar, ImPar, GPar, xiPar, deltaPar, model)
...
if (True):
  for model in ["Long"]:
    for y in range(1,4):
      for x in range(0,51):
        xi_err = max(0.001, 0.01 * x)
        xi_su3 = [1, xi_err]
        delta_err = max(0.1*D2R, y * 10 * D2R)
        delta_su3 = [0, delta_err]

        fittag = model+"_xi"+str(int(100*xi_err))+"_delta"+str(int(y*10))

        def myChi2(RePar, ImPar, GPar, xiPar, deltaPar):
          return chi2(RePar, ImPar, GPar, xiPar, deltaPar, model)
        doMinuit(myChi2, fittag, False)
...
if (False):
  ...
  def myChi2(RePar, ImPar, GPar, xiPar, deltaPar):
    return chi2(RePar, ImPar, GPar, xiPar, deltaPar, "Long")
  doMinuit(myChi2, "test", False)
