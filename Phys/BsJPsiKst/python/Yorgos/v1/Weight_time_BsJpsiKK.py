...
##########################################################################################
    ...
    for entry in range(int(f_MC_tree_1.GetEntries())):
        ...
        weight_addr[0]=w0*Afunc(angles[0],angles[1],angles[2],Invariant_mass + 0J,time_,A_2pe,A_2pa,A_20,A_0,A_pa,A_pe,A_S).real  * pow(s_pdf(x=angles[1]), -1)
        Norm_weight_addr[0]=w0*Norm_Const*Afunc(angles[0],angles[1],angles[2],Invariant_mass+0J,time_,A_2pe*NB_2*nt2,A_2pa*NB_2*nt1,A_20*NB_2*nt1,A_0*NB_1*nt1,A_pa*NB_1*nt1,A_pe*NB_1*nt2,A_S*NB_0*nt1).real * pow(s_pdf(x=angles[1]),-1)
        weight_Branch.Fill()
