...
############################################################################
    ...
    const_old_pdf = integrate(expr,(x,-1,1),(y,-1,1),(z,-pi,pi),(m,Mmin,Mmax))
    cc = simplify(const_old_pdf).n()
    
    thita_integral = integrate(expr , (y,-1,1),(x,-1,1),(z,-pi,pi))
    ...
    for entry in range(int(f_MC_tree_1.GetEntries())):
        ...
        weight_addr[0] = W_exp * s_pdf(x=angles[1] , M=Invariant_mass + 0J) * pow(s_pdf_old(x = angles[1]), -1)
        Norm_weight_addr[0] = W_exp * Norm_Const * s_pdf(x=angles[1] , M=Invariant_mass + 0J)* pow(s_pdf_old(x = angles[1]),-1)
        weight_Branch.Fill()
