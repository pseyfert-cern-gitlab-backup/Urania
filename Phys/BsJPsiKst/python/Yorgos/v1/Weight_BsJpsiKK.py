...
################### the sum of the norm weights is this script is 1249 and the expected number is 1284 #################
...
def A_2pe(): return sqrt(2.55) * e**0.0J
def A_2pa():return sqrt(2.55)* e**0.0
def A_20(): return sqrt(9.9) * e**0.0J
def A_pa(): return sqrt(17.3) * e**0.0J
def A_pe(): return sqrt(17.3) * e**0.0J
def A_0(): return sqrt(32.1) * e**0.0J
def A_S(): return sqrt(12.0) * e**-4.6J
...
################################################################################
    ...
    const_old_pdf = integrate(expr,(y,-1,1),(x,-1,1),(z,-pi,pi),(m,Mmin,Mmax))
...
#################################################################################
    ...
    for entry in range(int(f_MC_tree_1.GetEntries())):
        ...
        weight_addr[0]= W_exp* Afunc(angles[0],angles[1],angles[2],Invariant_mass + 0J,A_2pe(),A_2pa(),A_20(),A_0(),A_pa(),A_pe(),A_S()).real  * pow(s_pdf(x=angles[1]), -1)
        Norm_weight_addr[0] =W_exp * Norm_Const * Afunc(angles[0],angles[1],angles[2],Invariant_mass + 0J,A_2pe()*NB_2,A_2pa()*NB_2,A_20()*NB_2,A_0()*NB_1,A_pa()*NB_1,A_pe()*NB_1,A_S()*NB_0).real  * pow(s_pdf(x=angles[1]), -1)
        weight_Branch.Fill()
