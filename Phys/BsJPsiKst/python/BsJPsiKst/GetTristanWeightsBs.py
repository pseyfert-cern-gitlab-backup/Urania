...
T = TransAmpModuli
P = TransAmpPhases
MCApa2 = 0.1711818200736396
MCA02 = 0.5100663351576334
MCApe2 = 1.-MCApa2-MCA02
MCdpa = 3.3016025583629416
MCdpe = 3.0218128449201496 -pi

def CalculateWeights(neim):
    t,fp = getTuple("~/vol5/Kst_2014/MC/selsignal/Bs/2011" + neim, thing = "DecayTree")
    t2,fp2 = getTuple("~/vol5/Kst_2014/MC/selsignal/Bs//2012" + neim, thing = "DecayTree")
    t.GetEntry(1)
    charge = t.Kplus_ID
...
f = file("weights_Bs.py","w")
f.write("WeightsBs = {}\n")
for i in range(len(Kpibins)-1):
    neim = "p_"  + str(Kpibins[i]) + "_" + str(Kpibins[i+1])
    out = CalculateWeights(neim)
    f.write("WeightsBs['2011"+neim + "'] = " + str(out) + "\n")
    f.write("WeightsBs['2012"+neim + "'] = " + str(out) + "\n")
