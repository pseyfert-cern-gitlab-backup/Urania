...
gROOT.ProcessLine(".x " + os.environ["BSJPSIKSTROOT"]+"/src/AngJpsiKst_Swave_Asym.cxx++")
#gROOT.ProcessLine(".x AngJpsiKst_Swave_Asym.cxx++")
#gSystem.Load("AngJPsiKst_cxx")
f11 = TFile("~/vol5/OL_2011_GL.root")
#f11 = TFile("/scratch11/Ntuples_BsJpsiKst/Olivier/2011/OL_2011_GL.root")
t = f11.Get("T")
...
Aprod_s = RooRealVar("A_prod_s","A_prod_s", 0.)
Aprod_d = RooRealVar("A_prod_d","A_prod_d", 0.)
A_D = RooRealVar("A_D","A_D", 0.)


#fBsn = RooRealVar("fBsn","fBsn", 1e-02,1e-03,.1)
...
##         self.Ape = RooFormulaVar("Ape"+namephys,"Ape"+namephys,"sqrt(Ape2"+namephys+")",RooArgList(self.Ape2))
    ...
    def make(self):
        name = self.namephys + self.namesample
        self.model = AngJpsiKst_Swave_Asym("angular " + name,"angular " + name, self.Kcharge, CPsi, CTheta, Phi, self.A0, self.Apa, self.Ape, \
                     self.As, self.dpa, self.dpe, self.ds, self.c1_psi, self.c2_psi, self.c3_psi, self.c4_psi, self.c5_psi, self.c1_theta, \
                     self.c2_theta, self.c1_phi, self.c2_phi, self.c3_phi, self.c4_phi, self.c5_phi, self.c6_phi)
