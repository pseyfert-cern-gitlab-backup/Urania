...
faccBs = TFile("Histo_ACC_MC10_Bs.root")
accBs = faccBs.Get("A")
...
f=TFile("/afs/cern.ch/user/d/diegoms/vol5/BsJPsiKst_341_v2.root")
t2=f.Get("T/T")
...
# acc is the acceptance histogram, specifically:  acc = accBs.Scale(1./accBs.GetSum())
eff0 = acc.GetSumOfWeights()*1./(acc.GetNbinsX()*acc.GetNbinsY()*acc.GetNbinsZ())
#eff0 = 1.

# databinned = acc.Clone(), and then filled with data from the tree , written above
...
for i in range(acc.GetNbinsX()):
    for j in range(acc.GetNbinsY()):
        for k in range(acc.GetNbinsZ()):
            ...
            #--------------
            ndp, ndn = poissonErrors(Ndata)    #It does not have entry for Ndata=6, for example,.... !!!
            #ndn, ndp = err_poisson.pois_err(int(Ndata))[0], err_poisson.pois_err(int(Ndata))[1]
            #ndn, ndp = sqrt(Ndata), sqrt(Ndata)
            #--------------
...
fitres = fit.model.fitTo(dataset,RooFit.Minos(0),RooFit.Save(),RooFit.SumW2Error( kTRUE ))

cv3, cphif, cthf, cpsif = replot(16)
