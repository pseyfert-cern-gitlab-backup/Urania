...
c1_psi_Bs = RooRealVar("c1_psi_Bs","c1_psi_Bs",0)
c2_psi_Bs = RooRealVar("c2_psi_Bs","c2_psi_Bs",0)
c3_psi_Bs = RooRealVar("c3_psi_Bs","c3_psi_Bs",0)
c4_psi_Bs = RooRealVar("c4_psi_Bs","c4_psi_Bs",0)

        
y_Bs = RooRealVar("y_Bs","y_Bs",1)
...
c2_theta_Bs = RooRealVar("c2_theta_Bs","c2_theta_Bs",0.)

c1_phi_Bs = RooRealVar("c1_phi_Bs","c1_phi_Bs",0.)
c2_phi_Bs = RooRealVar("c2_phi_Bs","c2_phi_Bs",0.)
c3_phi_Bs = RooRealVar("c3_phi_Bs","c3_phi_Bs",0.)
...
c1_psi_Bd = RooRealVar("c1_psi_Bd","c1_psi_Bd",0)
c2_psi_Bd = RooRealVar("c2_psi_Bd","c2_psi_Bd",0)
c3_psi_Bd = RooRealVar("c3_psi_Bd","c3_psi_Bd",0)
c4_psi_Bd = RooRealVar("c4_psi_Bd","c4_psi_Bd",0)

        
y_Bd = RooRealVar("y_Bd","y_Bd",1)
...
c2_theta_Bd = RooRealVar("c2_theta_Bd","c2_theta_Bd",0.)

c1_phi_Bd = RooRealVar("c1_phi_Bd","c1_phi_Bd",0.)
c2_phi_Bd = RooRealVar("c2_phi_Bd","c2_phi_Bd",0.)
c3_phi_Bd = RooRealVar("c3_phi_Bd","c3_phi_Bd",0.)
...
class AngularPDF:
    def __init__(self, neim = "Bd",acc=TH3D()):
        ...
        self.sigmodel_phys = AngJpsiKst_Swave("phys angular " + neim,"phys angular " + neim,CPsi,CTheta,Phi,self.A0,self.Apa,self.Ape,self.As,self.dpa,self.ds,
        ...
        self.acc_histo = RooDataHist("acc_histo_" + neim,"acc_histo_" + neim,RooArgList(CPsi,CTheta,Phi),acc)

        self.acceptance = RooHistPdf("acceptance_" + neim,"acceptance_" + neim,RooArgSet(CPsi,CTheta,Phi),self.acc_histo)
...
class AngularPDF_Rel:
    def __init__(self, pdf, neim = "Bs",acc=TH3D()):
...
if __name__=="__main__":
    ...
    AngBd = AngularPDF("Bd",accBd)
    AngBs = AngularPDF_Rel(AngBd,"Bs",accBd)
...
foutput=TFile("results/output_amplit_ratio_MC11_BkgPol_binned.root","recreate")
