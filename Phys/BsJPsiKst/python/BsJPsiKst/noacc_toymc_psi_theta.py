...
proj2d = fit.sigmodel.createProjection(RooArgSet(fit.Phi))
histo = proj2d.createHistogram("histo2d",fit.CPsi,RooFit.YVar(fit.CTheta))
histo10 = proj2d.createHistogram("histo2d",fit.CPsi,RooFit.YVar(fit.CTheta),RooFit.Binning(10))
histo10.RebinY(10)
#histo.Draw("LEGO2")
histodata = TH2F("data h","data h", 100, -1, 1,100,-1,1)
histodata10 = TH2F("data h 10","data h 10", 10, -1, 1,10,-1,1)
histomc = TH2F("mc h","mc h", 100, -1, 1,100,-1,1)
histomc10 = TH2F("mc h 10","mc h 10", 10, -1, 1,10,-1,1)
fmct= TFile("~/vol5/BdJpsiKst_mcgen.root")
tmct = fmct.Get("Bd2JPsiKstar/Bd2JPsiKstar")
for entry in t:
    histodata.Fill(entry.CPsi, entry.CTheta)
    histodata10.Fill(entry.CPsi, entry.CTheta)
    
for entry in tmct:
    histomc.Fill(entry.cTh1P, entry.cTh2P)
    histomc10.Fill(entry.cTh1P, entry.cTh2P)
