...
s_fpa = Symbol("fpa",positive = True)
#s_xpa = Symbol("xpa",positive = True)
fpe = 1-s_fL - s_fpa
Ap2 = 1-s_As2

from Urania.MassAmplitudes import *
Pw  = NoRelBW.subs( [(m0,PDG.Kst0.mass), (Gamma0,PDG.Kst0.width)])
Sw = Kmatrix_KpiSwave
mass_subs = [(TransAmpModuli["1_pa"], Pw*Sqrt(Ap2*s_fpa)),(TransAmpModuli["1_0"], Pw*Sqrt(Ap2*s_fL)),(TransAmpModuli["1_pe"], Pw*Sqrt(Ap2*fpe)),(TransAmpModuli["0_0"],Sw*Sqrt(s_As2))] 
...
#func = func*acc
ang_integral = iter_integrate(func, (z,-Pi,Pi),(x,-1,1),(y,-1,1))
func = func.subs( mass_subs )
ang_integral = ang_integral.subs(mass_subs)
func *= B2JpsiKpi_ps
ang_integral *= B2JpsiKpi_ps

### Figure out which variables the fit will depend on

# potential_list = [x,y,z]+TransAmpModuli.values()+TransAmpPhases.values()
potential_list = [x,y,z]+[ s_As2, s_fL, s_fpa]+TransAmpPhases.values() + [c1_psi,c2_psi,c3_psi,c4_psi,y_acc, c2_theta, mass]
...
op2 = D.RooClassGenerator(func, final_list,"NoAccMdepAngJpsiKpi_J" +str( max(spins)))
...
op2.forceIntegral(1,[x,y,z], ang_integral)
