from ROOT import *
from math import *
from parameters import BlindingString, WeightsBdKpi, WeightsBd#, WeightsBdScaled
#from weights_for_testing import *
import os
...
sWeights_Bd =  RooRealVar("sWeights_nBd","sWeights_nBd",-3.,3.)
sWeights_Bs =  RooRealVar("sWeights_nBs","sWeights_nBs",-3.,3.)

cor_sWeights_Bd =  RooRealVar("cor_sWeights_nBd","cor_sWeights_nBd",-3.,3.)
cor_sWeights_Bs =  RooRealVar("cor_sWeights_nBs","cor_sWeights_nBs",-3.,3.)
ipa_beta = RooRealVar("ipatia_beta" , "ipatia_beta", 0.)
ipa_zeta = RooRealVar("ipatia_zeta" , "ipatia_zeta", 1e-05)
...
Aprod_s = RooRealVar("Aprod_s","Aprod_s",8.3475e-06)
Aprod_d = RooRealVar("Aprod_d","Aprod_d",0.004239768)

AD_s = RooRealVar("AD_s","AD_s",-1.1029e-02)
AD_d = RooRealVar("AD_d","AD_d",1.155e-02)
...
class AngularPDFMOD:
    ...
    def OwnPhys(self, equalAsym = 0):
        ...
        self.fpa = RooRealVar("fpa"+namephys,"fpa"+namephys,0.172,0.1, 1.)
        ...
        if (DWAVE):
            ...
            self.fD.setVal((not SET0DWAVE)*self.fD.getVal())
            self.fD.setConstant(SET0DWAVE)
            self.f2L.setConstant(SET0DWAVE)
            self.f2pa.setConstant(SET0DWAVE)
            self.d20.setConstant(SET0DWAVE)
            self.d2pa.setConstant(SET0DWAVE)
        if self.Kcharge.getVal() >= 0:
            ...
            if (DWAVE):
                ...
                self.d2pe.setConstant(SET0DWAVE)
        else :
            ...
            if (DWAVE):
                ...
                self.d2pe_.setConstant(SET0DWAVE)
        self.ds = RooRealVar("ds"+namephys,"ds"+namephys,0 , -10*pi,10*pi)
        self.ACPS_ = RooRealVar("ACPS"+namephys,"ACPS"+namephys,0.,-1.0, 1.0)
        self.ACPL_ = RooRealVar("ACPL"+namephys,"ACPL"+namephys,0.,-1.0, 1.0)
        if (DWAVE):
            self.ACP2L_ = RooRealVar("ACP2L"+namephys,"ACP2L"+namephys,0.,-1.0, 1.0)
            if (SET0DWAVE): self.ACP2L_.setConstant(1)
        if equalAsym:
            ...
            self.ACPpa_ = RooRealVar("ACPpa"+namephys,"ACPpa"+namephys,0.,-1.0, 1.0)
            self.ACPpe_ = RooRealVar("ACPpe"+namephys,"ACPpe"+namephys,0.,-1.0, 1.0)
            if (DWAVE):
                self.ACP2pa_ = RooRealVar("ACP2pa"+namephys,"ACP2pa"+namephys,0.,-1.0, 1.0)
                self.ACP2pe_ = RooRealVar("ACP2pe"+namephys,"ACP2pe"+namephys,0.,-1.0, 1.0)
                self.ACP2pa_.setConstant(SETD0WAVE)
                self.ACP2pe_.setConstant(SETD0WAVE)
        ...
        if (BLIND and NO_CP == 0):
            ...
            if (DWAVE):
                if SETD0WAVE: self.ACP2L = self.ACP2L_
                else: self.ACP2L = RooUnblindPrecision("ACP2L"+namephys + "_UNB", "ACP2L"+namephys + "_UNB", BlindingString + "2L", 0,0.2, self.ACP2L_)
        ...
        if NO_CP:
            self.ACPS_.setVal(0.)
            self.ACPS_.setConstant(1)
            self.ACPL_.setVal(0.)
            self.ACPL_.setConstant(1)
            if (DWAVE):
                self.ACP2L_.setVal(0.)
                self.ACP2L_.setConstant(1)
            if not equalAsym:
                self.ACPpa_.setVal(0.)
                self.ACPpa_.setConstant(1)
                self.ACPpe_.setVal(0.)
                self.ACPpe_.setConstant(1)
                if (DWAVE):
                    self.ACP2pa_.setVal(0.)
                    self.ACP2pa_.setConstant(1)
                    self.ACP2pe_.setVal(0.)
                    self.ACP2pe_.setConstant(1)
