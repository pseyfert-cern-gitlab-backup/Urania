...
if (len(sys.argv)<4) :
  ...
  print "the fourth argument, cast to string, specifies further signal pdf additions, e.g. 'fit_', 'VLLRRRAD_', ..."
  print "the fifth argument, cast to int, determines the branching fraction hypothesis to be tested"
  sys.exit()
...
# FIXME: comment out the following line when using the dalitz operator pdfs, because these do not include a mass cutoff"
...
##base_2011_newcalib_down_trash.root
##base_2011_newcalib_down_RAD_trash.root
##base_2011_newcalib_down_VLLLLRAD_trash.root
##base_2011_newcalib_down_VLLLL_trash.root
##base_2011_newcalib_down_VLLRRRAD_trash.root
##base_2011_newcalib_down_VLLRR_trash.root
#operator = "VLLRRRAD_"
operator = sys.argv[4]
...
use_bkg_sys = False
use_sig_sys = False
#1: prompt ds 1sigma down  (3.90)
#2: prompt d+ 1sigma down  (4.39)
#3: B/b 1sigma down        (4.23)
#4: secondary ds 1sigma down(3.97)
#5: prompt ds 1sigma up    (4.15)
#6: prompt d+ 1sigma up    (4.01)
#7: B/b 1sigma up          (3.80)
#8: secondary ds 1sigma up (4.08)
...
outname = "prel_datfile_"
for s in range(1,len(sys.argv)):
  outname += sys.argv[s] + "_"
