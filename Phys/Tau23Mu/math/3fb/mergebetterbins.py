   ...
   for g in range(6):
     for p in range(5):
       ...
       if 1+glob==binnumber:
         gstart = g
         pstart = p
   for g in range(gstart,6):
     for p in range(pstart,5):
       glob = g+6*p
       for y in range(hist.GetNbinsY()):
           targetbin = retval.GetBin(1,y+1)
           sourcebin = hist.GetBin(int(glob+1),y+1)
           sourcebincontent = hist.GetBinContent(sourcebin)
           targetbincontent = retval.GetBinContent(targetbin)
           print " adding g=",g,"\tp=",p,"\tglob=",glob,"\t",sourcebincontent,"+",targetbincontent
           retval.SetBinContent(targetbin,sourcebincontent+targetbincontent)
   scale = retval.GetSum()
   if doscale:
