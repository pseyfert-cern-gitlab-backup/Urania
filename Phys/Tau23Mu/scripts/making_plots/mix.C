...
void mcroc() {
  ...
  //classifiers.push_back("NeuroBayes_v3_a_0x149cf818");
  //titles.push_back("best NeuroBayes A");
  //classifiers.push_back("NeuroBayes_v3_b_0x149cf818");
  //titles.push_back("best NeuroBayes B");
  //classifiers.push_back("NeuroBayes_v3_0x149cf818");
  //titles.push_back("best NeuroBayes");
  classifiers.push_back("NeuroBayes_v3_a_0x141cf818");
  titles.push_back("best NeuroBayes A");
  classifiers.push_back("NeuroBayes_v3_b_0x141cf818");
  titles.push_back("best NeuroBayes B");
  classifiers.push_back("NeuroBayes_v3_0x141cf818");
  titles.push_back("best NeuroBayes");
...
//(min(p0_ANNmuon,min(p1_ANNmuon,p2_ANNmuon))>-0.1)&&
  ...
  TString sigstring = "L0Dec&&Hlt1Dec&&cleaningcut&&mass_p0p1>250&&abs(mass_p0p2-1020.4)>20&&abs(mass_p1p2-1020.4)>20&&(min(p0_ANNmuon,min(p1_ANNmuon,p2_ANNmuon))>-0.1)&&(TriggerDecHlt2TriMuonTauDecision||TriggerDecHlt2CharmSemilepD2HMuMuDecision||TriggerDecHlt2DiMuonDetachedDecision)&&cleaningcut&&LifeTime<0.025";
  background->Draw(classifiers[0]+">>buffer",cutstring.Data());
  float allbkg = buffer->GetSum();
  signal->Draw(classifiers[0]+">>buffer",sigstring.Data());
  ...
  while (scanpoint < 1.001) {
    ...
    for (unsigned u = ustart ; u < classifiers.size() ; ++u) {
      signal->Draw(classifiers[u]+">>buffer",Form("(%s)*(%s>%f)",sigstring.Data(),classifiers[u].Data(),scanpoint));
  ...
  std::cout << "classifiers filled" << std::endl;
  ...
  unsigned colmax = cols.size();
  std::cout << "colors filled" << std::endl;
  for (unsigned u = 0 ; u < classifiers.size() ; ++u) {
  ...
  std::cout << "graphs filled" << std::endl;
  ...
  for (unsigned u = 0u ; u < classifiers.size() ; ++u) {
    ...
    if (drew) {
      ...
      drew = true;
      graphs[u]->Draw("al");
      graphs[u]->GetXaxis()->SetRangeUser(0.,0.5);
      graphs[u]->GetYaxis()->SetRangeUser(0.98,1.0);
    }
  }
  graphs[firstplot]->SetFillStyle(0);
