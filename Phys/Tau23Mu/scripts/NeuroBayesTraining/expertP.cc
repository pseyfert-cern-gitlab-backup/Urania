...
// ---------------------------------------------------------------
readNtuple* expert(int varset = 0x149cf818, TTree* tree = NULL) 
{
  ...
  Expert* expert  = new Expert(Form("taupmumu_%x.nb",varset));
  float response;


  readNtuple* ntuple = new readNtuple(tree);
  TBranch* b_nb = ntuple->fChain->Branch(Form("NeuroBayes_0x%x",varset),&response,Form("NeuroBayes_0x%x/F",varset));
...
int main(int argc, char** argv) {
  if (2>argc) {
    std::cout << "USAGE expert <varset=0x149cf818> <filename(s)>" << std::endl;
    return 1;
  }
  char* buffer;
