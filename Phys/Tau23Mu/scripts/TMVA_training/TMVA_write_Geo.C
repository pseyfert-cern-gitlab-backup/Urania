...
TTree* TMVACC( TString myMethodList = "" , int varset = 0xFFFFFF, TTree* intree = NULL, TTree* outtree = NULL)
{
  ...
  reader->BookMVA( TString("BDT method"), TString(Form("weights/GeoMVA2013_v3_%x_BDT.weights.xml",varset)));
  reader->BookMVA( TString("BDTold method"), TString(Form("weights/GeoMVA2013_v3_%x_BDTold.weights.xml",varset)));
  reader->BookMVA(TString("MLP method"),TString(Form("weights/GeoMVA2013_v3_%x_MLP.weights.xml",varset)));
  reader->BookMVA(TString("MLPCJ method"),TString(Form("weights/GeoMVA2013_v3_%x_MLPCJ.weights.xml",varset)));
  ...
  TBranch* b_nb   = outtree->Branch(Form("TMVA_v3_0x%x",varset),&response,Form("TMVA_v3_0x%x/F",varset));
  TBranch* b_nbold   = outtree->Branch(Form("oldTMVA_v3_0x%x",varset),&responseold,Form("oldTMVA_v3_0x%x/F",varset));
  TBranch* b_nb3  = outtree->Branch(Form("MLP_v3_0x%x",varset),&mlp_response,Form("MLP_v3_0x%x/F",varset));
  TBranch* b_nb4  = outtree->Branch(Form("MLPCJ_v3_0x%x",varset),&mlpcj_response,Form("MLPCJ_v3_0x%x/F",varset));
...
int main(int argc, char** argv) {
  ...
  //for (int i = 2 ; i < argc ; ++i) {
  {
    ...
    for (std::map<std::string,int>::iterator  it  = done.begin() ; it != done.end() ; ++it) {
      ...
      intree->GetEntries();
      intree->GetEntry(1);
      printf("got tree in file %s\n",intree->GetCurrentFile()->GetName());
