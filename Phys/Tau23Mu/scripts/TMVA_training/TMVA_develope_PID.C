...
void TMVACC( TString myMethodList = "" , int varset = 0xFFFFFF)
{
   ...
   TString outfileName( Form("%xfewer.root",varset) );
   ...
   TMVA::Factory *factory = new TMVA::Factory( Form("PID_v4_%x",varset), outputFile,
...
//   factory->AddVariable("p2_eta");
   if (varset & 1 << 0)
   factory->AddVariable("track_Chi2Dof");
//   factory->AddVariable("p1_track_Chi2Dof");
//   factory->AddVariable("p2_track_Chi2Dof");
   if (varset & 1 << 1)
   factory->AddVariable("track_MatchChi2");
//   factory->AddVariable("p1_track_MatchChi2Dof");
//   factory->AddVariable("p2_track_MatchChi2Dof");
...
//   factory->AddVariable("p2_MuDLL");
   if (varset & 1 << 5)
   factory->AddVariable("MuBkg");
   if (varset & 1 << 6)
   factory->AddVariable("richMU");
   if (varset & 1 << 7)
   factory->AddVariable("richK");
   if (varset & 1 << 8)
     {}
   if (varset & 1 << 9)
   factory->AddVariable("ecalMU");
   if (varset & 1 << 10)
   factory->AddVariable("hcalMU");

   TString basicvars = "MuLL:MuBkg:richMU:richK:ecalMU:hcalMU";
  
   if (varset & 1 << 11)
   factory->AddVariable("observedTT");
   if (varset & 1 << 12)
   factory->AddVariable("observedOT");
   if (varset & 1 << 13)
   factory->AddVariable("observedIT");
...
*/
   ...
   TString fname = "/tmp/sigPID.root";
   TString bname = "/tmp/bkgPID.root";
...
TCut mycuts = "((expectedIT==0)||(expectedOT==0))&&(expectedIT+exptectedOT>0)&&L0Dec&&Hlt1Dec&&Hlt2Dec&&cleaningcut&&abs(mass_p0p2-1020)>20&&abs(mass_p1p2-1020)>20&&mass_p0p1>250&&LifeTime<0.025&&LifeTime>-0.01&&dira>0.99&&truetau==15&&truemuon!=0";
TCut mycutb = "((expectedIT==0)||(expectedOT==0))&&(expectedIT+exptectedOT>0)&&L0Dec&&Hlt1Dec&&Hlt2Dec&&cleaningcut&&abs(mass_p0p2-1020)>20&&abs(mass_p1p2-1020)>20&&mass_p0p1>250&&LifeTime<0.025&&LifeTime>-0.01&&dira>0.99&&truemuon==0";
   ...
   if (Use["BDTG"]) // Gradient Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDTG",
                           "!H:!V:NTrees=50:BoostType=Grad:Shrinkage=0.5:UseBaggedGrad:GradBaggingFraction=0.5:nEventsMin=65:MaxDepth=8:NNodesMax=8" );
...
*/
   ...
   if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=525:nEventsMin=20:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20:PruneMethod=NoPruning" );
...
*/
     ...
     TMVA::MethodBase* cats = factory->BookMethod(TMVA::Types::kCategory,"GHOST","");
   TMVA::MethodCategory* mcat = dynamic_cast<TMVA::MethodCategory*>(cats);

      mcat->AddMethod("expectedIT==0&&expectedTT==0",basicvars+"observedOT",TMVA::Types::kBDT, "BDT_OT","!H:!V:NTrees=525:nEventsMin=20:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20:PruneMethod=NoPruning" );
      mcat->AddMethod("expectedOT==0&&expectedTT==0",basicvars+"observedIT",TMVA::Types::kBDT, "BDT_IT","!H:!V:NTrees=525:nEventsMin=20:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20:PruneMethod=NoPruning" );
      mcat->AddMethod("expectedIT==0&&expectedTT!=0",basicvars+"observedOT:observedTT",TMVA::Types::kBDT, "BDT_OT","!H:!V:NTrees=525:nEventsMin=20:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20:PruneMethod=NoPruning" );
      mcat->AddMethod("expectedOT==0&&expectedTT!=0",basicvars+"observedIT:observedTT",TMVA::Types::kBDT, "BDT_IT","!H:!V:NTrees=525:nEventsMin=20:MaxDepth=4:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20:PruneMethod=NoPruning" );
   ...
   if (Use["BDTB"]) // Bagging
      factory->BookMethod( TMVA::Types::kBDT, "BDTB",
                           "!H:!V:NTrees=1000:BoostType=Bagging:SeparationType=GiniIndex:nCuts=20:MaxDepth=10:nEventsMin=20:PruneMethod=NoPruning" );
...
int DLL, RICH, MUON, CAL, MATCH, TRACK;
...
/*
  ...
  //return;
  initvar();
  TMVAGui(Form("%xfewer.root",varset));
}
