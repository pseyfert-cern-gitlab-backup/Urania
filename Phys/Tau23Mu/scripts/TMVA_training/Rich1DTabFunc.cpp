                                            ...
                                            const gsl_interp_type * interType )
{
  ...
  if ( x.size() != y.size() )
  {
    m_OK = false;
    clearInterpolator();
    throw std::string( "Size of x vector != size of y vector");
  }
  else
  {
    // copy data to internal container
...
bool TabulatedFunction1D::initInterpolator( const gsl_interp_type * interType )
{
  ...
  {
    clearInterpolator();
    throw std::string( "Error whilst initialising GSL interpolators");
    return false;
  }
                          ...
                          const TabulatedFunction1D * weightFunc ) const
{
  if ( samples < 2 )
  {
    throw std::string( "rms() : samples must be > 1");
  }
                                        ...
                                        const TabulatedFunction1D * weightFunc ) const
{
  if ( samples < 2 )
  {
    throw std::string( "standardDeviation() : samples must be > 1" );
  }
                                                    ...
                                                    const gsl_interp_type * interType )
{
  if ( samples < 2 )
  {
    throw std::string( "combine() : samples must be > 1" );
  }
