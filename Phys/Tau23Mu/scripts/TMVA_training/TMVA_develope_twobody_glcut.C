...
void TMVACC( TString myMethodList = "" , int varset = 0xFFFFFF)
{
   ...
   TString outfileName( Form("twobody_glcutp.root",varset) );
   ...
   TMVA::Factory *factory = new TMVA::Factory( Form("twobody_glcutp",varset), outputFile,
...
//factory->AddVariable("p2_expectedTT");
   ...
   Double_t BbackgroundWeight = 0.;//10./9.1;
   Double_t CbackgroundWeight = 0.;//10./4.4;
...
//   TCut mycutb = "cleaningcut&&L0Dec&&Hlt1Dec&&Hlt2Dec"; // for example: TCut mycutb = "abs(var1)<0.5";
     ...
     TCut mycuts;
     mycuts = "GeoMVA>0.55&&L0Dec&&Hlt1Dec&&Hlt2Dec&&cleaningcut&&abs(mass_p0p2-1020)>20&&abs(mass_p1p2-1020)>20&&mass_p0p1>250&&LifeTime<0.025&&LifeTime>-0.01&&dira>0.99&&truetau==15";
     TCut mycutb = "GeoMVA>0.55&&L0Dec&&Hlt1Dec&&Hlt2Dec&&cleaningcut&&abs(mass_p0p2-1020)>20&&abs(mass_p1p2-1020)>20&&mass_p0p1>250&&LifeTime<0.025&&LifeTime>-0.01&&dira>0.99";
...
void TMVA_develope_twobody_glcut() {
  TMVACC();
  int varset = 0000;
  TMVAGui(Form("twobody_glcutp.root",varset));
}
