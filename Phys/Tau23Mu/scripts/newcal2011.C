...
// prefixes :MC_Sig_pDsMinus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_BPlus_offline.root.Down.2.45.2011.root
// prefixes :MC_Sig_pDPlus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_DsPlus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_pDsPlus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_DsMinus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_pDMinus_offline.root.Down.0.45.2011.root
// prefixes :MC_Sig_BMinus_offline.root.Down.0.45.2011.root
...
void setmasses(int par, float sigmas) {
  ...
  g_taumass = 1779.1; // 0.1
  if (1==par) {
    g_taumass = 1779.1 + sigmas* 0.1;
  }
  g_tauwidth1= 7.7;   // 0.1
  if (2==par) {
    g_tauwidth1= 7.7 + sigmas* 0.1;
  }
  g_tauwidth2= 12.;   // 0.8
  if (3==par) {
    g_tauwidth2= 12. + sigmas * 0.8;
  }
...
// if (4==prechoice) return  TString("(2.-(((production==5)*(1.2234+0.2234))+((production!=5)*(0.9768-(1.-0.9768)))))*(");
 ...
 if (prechoice<=8 && prechoice >=1) {return TString("(");}
...
limit_interface* newcal(bool up = true, int prechoice = 0) {
  ...
  for (int p = 0 ; p < li->the_pidbins() ; ++p) {
    for (int g = 0 ; g < li->the_geobins() ; ++g) {
      ...
      if (0==p) {
        if (up) {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Up.0.4.2011.root","read");
        } else {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Down.0.4.2011.root","read");
        }
      }
      if (1==p) {
        if (up) {
      ...
      if (2==p) {
        if (up) {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Up.0.45.2011.root","read");
        } else {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Down.0.45.2011.root","read");
        }
      }
      if (3==p) {
        if (up) {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Up.0.54.2011.root","read");
        } else {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Down.0.54.2011.root","read");
        }
      }
      if (4==p) {
        if (up) {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Up.0.63.2011.root","read");
        } else {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Down.0.63.2011.root","read");
        }
      }
      if (5==p) {
        if (up) {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Up.0.75.2011.root","read");
        } else {
          ff = new TFile(TString("~/physiwork/pseyfert/zoontuple/Urania_v2r1/PIDCalib/PIDPerfScripts/scripts/python/MultiTrack/")+prefix+".Down.0.75.2011.root","read");
        }
      }
  ...
  for (int p = 0 ; p < li->the_pidbins() ; ++p) {
    for (int g = 0 ; g < li->the_geobins() ; ++g) {
      ...
      if (0!=p) {
        ...
        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)");
        den = buffer->GetSum();
        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)*Event_PIDCalibEff");
        num = buffer->GetSum();

        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)*(Event_PIDCalibEff+Event_PIDCalibErr)");
        nume = buffer->GetSum();
        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)*(Event_PIDCalibEff-Event_PIDCalibErr)");
        numee = buffer->GetSum();
      ...
      if (p < li->the_pidbins() -1) {
        ...
        trees[glob2]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)");
        float den2 = buffer->GetSum();
        trees[glob2]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)*Event_PIDCalibEff");
        float num2 = buffer->GetSum();

        trees[glob2]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")*(Event_PIDCalibEff!=0)*(Event_PIDCalibEff+Event_PIDCalibErr)");
  ...
  for (int p = 0 ; p < li->the_pidbins() ; ++p) {
    for (int g = 0 ; g < li->the_geobins() ; ++g) {
      ...
      for (int mm = 0 ; mm < (int)(li->GetNMassbins()+0.1) ; ++mm) {
        ...
        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+"&&"+Form("MN_BLEND_FLAT>%f&&MN_BLEND_FLAT<%f",li->GetGeolow(g),li->GetGeohigh(g))+")");
        float num = buffer->GetSum();
        trees[glob]->Draw("mass>>buffer",preweight(prechoice)+li->CutString2011()+")");
...
void newcal2011(int c) {
  ...
  limit_interface* li;
    ...
    if (c!=0) {
      ff = new TFile(Form("base_2011_calibV4_up_sys%d.root",c),"recreate");
    } else {
      ff = new TFile(Form("base_2011_calibV4_up_noveto.root"),"recreate");
    }
    ff->WriteTObject(calib);
    ...
    calibm->SetName("minus");
    if (c!=0) {
      ff = new TFile(Form("base_2011_calibV4_down_sys%d.root",c),"recreate");
    } else {
      ff = new TFile(Form("base_2011_calibV4_down_noveto.root"),"recreate");
    }
    ff->WriteTObject(calib);
    ff->WriteTObject(calibp);
    ff->WriteTObject(calibm);
