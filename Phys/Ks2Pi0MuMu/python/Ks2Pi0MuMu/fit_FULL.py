...
from scipy import random as rnd
import ipa_dmsFULL as ipa_params
gROOT.ProcessLine(".L $URANIAROOT/src/RooIpatia2.cxx++")


L = 50
if len (sys.argv)>1: L = float(sys.argv[1])
kPaula = TColor.GetColor("#ff99cc")
mass_name = "M_VC"
bdtname = "DMGLBDTG"

######  Prepare files
tmm0, mainF =  getTuple(MY_TUPLE_PATH + "TIS_FULL_GL_2")
#BREAK
mass = RooRealVar(mass_name,mass_name,420,580)
Mass = mass
samplename = "sample"

NTIS_pipi = (367797 + 993896) * 0.5 ## We split half for training and half for fitting
Leff= 4.84e-03/2 ## We split half for training and half for fitting
...
alpha0 = 0.692*1./NTIS_pipi * (eff_PIPI/eff_Sig)#*1./Leff
BREAK
...
def doToy(leibol, gendeita):
    ...
    for i in datamm.keys():
        ...
        #B = genB(i)* L/6e-04
        B =rnd.normal( meanbs[str(i)], sigmasbs[str(i)]) * L*1./(Leff)
