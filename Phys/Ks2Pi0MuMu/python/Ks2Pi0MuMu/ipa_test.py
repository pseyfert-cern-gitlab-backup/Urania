...
alpha0 = 1#0.692*1./147000. * (.015*1e-03/1e-03) 
...
BR_s = RooRealVar("BRs","BRs",0,tmm0.GetEntries())
...
class Ks2Pi0MuMuModel:
    ...
    def __init__(self, glbin):
        ...
        self.nbs = RooRealVar("NBs" + i ,"NBs" + i, 0, tmm0.GetEntries())
       
        self.nbkg = RooRealVar("MuMuBkg"+i,"MuMuBkg"+i, 0)
...
fiter.fitTo(allData,RooFit.Minos(kFALSE))#, RooFit.ExternalConstraints(RooArgSet(summaryConstraints)),RooFit.Strategy(2))
...
def doToy(leibol):
    ...
    fiter.fitTo(deita2, RooFit.Minos(kFALSE))
