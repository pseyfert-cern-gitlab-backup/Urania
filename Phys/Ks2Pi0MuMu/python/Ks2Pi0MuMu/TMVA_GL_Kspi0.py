...
#mu1pt, mu2pt
#check mu1_hitsInOT for diff between MC and data
...
#mycutSig_ =  "(mu1_hitsInTT > 0.1) && (mu2_hitsInTT > 0.1) && (LF_time > 1) && ((2*105.658*105.658 + 2*sqrt(105.658*105.658+mu1p1*mu1p1+mu1p2*mu1p2+mu1p3*mu1p3)*sqrt(105.\
#658*105.658+mu2p1*mu2p1+mu2p2*mu2p2+mu2p3*mu2p3)- 2*(mu1p1*mu2p1+mu1p2*mu2p2+mu1p3*mu2p3)) < 450*450)"
mycutSig = TCut(TMVA_cut.mycutSig) 
mycutBkg = TCut(TMVA_cut.mycutSig)
