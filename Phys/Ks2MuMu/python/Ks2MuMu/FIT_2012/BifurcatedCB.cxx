...
Double_t BifurcatedCB::ApproxErf(Double_t arg) const
{
 ...
 Double_t BifurcatedCB::evaluate() const
 {
   ...
   if ( t <= -absAlphaL )
   {
      double a = Power(nL/absAlphaL, nL) * Exp(-0.5*absAlphaL*absAlphaL) ;
      double b = nL/absAlphaL - absAlphaL ;
      result = a/Power(b-t, nL) ;
   }
   // Right side
   else if ( t >= fabs(alphaR) )
   {
      double a = Power(nR/absAlphaR, nR) * Exp(-0.5*absAlphaR*absAlphaR) ;
      double b = nR/absAlphaR - absAlphaR ;
      result = a/Power(b+t, nR) ;
   }
   // Gaussian center
   else
   {
...
Double_t BifurcatedCB::analyticalIntegral(Int_t code, const char* rangeName) const
{
   ...
   if ( tmin <= -absAlphaL ) // Tmin is in the left tail
   {
      ...
      {
          if (useLogL) return a * sig * ( Log(b-tmin) - Log(b-tmax) ) ;
          else         return a*sig/(1.0-nL) * (1.0/Power(b-tmin, nL-1.0) - 1.0/Power(b-tmax, nL-1.0)) ;
      }
      else if ( tmax > -absAlphaL && tmax <= absAlphaR )
      {
          ...
          else         result = a*sig/(1.0-nL) * (1.0/Power(b-tmin, nL-1.0) - 1.0/Power(nL/absAlphaL, nL-1.0)) ;
      ...
      else // We need to integrate the full gaussian part
      {
          ...
          double aR = Power(nR/absAlphaR, nR)*Exp(-0.5*absAlphaR*absAlphaR) ;
          double bR = nR/absAlphaR - absAlphaR ;
          if (useLogR) return (result + aR*sig*( log(bR+tmax) - log(nR/absAlphaR) )) ;
          else         return (result + aR*sig/(1.0-nR)*(1.0/Power(bR+tmax, nR-1.0)-1.0/Power(nR/absAlphaR, nR-1.0))) ;
      }
   }
   else if ( tmin > -absAlphaL && tmin <= absAlphaR ) // Tmin is in the Gaussian part
   {
      ...
      else
      {
          ...
          // Finally, the right tail
          double aR = Power(nL/absAlphaR, nR)*Exp(-0.5*absAlphaR*absAlphaR) ;
          double bR = nR/absAlphaR - absAlphaR ;
          if (useLogR) return (result + aR*sig*( log(bR+tmax) - log(nR/absAlphaR) )) ;
          else         return (result + aR*sig/(1.0-nR)*(1.0/Power(bR+tmax, nR-1.0)-1.0/Power(nR/absAlphaR, nR-1.0))) ;
   ...
   {
      double a = Power(nL/absAlphaR, nR)*Exp(-0.5*absAlphaR*absAlphaR) ;
      double b = nR/absAlphaR - absAlphaR ;
      if (useLogR) return a*sig*( log(b+tmax) - log(nR/absAlphaR) ) ;
      else         return a*sig/(1.0-nR)*(1.0/Power(b+tmax, nR-1.0)-1.0/Power(nR/absAlphaR, nR-1.0)) ;
   }
