...
def plot_with_residuals(canvas, frame, roovar, nbins, r_min, r_max, r_blind_min, r_blind_max):
     ...
     xMin = xaxis.GetXmin()
     xMax = xaxis.GetXmax()
     res = rt.TH1F("residuals", "", nbins, xMin, xMax)
     ...
     line = rt.TLine(r_min,-3.,r_max,-3)
     line.SetLineStyle(1)
     line.SetLineColor(2)
     line.SetLineWidth(1)
     line.Draw()
     line2 = rt.TLine(r_min,+3.,r_max,+3)
...
if eopt=="BifurCB":
     ...
     '''
     misid_n = RooRealVar('misid_n', 'misid_n', 3, 1.5, 100)
     misid_m = RooRealVar('misid_m', 'misid_m', 410, 200, 420)
     signalK = rt.RooPowerLaw('misid', 'misid', kMass, misid_m, misid_n)
...
if bkg=='expo': 
     slopeK =  RooRealVar("slopeK", "slopeK",   -0.02, -0.1, 0)
...
model.plotOn(kFrame,RooFit.Components("signalK"),RooFit.LineStyle(9),RooFit.LineWidth(2),RooFit.LineColor(rt.kOrange), RooFit.Range("left,right"),RooFit.NormRange("left,right"))
model.plotOn(kFrame,RooFit.Components("ChebK"),RooFit.LineStyle(9),RooFit.LineWidth(2), RooFit.LineColor(rt.kRed), RooFit.Range("left,right"),RooFit.NormRange("left,right"))
