...
#===============================================
# 2013 - analysis of the 2011 dataset
#===============================================
...
Bsmumu_BR = EVal( 3.34e-9, 0.27e-9 )

# ratio of production fractions:
#https://groups.cern.ch/group/bsmumu-authors/Lists/Archive/Flat.aspx?RootFolder=%2Fgroup%2Fbsmumu-authors%2FLists%2FArchive%2Fimproved%20D_s%20BRs%20from%20CLEO&FolderCTID=0x01200200FDDA4DAA3D364E4F8BB386765CF56DC5
fsfd = EVal(0.259,0.016)
fdfs = fsfd.invert()
fsfs = EVal(1.,0.)
fdfd = EVal(1.,0.)
...
JpsiE_trg = EVal(0.882,[0.005,0.025]) # Justine 20130516
JpsiE_trg = JpsiE_trg.compress_errors()
BuE_trg = JpsiE_trg

# Trigger efficiency for Bsmumu
#https://indico.cern.ch/getFile.py/access?contribId=6&resId=0&materialId=slides&confId=250103
BmmE_trg = EVal(0.921,[0.005,0.016]) # Justine 20130516
BmmE_trg = BmmE_trg.compress_errors()

# Trigger efficiency ratio for BuKpi/Bsmumu
#https://indico.cern.ch/getFile.py/access?contribId=6&resId=0&materialId=slides&confId=250103
BuRatE_trg = EVal(0.958,[0.007,0.019]) # Justine 20130516
BuRatE_trg = BuRatE_trg.compress_errors()

# TIS trigger ebdEficiency
#TISE_trg = EVal(0.0577 , 0.0004) # Siim
TISE_trg = EVal(0.0518, 0.0004) # Siim 20130611
TISE_trg.add_error(0.004) # Johannes 190711 (We keep the same) # Marco make it absolute
...
BmmE_massCorr = EVal(0.99305 , 0.00058)# Francesco 21062013
...
BmmE_ghostProb = EVal(0.9970, 0.0001) #Siim 30052013
BmmE_ghostProb.add_relative_error(0.0004) # Barbara 30052013
BmmE_ghostProb = BmmE_ghostProb.compress_errors()

BmmE_recsel = BmmE_recsel_raw * BmmE_ghostProb * BmmE_tracking
...
#Bu_MuId = EVal(1.029,0.) # Barbara 140513
Bu_MuId = EVal(1.029,0.) # Barbara 090613
...
#BuRatE_recsel.add_relative_error(0.009) # muID acceptance and efficiency, PID, Barbara 140513
BuRatE_recsel.add_relative_error(0.008) # muID acceptance and efficiency, PID, Barbara 090613

BuRatE_recsel = BuRatE_recsel.compress_errors()

# BuRatE_trg  is computed by Justine and provided as input above

#BuNcan = EVal(364390.6,615.7)# Siim 04042013  from 1041.7pb 2011 S20r1 #wrong, included 71pb twice..
BuNcan = EVal(338650.9,593.4)# Siim 09052013  from 970.7pb 2011 S20r1 new model!
...
#Bd_MuId = EVal(1.020,0.) # Barbara 140513
Bd_MuId = EVal(1.020,0.) # Barbara 090613 
...
BdE_HLT2MC = EVal(0.915, 0.003) #Justine 20130611
...
#BdRatE_recsel.add_relative_error(0.006) # muID acceptance and efficiency, PID,  Barbara 140513
BdRatE_recsel.add_relative_error(0.004) # muID acceptance and efficiency, PID, Barbara 090613
...
#BhhNcanTis = EVal( 27036., 1376. ) # UPDATE!

#BdKpiNcan = Bhh_fkp*BdNcan
#BdKpiNcan = EVal(14579 , 1110)  #Christian, Nov.Ana.
#BdKpiNcan = EVal(11977, 932)  #Christian 26042604
BdKpiNcan = EVal(10472, 430)  #Christian 04072013
BdNcan = BdKpiNcan
...
# to be updated with new numbers:
    ...
    ejpsi_trg.add_relative_error(0.018) # TISTOS Justine 260413
    #ebsmm_trg.add_relative_error(0.02) # Extra lines Justine 140711 , why ebsmm, should be jpsi? (Siim)
    # Extra lines are already accounted for in the JpsiE_trg, Siim

    ebsmm_trg = BmmE_trg
    #ebsmm_trg.add_relative_error(0.03) # TISTOS Justine 140711
    ebsmm_trg.add_relative_error(0.001) # TISTOS Justine 260413
