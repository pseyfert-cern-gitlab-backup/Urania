...
#include "CalibDataScripts/LcfromBFit.h"
...
void LcfromBFit(TChain* ch, 
             const char* charge,
             const char* pName,
             const char* wsName,
             const char* fName,
             const char* dsName,
             const char* LcfromBName,
             ...
             Bool_t exitOnBadFit)
{
  ...
  // Constants
  //==================================================//
  Float_t LcMassPDG=2286.46; //check! 
  ...
  massFitter->SetDMassName(LcfromBName);
  massFitter->SetDMassSigModelName("sigModel");
  massFitter->SetDMassBkgModelName("bkgModel");
  massFitter->MakeDMassVar(LcMassPDG-110, //change the range!
                           LcMassPDG+110,
                           "MeV/c^{2}",
                           "m_{pK#pi}");
  ...
  /*                                   
  massFitter->MakeDMassSigGauss(LcMassPDG, LcMassPDG-5, LcMassPDG+5, //mean mass and limits (units = MeV)
				11.0, 0.0, 30.0); // sigma and limits. Change to something sensible)

  */                           



  massFitter->MakeDMassSigCB(LcMassPDG, LcMassPDG-10, LcMassPDG+10, //mass 
			     12.0, 9.0, 30.0,//sigma
			     10, 0, 30, //alpha
			     1, 0, 10);//n
  ...
  //  massFitter->MakeDMassBkgExp(-1000,-9000,99999); // central values, lower limit, upper limit, units MeV. change to something sensible.

  //==================================================//
  // Make composite model with following estimate of 
  // yield breakdown
  //==================================================//
  massFitter->MakeDMassModel(0.25, 0.75);// numbers are signal fraction, 1- signal fraction. Change to something appropriate so the fit starts in a reasonable place.
...
//  plotParams+=",dmass_bkg_poly_c1,dmass_bkg_poly_c2";
  ...
  massFitter->SetPlotBins(LcfromBName, nbins);

  
  //==================================================// 
  // Make plots
  //==================================================//
  massFitter->SetRange(LcfromBName, LcMassPDG-55,LcMassPDG+55, "sigRange");
  ...
  // Plot Lc Mass Fit
  //==================================================//
  RooPlot* rp_mass = massFitter->PlotFitResults(LcfromBName, 
                                                "fitResults",
                                                "sigRange",
                                                rangeTitle.Data(),
  ...
  // get the pull plot
  RooPlot* rp_mass_pull = massFitter->PlotFitPulls(LcfromBName,
                                                   "fitResults");
  ...
  c1_lower->cd();
  rp_mass_pull->Draw();
  c1->SaveAs(TString::Format("FitRes_%s_%s_%s_%s.eps",LcfromBName,pName,
                             charge,index).Data());
  ...
  rp_pdll->Draw();
  c2->SaveAs(TString::Format("DLLP_%s_%s_%s.eps",pName,charge,index).Data());
  ...
  rp_probnnp->Draw();
  c3->SaveAs(TString::Format("ProbNNP_%s_%s_%s.eps",
  ...
  rp_probnnp->Draw();
  c4->SaveAs(TString::Format("ProbNNP_%s_%s_%s.eps",
                             pName,charge,index).Data());
  */
  //==================================================// 
  // Clean up
...
int main(int argc, char *argv[])
{
  ...
  const char* wsName="LcfromBCalib";
  ...
  BOOST_FOREACH(const ssMap::value_type& charge, charges) {
    tupleFullName.Form("Lb2LcMuNu_Lc2PKPiTuple/%s", 
    ...
    fname.Form("%s/LcfromB_P%s_Mag%s_Strip%s_%s.root",
    ...
    std::cout << "Making Lc fit for "
                ...
                << ")" << std::endl;
    try {
        LcfromBFit(t, (charge.first).Data(), 
                pName,
