                         ...
                         Bool_t exitOnBadFit)
{
  ...
  {
    std::cerr << "D* " << charge << " MuonUnBiased " << pName 
  ...
  TString chargeCut = TString::Format("%s_ID/abs(%s_ID)=="+chrg, 
    pName, pName);
  TString muonUnBiasedCut = TString::Format("%s_MuonTisTagged==1",
    pName);
  TString Cuts = TString::Format("(%s) && (%s)", 
    chargeCut.Data(), muonUnBiasedCut.Data());
  ...
  */
  ...
  // yield breakdown
  //==================================================//
  massFitter->MakeDelmModel(0.35, /* frac(signal) */
                            0.55, /* frac(comb) */
                            0.1, /* frac(wrong pi_s) */
                            -1.0); /* frac(bkg-like D0, signal-like delta(m) */
  ...
  if ( !(checkFitStatus(*massFitter, "fitResults", status)) ) 
  {
    ...
    cerr << "D* " << charge << " MuonUnBiased " << pName 
  ...
  {
    cerr << "WARNING: " << "D* " << charge << " MuonUnBiased "
  ...
  massFitter->SetRange(d0Name,d0MassPDG-40,d0MassPDG+40,"sigRange");
  ...
  c1->SaveAs(TString::Format("FitRes_%s_%s_MuonUnBiased_%s_%s.eps", d0Name,pName,
                             charge, index).Data());
  ...
  c2->SaveAs(TString::Format("FitRes_%s_%s_MuonUnBiased_%s_%s.eps",
                             delmName,pName,
                             charge,index).Data());
  ...
  TCanvas* c3 = new TCanvas("cnv_pidk", 
                            "", 1024, 768);
  rp_pidk->Draw();
  c3->SaveAs(TString::Format("DLLK_%s_MuonUnBiased_%s_%s.eps",
                             pName,charge,index).Data());
  
  //==================================================// 
  // Plot weighted DLL(mu-pi) distribution
  //==================================================//
  varName.Form("%s_CombDLLmu",DSpName);
  massFitter->SetRange(varName.Data(), -150., 150., "hasRichRange");

  RooPlot* rp_pidmu = massFitter->PlotVariable(varName.Data(),
                                                "", "", kTRUE);
  TCanvas* c4 = new TCanvas("cnv_pidmu", "", 1024, 768);
  rp_pidmu->Draw();
  c4->SaveAs(TString::Format("DLLmu_%s_MuonUnBiased_%s_%s.eps",
                             pName,charge,index).Data());
  
  //==================================================// 
  // Plot weighted ProbNN(K) distribution
  //==================================================//
  RooPlot* rp_probnnk = massFitter->PlotVariable(TString::Format("%s_ProbNNK",
                                                                 DSpName).Data(),
                                                 "", "", kTRUE);
  TCanvas* c5 = new TCanvas("cnv_probnnk", "", 1024, 768);
  rp_probnnk->Draw();
  c5->SaveAs(TString::Format("ProbNNk_%s_MuonUnBiased_%s_%s.eps",
                             pName,charge,index).Data());
  ...
  RooPlot* rp_probnnpi = massFitter->PlotVariable(TString::Format("%s_ProbNNpi",
                                                                  DSpName).Data(),
                                                  "", "", kTRUE);
  TCanvas* c6 = new TCanvas("cnv_probnnpi", "", 1024, 768);
  rp_probnnpi->Draw();
  c6->SaveAs(TString::Format("ProbNNpi_%s_MuonUnBiased_%s_%s.eps",
                             pName,charge,
                             index).Data());
  ...
  //==================================================//
  varName.Form("%s_ProbNNmu", DSpName);
  massFitter->SetRange(varName.Data(), 0, 1, "isMuonRange");
  RooPlot* rp_probnnmu = massFitter->PlotVariable(varName.Data(),
                                                  "", "isMuonRange", kTRUE);
  TCanvas* c7 = new TCanvas("cnv_probnnmu", "", 1024, 768);
  rp_probnnmu->Draw();
  c7->SaveAs(TString::Format("ProbNNmu_%s_MuonUnBiased_%s_%s.eps",pName,charge,
                             index).Data());
...
int main(int argc, char *argv[])
{
  ...
  //---------------------------------------------------//
  TString fname;
  TString tupleFullName=TString::Format("DSt2D0Pi_D02RSKPi_MuonUnBiasedTuple/%s", 
    tupleName);
  BOOST_FOREACH(const ssMap::value_type& charge, charges) {
    ...
    std::cout << "Making D* MuonUnBiased fit for particle (charge) = "
              << pName << " (" << charge.second
              << ")" << std::endl;
    try {    
