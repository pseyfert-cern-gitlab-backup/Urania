...
fileSuffix='lam0_p_muonUnBiased'
...
# the LoP 'CalibPID' nTuple
tupleDir_loP = "Lam02PPi_LoP_MuonUnBiasedTuple"
tupleDir_hiP = "Lam02PPi_HiP_MuonUnBiasedTuple"

# Set the selection cuts, e.g. 'MuonUnBiased' cuts
#
# N.B. The run-range splitting is determined after application of
# the selection cuts
cuts='P_MuonTisTagged==1'
...
app=Executable(
    exe=basedir+'/scripts/sh/GetRunRanges_Lam0.sh',
    # arguments are:
    # 1) ganga directory
    # 2) Urania version
    # 3) platform
    ...
    # 5) stripping version
    # 6) file suffix
    # 7) LoP nTuple directory
    # 8) HiP nTuple directory
    # 9) selection cuts (if applicable)
    # 10) starting value of 'maxEntries'
    #    (the maximum number of entries per subsample)
    # 11) step fraction for maxEntries
    # 12) minimum fraction of maxEntries allowed after
