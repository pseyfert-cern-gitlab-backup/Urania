...
do
    ...
    echo "Performing D* mass fit for index $n" 
    $CALIBAPPDIR/DStFit_MuonUnBiased.exe $PART_NAME $MAG_POL $STRIP_VER $CALIBDATATYPE $n $OUT_DIR $VERBOSE $MYDATA
    sleep 3

    # merge the h+ and h- samples
    echo "Merging h+ and h- samples for index $n" 
    $CALIBAPPDIR/MergeCharges.exe DSt_MuonUnBiased $OUT_DIR $PART_NAME $MAG_POL $STRIP_VER $OUT_DIR $n
    sleep 3
