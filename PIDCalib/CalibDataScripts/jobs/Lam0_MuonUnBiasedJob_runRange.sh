...
do
    ...
    $CALIBAPPDIR/Lam0Fit_MuonUnBiased.exe $PART_NAME $MAG_POL $STRIP_VER $CALIBDATATYPE $n $OUT_DIR $VERBOSE $MYDATA
    sleep 2
	...
	$CALIBAPPDIR/MergeProtonLines_MuonUnBiased.exe $OUT_DIR $PART_NAME Plus $MAG_POL $STRIP_VER $OUT_DIR $n
	sleep 2

        # merge the LowP and HiP antiproton samples
	echo "Merging antiproton lines for index $n" 
	$CALIBAPPDIR/MergeProtonLines_MuonUnBiased.exe $OUT_DIR $PART_NAME Minus $MAG_POL $STRIP_VER $OUT_DIR $n
	sleep 2
    fi

    # merge the h+ and h- samples
    echo "Merging h+ and h- samples for index $n" 
    $CALIBAPPDIR/MergeCharges.exe Lam0_MuonUnBiased $OUT_DIR $PART_NAME $MAG_POL $STRIP_VER $OUT_DIR $n 
    sleep 2
