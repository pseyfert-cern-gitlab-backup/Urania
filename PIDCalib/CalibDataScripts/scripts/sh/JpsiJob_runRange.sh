...
$CALIBAPPDIR/JpsiFit.exe "$MAG_POL" "$STRIP_VER" "$CALIBDATATYPE" "$INDEX" \
  "$OUT_DIR" "$PRINTENTRIES" "$EXITONBADFIT" $MYDATA
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
    exit $EXITCODE
fi
...
# merge the mu+ and mu- samples
echo "Merging mu+ and mu- samples" 
$CALIBAPPDIR/MergeCharges.exe "Jpsi" "$OUT_DIR" "Mu" "$MAG_POL" "$STRIP_VER" "$OUT_DIR" "$INDEX"
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
    exit $EXITCODE
fi
