...
$CALIBAPPDIR/MergeProtIncLc.exe "$OUT_DIR" "$PART_NAME" "Plus" "$MAG_POL" \
...
$CALIBAPPDIR/MergeProtIncLc.exe "$OUT_DIR" "$PART_NAME" "Minus" "$MAG_POL" \
  "$STRIP_VER" "$OUT_DIR" "$INDEX"
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
    exit $EXITCODE
...
$CALIBAPPDIR/MergeCharges.exe "TotLc" "$OUT_DIR" "P" "$MAG_POL" \
  "$STRIP_VER" "$OUT_DIR" "$INDEX"
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
    exit $EXITCODE
