...
if [ $# -lt 8 ]; then
  echo "Usage: `basename $0` <gangaJobsDir> <uraniaVersion> <platform> <magPol> " \
       "<stripVersion> <fileSuffix> <tupleDir_loP> <tupleDir_hiP> (<cuts=\"\"> " \
       "<maxEntries=2e6> <stepFrac=0.01> <minFrac=0.9>)"
  exit 1
fi
...
echo "LoP NTuple directory: ${TUPLEDIR_LOP}"
echo "HiP NTuple directory: ${TUPLEDIR_HIP}"
...
APP="${CALIBDATASCRIPTSROOT}/scripts/python/getRunRanges_Lam0.py"

python $APP -c "${CUTS}" -m ${MAXENTRIES} -s ${STEPFRAC} -f ${MINFRAC} \
  ${TUPLEDIR_LOP} ${TUPLEDIR_HIP} ${PKLNAME} ${DATA}
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
  exit $EXITCODE
fi
