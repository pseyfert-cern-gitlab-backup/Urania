...
# perform the Lam0 mass fit
echo "Performing Lam0 mass fit" 
$CALIBAPPDIR/Lam0Fit.exe "$PART_NAME" "$MAG_POL" "$STRIP_VER" "$CALIBDATATYPE" \
...
# merge the LoP and HiP proton samples
echo "Merging LoP and HiP proton samples" 
$CALIBAPPDIR/MergeProtonLines.exe "$OUT_DIR" "$PART_NAME" "Plus" "$MAG_POL" \
...
# merge the LoP and HiP antiproton samples
echo "Merging LoP and HiP antiproton samples" 
$CALIBAPPDIR/MergeProtonLines.exe "$OUT_DIR" "$PART_NAME" "Minus" "$MAG_POL" \
  "$STRIP_VER" "$OUT_DIR" "$INDEX"
EXITCODE=$?
if [ $EXITCODE -ne 0 ]; then
    exit $EXITCODE
