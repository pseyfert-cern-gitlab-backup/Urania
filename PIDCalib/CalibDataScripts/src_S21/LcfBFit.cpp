...
#include "CalibDataScripts/LcfBFit.h"
             ...
             Bool_t exitOnBadFit)
{
  ...
  massFitter->SetDMassName(LcfBName);
...
//  plotParams+=",dmass_bkg_poly_c1,dmass_bkg_poly_c2";
  ...
  massFitter->SetPlotBins(LcfBName, nbins);

  
  //==================================================// 
  // Make plots
  //==================================================//
  massFitter->SetRange(LcfBName, LcMassPDG-55,LcMassPDG+55, "sigRange");
  ...
  // Plot Lc Mass Fit
  //==================================================//
  RooPlot* rp_mass = massFitter->PlotFitResults(LcfBName, 
                                                "fitResults",
                                                "sigRange",
                                                rangeTitle.Data(),
  ...
  // get the pull plot
  RooPlot* rp_mass_pull = massFitter->PlotFitPulls(LcfBName,
                                                   "fitResults");
  ...
  c1_lower->cd();
  rp_mass_pull->Draw();
  c1->SaveAs(TString::Format("FitRes_%s_%s_%s_%s.pdf",LcfBName,pName,
                             charge,index).Data());
...
fun_vec.push_back(SetSpecVars_EvtVars);
 ...
 fun_vec.push_back(SetSpecVars_PMuonUNB);
  ...
  BOOST_FOREACH(const ssMap::value_type& charge, charges) {
    tupleFullName.Form("Lb2LcMuNu_Lc2PKiPiTuple/%s", 
    ...
    fname.Form("%s/LcfB_P%s_Mag%s_Strip%s_%s.root",
