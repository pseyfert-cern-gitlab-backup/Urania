...
{  
  ...
  //    TString chargeCut= TString::Format("pi_TRACK_GHOSTPROB<0.5 && pis_TRACK_GHOSTPROB<0.5 && K_TRACK_GHOSTPROB<0.5 && abs(Dz_WM_KK-1864.83)>25 && abs(Dz_WM_PiPi-1864.83)>25 && abs(Dz_WM_DCS-1864.83)>25 && %s_ID/abs(%s_ID) == "+chrg, pName,       pName);

    TString chargeCut= TString::Format("abs(Dz_WM_KK-1864.83)>25 && abs(Dz_WM_PiPi-1864.83)>25 && abs(Dz_WM_DCS-1864.83)>25 && %s_ID/abs(%s_ID) == "+chrg, pName,       pName);

    //TString chargeCut= TString::Format(" abs(Dz_WM_KK-1864.83)>25 && abs(Dz_WM_PiPi-1864.83)>25 && abs(Dz_WM_DCS-1864.83)>25 && %s_ID/abs(%s_ID) == "+chrg, pName,       pName);

  //  TString VetoWindow = TString::Format("abs(Dz_WM_KK-d0MassPDG)<25 && abs(Dz_WM_PiPi-d0MassPDG)<25");
 ...
 */
 ...
 massFitter->AddPdfComponent("d0Bkg2DModel",   kCyan,  kDashed, 3);
  ...
  TString varName;
  //varName.Form("%s_Tesla_CombDLLK",DSpName);
...
fun_vec.push_back(SetSpecVars_KPiMuonUNB);
  ...
  BOOST_FOREACH(const ssMap::value_type& charge, charges) {
    ...
    // could do something here to rename it as Pi not pi
