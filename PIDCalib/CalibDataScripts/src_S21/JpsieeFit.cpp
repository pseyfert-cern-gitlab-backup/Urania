...
TString Cuts = TString::Format(" (%s) && (%s) && (%s) && (%s)",
  ...
  std::cout<<" Cut data, about to configure line shape"<<std::endl;
...
//                                 0.75);
    ...
    massFitter->MakeDMassSigCBplusGauss(
      3050, 3000, 3200,
      50, 25., 150.,
      0.7, 0.01, 5.,
      20.0,
...
//   massFitter->MakeDMassBkgExp(-2e-3, -.2, .1);
  ...
  if (bremAdded == -1) {//fundge
    massFitter->AddPdfComponent("sigModel",      kRed,      kDashed, 3);
  } else {
    massFitter->AddPdfComponent("dmass_sig_cb",      kRed,      kDashed, 3);
    massFitter->AddPdfComponent("dmass_sig_gauss",     kGreen,    kDashed, 3);
  }
  massFitter->AddPdfComponent("bkgModel",  kOrange+2, 7,       3);

  TString plotParams("");
  if (bremAdded == -1) {//fudge
    plotParams = "dmass_sig_mu,dmass_sig_sigma,dmass_sig_alpha";
  } else {
    plotParams = "dmass_sig_mu,dmass_sig_m_gaus,dmass_sig_sigma_left,dmass_sig_sigma_gaus,dmass_sig_alpha_left,dmass_sig_frac_CB";
  }
  ...
  if ( bremAdded == 0 ) {
    ...
    bremFileSuffix = "Brem1";
  } else if ( bremAdded == 2 ) {
    bremFileSuffix = "Brem2";
  }
...
int main(int argc, char *argv[])
{
  ...
  ssMap electronNames = bass::map_list_of("Positive", "ep")("Negative", "em");
  // map of charge to other electron name
  ssMap otherElectronNames = bass::map_list_of("Positive", "em")("Negative", "ep");
...
//  else if (strcmp(DataType,"2011")==0) fun_vec.push_back(SetSpecVars_EvtVars2012);
 // else {
  ...
  BOOST_FOREACH(const ssMap::value_type& charge, charges) {
    ...
    float bkgslope=0; //for backwards comp
    ...
    for (int ibrem = 0 ; ibrem < 2; ++ibrem) {
      ...
      std::string brname;
      if(ibrem ==0) brname="0brem";
      if(ibrem ==1) brname="1brem";
      if(ibrem ==2) brname="2brem";

      const char* bname = brname.c_str();

      fname.Form("%s/Jpsi_e%s_Mag%s_Strip%s_%s_%s.root",
                outDir, (charge.second).Data(),
                magPol, stripVersion, bname,
                index);
      std::cout << "Making J/psi ee fit for "
                << "particle (charge) = "
                << electronName.Data() << " (" << charge.second
                << ") with " << bname << "brems" << std::endl;
      ...
      try {
          JpsieeFit(t, (charge.first).Data(), ibrem,
		    ...
		    fun_vec, index, printEntries, exitOnBadFit, bkgslope
          );
      }
