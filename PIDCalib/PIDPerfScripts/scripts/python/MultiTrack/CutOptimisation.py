...
""" )
    ...
    if not f_Bkg:
        raise IOError("Failed to open bacground file {0} for reading".format(fname_Bkg))
    t_Bkg = f_Bkg.Get(tname_Bkg)
    if not t_Bkg:
    ...
    # Check DLL Cuts etc.
    #======================================================================
    if opts.verbose: #TODO: update this printout
    ...
    for CutNumber,(CutVal) in enumerate(CutVals):
        ...
        CalibTools.append( ROOT.MultiTrackCalibTool("CalibTool", t_Ref, opts.outFilename, "RECREATE",
        ...
        print 'Selected background events: {eff} +/- {err}'.format(
            eff=int(bkg), err=int(round(sqrt(bkg))))
        
        sigma_over_two = 3./2.
        punzi = NaiveResult.first / (sigma_over_two + sqrt(bkg))
        punzi_err = 0.
        if bkg>0:
          punzi_err = sqrt( (NaiveResult.second/(sqrt(bkg) + sigma_over_two))**2.
                            + (sqrt(bkg)*(-NaiveResult.first)/(2.*sqrt(bkg)*(sqrt(bkg)+sigma_over_two)**2.))**2.
                          )
        
        print 'FoM: {eff:.3f} +/- {err:.3f}'.format(
            eff=punzi, err=punzi_err)
        
        EffGraph.SetPoint(CutNumber, CutVal, NaiveResult.first)
        EffGraph.SetPointError(CutNumber, 0., NaiveResult.second)
        
        BkgGraph.SetPoint(CutNumber, CutVal, bkg)
        BkgGraph.SetPointError(CutNumber, 0., sqrt(bkg))
        
        FoMGraph.SetPoint(CutNumber, CutVal, punzi)
        FoMGraph.SetPointError(CutNumber, 0., punzi_err)
