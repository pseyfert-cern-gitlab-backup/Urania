...
MC10 B2DX signal TTree. The output is a RooDataSet with a per event weight
set.
'''
...
if '__main__' == __name__:
    ...
    f_Pion = ROOT.TFile.Open(fileRoot+'MC10/MagUp/DSt_Pi_MagUp_MC10.root')
    wsPion = f_Pion.Get('RSDStCalib')
    Pion_Data = wsPion.data('data')
    
    f_Kaon = ROOT.TFile.Open(fileRoot+'MC10/MagUp/DSt_K_MagUp_MC10.root')
    ...
    f_Sig = ROOT.TFile.Open(fileRoot+"MC10/MagUp/Bu2D0K.root")
    t_Sig = f_Sig.Get('Bu2D0K_D02KPiFAVTuple/Bu2D0X')
    ...
    #===========================================================================
    Mom = ROOT.RooRealVar('Bach_P', '', Mom_Bin.lowBound(), Mom_Bin.highBound(), 'MeV/c')
    Pt = ROOT.RooRealVar('Bach_PT', '', Pt_Bin.lowBound(), Pt_Bin.highBound(), 'MeV/c')
    Eta = ROOT.RooRealVar('Bach_TRACK_Eta', '', Eta_Bin.lowBound(), Eta_Bin.highBound(), '')
    DLLK = ROOT.RooRealVar('Bach_PIDK', '', -150, 150, '')
    DLLp = ROOT.RooRealVar('Bach_PIDp', '', -150, 150, '')
    BMass = ROOT.RooRealVar('Bu_M','',5279.15-50.0,5279.15+50.0)
    
    Sig_DataSet = gbl.TrackDataSet('B->DK Kaon'
                                   , 'Signal Track'
                                   , t_Sig
                                   , ROOT.RooArgSet(Mom
                                                    , Pt
    ...
    ws.writeToFile('MCKaon_32Momx4Eta.root')
