{
  	...
  	TFile PasMu("PubPlots_P_as_Mu_minus_P.root");
  	
  	TH1F* Proton_IsMuon = (TH1F*)PasMu.Get("P_IsMuon_All");
  	TH1F* Proton_0 = (TH1F*)PasMu.Get("P_IsMuon && (DLLmu-DLLp) > 0.0_All");
  	
  	TH1F* Muon_IsMuon = (TH1F*)MuasMu.Get("Mu_IsMuon_All");
  	TH1F* Muon_0 = (TH1F*)MuasMu.Get("Mu_IsMuon && (DLLmu-DLLp) > 0.0_All");
    ...
    c1->SetBottomMargin(0.161017);
 	...
 	PlotFourTH1FwErrors(Muon_IsMuon,Proton_IsMuon,Muon_0, Proton_0);
 	...
 	leg->SetFillColor(0);
  	...
  	leg->SetTextSize(0.04);
   	leg->AddEntry(Proton_IsMuon,"IsMuon","");
    leg->AddEntry(Proton_0,"IsMuon + #Delta LL(#mu - p) > 0","pe");
    ...
    leg1->AddEntry(Muon_0,"","pe");
  	...
  	lhcbLatex->DrawLatex(70000-15000, 1.120, "IsMuon + #Delta LL(#mu - p) > 0");

  	lhcbLatex->SetTextSize(0.05);
  	lhcbLatex->DrawLatex(25000, 0.8, "#color[2]{#mu #rightarrow #mu}");
  	lhcbLatex->DrawLatex(25000, 0.30, "p #rightarrow #mu");

  	TMarker* Marker_Proton_IsMuon = new TMarker(66000-15000, 1.275, 25);
  	Marker_Proton_IsMuon->Draw();
  	TMarker* Marker_Muon_IsMuon = new TMarker(58500-15000, 1.275, 24);
  	Marker_Muon_IsMuon->SetMarkerColor(2);
  	Marker_Muon_IsMuon->Draw();
	TMarker* Marker_Proton_0 = new TMarker(66000-15000, 1.125, 21);
  	...
  	c1->Print("Plot_2017Validation_MuP_P.eps");
  	c1->Print("Plot_2017Validation_MuP_P.pdf");
