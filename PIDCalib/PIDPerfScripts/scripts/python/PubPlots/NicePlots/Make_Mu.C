{
  	...
  	TFile PiasMu("PubPlots_Pi_as_Mu.root");
  	
  	TH1F* Pion_IsMuon = (TH1F*)PiasMu.Get("Pi_IsMuon_All");
  	TH1F* Pion_0 = (TH1F*)PiasMu.Get("Pi_IsMuon && DLLmu > 0.0_All");
  	
  	TH1F* Muon_IsMuon = (TH1F*)MuasMu.Get("Mu_IsMuon_All");
  	TH1F* Muon_0 = (TH1F*)MuasMu.Get("Mu_IsMuon && DLLmu > 0.0_All");
    ...
    c1->SetBottomMargin(0.161017);
 	...
 	PlotFourTH1FwErrors(Muon_IsMuon,Pion_IsMuon,Muon_0, Pion_0);
 	...
 	leg->SetFillColor(0);
  	...
  	leg->SetTextSize(0.04);
   	leg->AddEntry(Pion_IsMuon,"IsMuon","");
    leg->AddEntry(Pion_0,"IsMuon + #Delta LL(#mu - #pi) > 0","pe");
    ...
    leg1->AddEntry(Muon_0,"","pe");
  	...
  	lhcbLatex->DrawLatex(70000-15000, 1.120, "IsMuon + #Delta LL(#mu - #pi) > 0");

  	lhcbLatex->SetTextSize(0.05);
  	lhcbLatex->DrawLatex(25000, 0.8, "#color[2]{#mu #rightarrow #mu}");
  	lhcbLatex->DrawLatex(25000, 0.30, "#pi #rightarrow #mu");

  	TMarker* Marker_Pion_IsMuon = new TMarker(66000-15000, 1.275, 25);
  	Marker_Pion_IsMuon->Draw();
  	TMarker* Marker_Muon_IsMuon = new TMarker(58500-15000, 1.275, 24);
  	Marker_Muon_IsMuon->SetMarkerColor(2);
  	Marker_Muon_IsMuon->Draw();
	TMarker* Marker_Pion_0 = new TMarker(66000-15000, 1.125, 21);
  	Marker_Pion_0->Draw();
  	...
  	c1->Print("Plot_2017Validation_MuPi_P.eps");
  	c1->Print("Plot_2017Validation_MuPi_P.pdf");
