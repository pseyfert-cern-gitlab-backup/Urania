...
""").format(os.path.basename(sys.argv[0])),
    ...
    if opts.schemeName is None:
        schemeName = 'PerfPlots_{IDPart}{MisIDPart}'.format(
            IDPart=PartName, MisIDPart=MisIDPartName)
    else:
    ...
    trackDict = {
        ...
        'DLLCuts'   : PIDCuts,
        'Binning'   : v_BinScheme,
        'TrackCuts' : opts.cutsPart
        },
        '{MisIDPart}_as_{IDPart}'.format(IDPart=PartName,
                                         MisIDPart=MisIDPartName) : {
        'TrackType' : MisIDPartName,
        'DLLCuts'   : PIDCuts,
        'Binning'   : v_BinScheme,
        'TrackCuts' : opts.cutsMisPart
        }
        }
