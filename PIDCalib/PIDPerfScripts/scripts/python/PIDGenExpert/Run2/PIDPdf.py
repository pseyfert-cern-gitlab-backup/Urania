...
sigma_pid_2d *= (sigma_scale**0.5)*pid_scale
sigma_mom_2d *= (sigma_scale**0.5) 

sigma_pid_3d *= (sigma_scale**0.33)*pid_scale
sigma_mom_3d *= (sigma_scale**0.33)
sigma_eta_3d *= (sigma_scale**0.33)

sigma_pid_4d *= (sigma_scale**0.25)*pid_scale
sigma_mom_4d *= (sigma_scale**0.25)
sigma_eta_4d *= (sigma_scale**0.25)
sigma_ntr_4d *= (sigma_scale**0.25)
...
if mcgen : 
  ...
  if bootstrap_file :
    ...
    nt2 = TNtuple(ntname + "_boot","pid", pidname + ":Pt:Eta:Ntracks:w")
    ...
    f2.cd()
    for i in nt1 :
      nt2.Fill(eval("i." + pidname), i.Pt, i.Eta, i.Ntracks, i.w*rnd.Poisson(1.) )
    nt2.Write()
    f2.Close()
    f1.Close()
    ntname = ntname + "_boot"
...
if not elec_mode : 
  ...
  kde_pidmom = BinnedKernelDensity("KDEPDF_PIDMom", pidmom_phsp, ntuple, 
                          pidname,"Pt","w", 300,200, sigma_pid_2d, sigma_mom_2d,
                          fact_pidmom, toystat )
  ...
  kde_pidmometa = BinnedKernelDensity("KDEPDF_PIDMomEta", pidmometa_phsp, ntuple, 
                          pidname,"Pt","Eta","w", 150,100,100, sigma_pid_3d, sigma_mom_3d, sigma_eta_3d,
  ...
  kde_pideta = BinnedKernelDensity("KDEPDF_PIDEta", pideta_phsp, ntuple, 
                          pidname,"Eta","w", 200,200, sigma_pid_2d, sigma_eta_2d,
                          fact_pideta, toystat )
...
kde_pidmometantr = BinnedKernelDensity("KDEPDF_PIDMomEtaNtr", phsp, ntuple, 
                          pidname,"Pt","Eta","Ntracks","w", 70, 70, 70, 10, sigma_pid_4d, sigma_mom_4d, sigma_eta_4d, sigma_ntr_4d,
