...
int main(void)
{
  ...
  const std::string fileRoot = "$DATADISK/2011_PIDCalib/Strip15_MagDown/";
  
  //======================================================================
  // Ouput file from Test_EvtData.cpp
  //======================================================================
  TFile* f_Pi = new TFile((fileRoot+"DSt_Pi_MagDown_Strip15_0.root").c_str());
  if (!f_Pi) return 1;

  TFile* f_K = new TFile((fileRoot+"DSt_K_MagDown_Strip15_0.root").c_str());
  if (!f_K) return 1;
  ...
  EvtTrackDataSet* Pion_DataSet = new EvtTrackDataSet("Calib. Pion",
                                                      ...
                                                      *Pion_Data->get(),
                                                      ...
                                                      "Pi_CombDLLK",
                                                      "Pi_CombDLLp",
                                                      "Charge==Charge::Negative && Pi_P<100000",
                                                      "nsig_sw");
  ...
  EvtTrackDataSet* Kaon_DataSet = new EvtTrackDataSet("Calib. Kaon",
                                                      ...
                                                      *Kaon_Data->get(),
                                                      ...
                                                      "K_CombDLLK",
                                                      "K_CombDLLp",
                                                      "Charge==Charge::Negative && K_P<100000",
                                                      "nsig_sw");
  ...
  cout<<"Efficiency (DLLK>0)"<<'\t'<<"Low Error"<<'\t'<<"High Error"<<endl;
  cout<<Kaon_0.Eff<<'\t'<<Kaon_0.Err_Lo<<'\t'<<Kaon_0.Err_Hi<<endl;
  ...
  cout<<"Efficiency (DLLK>5)"<<'\t'<<"Low Error"<<'\t'<<"High Error"<<endl;
  cout<<Kaon_5.Eff<<'\t'<<Kaon_5.Err_Lo<<'\t'<<Kaon_5.Err_Hi<<endl;
  ...
  vector<TGraphAsymmErrors*> KaonPlots;
  vector<TGraphAsymmErrors*> PionPlots;
  ...
  // Save graphs to file
  //======================================================================
  TFile *myfile = new TFile("KPi_Neg_Separation_MagDown_Bayes_Strip15.root", "RECREATE");
  
  vector<TGraphAsymmErrors*>::iterator his_itr;
