...
int main(int argc, char* argv[])
{
  ...
  for(int i=-15; i<15; ++i)
  {
    PIDCuts_Kaon.push_back( ( TString::Format("DLLK > %s",
                                             (boost::lexical_cast<string>(i)).c_str()) ).Data() );
    PIDCuts_Pion.push_back( ( TString::Format("DLLK < %s",
                                             (boost::lexical_cast<string>(i)).c_str()) ).Data());
    cout<<left
  ...
  //======================================================================
  TFile *myfile = new TFile(TString::Format("Perf_DLLK_Mag%s_Strip%s.root",
                                            argv[2],
  ...
  PIDCuts.push_back("DLLK > 0.0");
  PIDCuts.push_back("DLLK > 5.0");
