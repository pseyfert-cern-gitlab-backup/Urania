...
config = configDictFromFile('glm_conf000.py')
...
def buildTimePdf(config):
    ...
    # build the time pdf
    Cf = WS(ws, RooRealVar('Cf', 'C_{f}', 1.0, -3.0, 3.0) )
    Sf = WS(ws, RooRealVar('Sf', 'S_{f}', 0.0, -3.0, 3.0) )
    Sfbar = WS(ws, RooRealVar('Sfbar', 'S_{#bar f}', 0.0, -3.0, 3.0) )
    
    pdf = buildBDecayTimePdf(
        config, 'pdf', ws,
        time, timeerr, qt, qf, [ [ omega, omegabar ] ], [ tageff ],
        Gamma, DGamma, Dm,
        C = Cf, D = zero, Dbar = zero, S = Sf, Sbar = Sfbar,
...
f.Close()
del f
genpdf['ws'].writeToFile('workspace000_%04d.root' % SEED, True)
fitpdf['ws'].writeToFile('workspace000_%04d.root' % SEED, False)
...
canv.cd()
frame.Draw()
canv.SaveAs('plot000_%04d.pdf' % SEED)
