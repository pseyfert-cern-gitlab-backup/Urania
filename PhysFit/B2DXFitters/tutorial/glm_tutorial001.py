...
#
# @brief first attempt to illustrate the usage of Espresso in RooFit
#
# gaussian-constrained calibration parameters
#
...
config = configDictFromFile('glm_conf001.py')
...
def buildTimePdf(config):
    ...
    time = WS(ws, RooRealVar('time', '#tau [ps]', 0.2, 15.0))
    ...
    # now other settings
    Gamma  = WS(ws, RooRealVar( 'Gamma',  'Gamma',  0.661, 0.1, 3.0)) # ps^-1
    ...
    for sfx in ('omega0', 'omegaavg', 'f'):
        mistagpdfparams[sfx] = WS(ws, RooRealVar(
            'mistagpdf_%s' % sfx, 'mistagpdf_%s' % sfx,
    ...
    glm = ROOT.Espresso.GLMBuilder("glm",
                                   "glm",
                                   eta,
    ...
    omegabar = WS(ws, glm.bbar_mistag())

    # get multivariate gaussian constraint on tagging calibration parameters
    covariance = WS(ws, glm.covariance_matrix() )
        
    # build the time pdf
    pdf = buildBDecayTimePdf(
        config, 'pdf', ws,
        time, timeerr, qt, qf, [ [ omega, omegabar ] ], [ tageff ],
        Gamma, DGamma, Dm,
...
gausscons = RooArgSet( fitpdf['cov'] )
fitopts = [ RooFit.Timer(), RooFit.Save(),
            ...
            RooFit.ExternalConstraints(gausscons) ]
...
f = TFile('fitresult001_%04d.root' % SEED, 'recreate')
f.WriteTObject(rawfitresult, 'fitresult001_%04d' % SEED)
f.Close()
del f
genpdf['ws'].writeToFile('workspace001_%04d.root' % SEED, True)
fitpdf['ws'].writeToFile('workspace001_%04d.root' % SEED, False)

# plot fit projection
time = ds.get().find('time')
frame = time.frame(RooFit.Title(""))
ds.plotOn(frame)
fitpdf['pdf'].plotOn(frame)
from ROOT import TCanvas
canv = TCanvas("canv")
canv.cd()
frame.Draw()
canv.SaveAs('plot001_%04d.pdf' % SEED)
