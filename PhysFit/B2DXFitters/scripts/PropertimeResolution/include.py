...
dp_dir = os.path.join("DataPlots", desc)
ws_dir = os.path.join("Workspaces", desc)
sWeight_plots_dir = os.path.join("SWeightPlots", desc)
fit_dir_no_bins = os.path.join("FitResults", fit_desc)
fit_dir = os.path.join(fit_dir_no_bins, "{}Bins".format(nBins))
...
if desc == desc_ALL:
    ...
    in_file_name = os.path.join(base_dir, "TD_DsK3fb_MC_LTUB", "PromptDsH2", "PromptDsH2_PTR.root")
elif desc == desc_DsPi_MC:
    in_file_name = os.path.join(base_dir, "TD_DsK3fb_MC", "B2DX_MC_Bs_Dspi_KKpi_PTR.root")
elif desc == desc_DsK_MC_LTUB:
    in_file_name = os.path.join(base_dir, "TD_DsK3fb_MC_LTUB", "Bs_DsK", "Bs_DsK_LTUB_Up.root")
else:
    year = desc.split("_")[0]
    fs = desc.split("_")[-1]
    mag = "Up" if "Up" in desc else "Dw"
    in_file_name = os.path.join(base_dir, "TD_DsK3fb_LTUB", "MERGED", "Bs_DsK_{0}_{1}_{2}.root".format(fs, year, mag))
