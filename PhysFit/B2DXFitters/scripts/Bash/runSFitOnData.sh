...
export nickname="SSbarAccAsymmFloatingDMGammaFTConstrOSExclusiveBlinded_SSrlogit"
#export nickname="SSbarAccAsymmFloatingDMGammaConstrAllSamplesBlinded_Nov2016"
...
${rundir}run python ${pyscriptpath}runSFit_Bd.py --debug --pereventmistag --fileName $inputfile --save $outputfile --configName $config --pol $pol --mode $mode --year $year --hypo $hypo --merge both --NCPU 8 --HFAG --blindingString $blindingString --UniformBlinding --outputdir $outputdir --singletagger TagDecOS --preselection "TagDecOS!=0&&TagDecSS==0" | tee ${outputdir}logfile.txt
export Stop=`date`
