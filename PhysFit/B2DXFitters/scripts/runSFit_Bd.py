...
def getCPparameters(ws, myconfigfile, UniformBlinding, HFAG):
    ...
    limit = [-3.0, 3.0]
    if "CPlimit" in myconfigfile["ACP"]["Signal"]:
...
def runSFit(debug, wsname,
    ...
    # Empty list for some of the WS.py calls
    opts = []
    ...
    if not MC:
        ...
        if preselection != "":
            ...
            dataWA = WS(ws, RooDataSet(data_temp.GetName(), dataWA_temp.GetTitle(), data_temp, data_temp.get(), preselection, "sWeights"), opts)
            data = WS(ws, RooDataSet(dataWA_temp.GetName(), data_temp.GetTitle(), dataWA_temp, dataWA_temp.get(), preselection), opts)
            ...
            data = WS(ws, GeneralUtils.GetDataSet(workspace[0], nameData, debug), opts)
            dataWA = WS(ws, GeneralUtils.GetDataSet(workspaceW[0], nameDataWA, debug), opts)
    ...
    if not truetag:
        ...
        if obs.find("TagDecTrue") is not None:
    ...
    gamma = WS(ws, RooRealVar('Gamma', '#Gamma', *(myconfigfile["ACP"]["Signal"]["Gamma"] + ['ps^{-1}'])), opts)
    # setConstantIfSoConfigured(ws.obj(gamma.GetName()),myconfigfile)
    deltaGamma = WS(ws, RooRealVar('deltaGamma', '#Delta#Gamma', *(myconfigfile["ACP"]["Signal"]["DeltaGamma"] + ['ps^{-1}'])), opts)
    # setConstantIfSoConfigured(ws.obj(deltaGamma.GetName()),myconfigfile)
    deltaM = WS(ws, RooRealVar('deltaM', '#Delta m', *(myconfigfile["ACP"]["Signal"]["DeltaM"] + ['ps^{-1}'])), opts)
    ...
    if myconfigfile["ResolutionAcceptance"]["Signal"]["Resolution"] is None:
        if debug:
        ...
        if myconfigfile["ResolutionAcceptance"]["Signal"]["Resolution"]["Type"] == "AverageModel":
            ...
            if myconfigfile["ResolutionAcceptance"]["Signal"]["TimeErrorPDF"]["Type"] == "FromWorkspace":
                ...
                terrpdf = WS(ws, GeneralUtils.CreateHistPDF(dataWA,
                                                            terr,
                                                            TString(myconfigfile["ResolutionAcceptance"]["Signal"]["TimeErrorPDF"]["Name"]),
                                                            20,
                                                            debug))
                if debug:
                ...
                if "Type" in myconfigfile["Taggers"]["Signal"][nametag]["MistagPDF"].keys():
                    if myconfigfile["Taggers"]["Signal"][nametag]["MistagPDF"]["Type"] == "Mock":
                        ...
                        mistagpdf.append(WS(ws, Bs2Dsh2011TDAnaModels.GetRooHistPdfFromWorkspace(mistagWork,
                                                                                                 TString(myconfigfile["Taggers"]["Signal"]
                                                                                                                     [nametag]["MistagPDF"]["Name"]),
                                                                                                 debug)))
    ...
    if "ProductionAsymmetry" in myconfigfile:
        aProd = WS(ws, RooRealVar('ProdAsymm', 'a_{prod}', *myconfigfile["ProductionAsymmetry"]["Signal"]), opts)
