...
int main(int argc, char* argv[])
{
	...
	while (i <= 25) //Cap
		{
			std::stringstream ss;
			ss << i;
			temp = TFile::Open(("/afs/cern.ch/user/l/lohenry/cmtuser/Urania_v2r4/PhysFit/V0hhFitter/KShh/temp/SystII-CombModel-"+option+"-"+selectionType+"-"+combModel+"-"+ss.str()+".root").c_str());
			if (!temp)
				{
					//					break;
					i++;
	...
	V0hhFit::FitterPulls fPulls(files,"SystII-TypeII","dataFit","SystII-CombModel-"+selectionType+"-"+combModel+"-"+option+".txt");
	//	fPulls.fillVectors_SystII(files);
	fPulls.plotSystII(("KShh/temp/plots/SystII-CombModel/"+selectionType+"/").c_str(),"SystII-CombModel-"+combModel+"-"+option,2);//("PolSlopes-"+signalWS).c_str(),2);  
}
