...
int main(int argc, char* argv[])
{
  ...
  while (i < 200) //Cap
	  {
		  std::stringstream ss;
		  ss << i;
		  temp = TFile::Open(("/afs/cern.ch/user/l/lohenry/cmtuser/Urania_v2r4/PhysFit/V0hhFitter/KShh/temp/SystI-Sig-"+option+"-"+selectionType+"-"+combModel+"-"+ss.str()+".root").c_str());
		  if (!temp)
			  break;
		  temp->Close();
  ...
  V0hhFit::FitterPulls fPulls(files,"SystI","dataFit","SystI-Signal-"+signalWS+"-"+selectionType+"-"+combModel+"-"+option+".txt");
  ...
  fPulls.plotSystI(("KShh/temp/plots/Signal-"+signalWS+"/"+selectionType+"/"+combModel+"/").c_str(),("Signal-"+option).c_str(),2);
  fPulls.plotConstant(("KShh/temp/plots/Signal-"+signalWS+"/"+selectionType+"/"+combModel+"/").c_str(),("Signal-"+option).c_str(),2);
  //  fPulls.plotErrorVsResiduals(("KShh/temp/plots/Signal-"+signalWS+"/"+selectionType+"/"+combModel+"/").c_str(),("Signal-ErrVsResiduals-"+option).c_str());
