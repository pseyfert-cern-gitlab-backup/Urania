...
int main(int argc, char* argv[])
{
  ...
  std::string name("Fitter"), signalWS("");
  ...
  while (i < 200) //Cap
	  {
		  std::stringstream ss;
		  ss << i;
		  temp = TFile::Open(("/afs/cern.ch/user/l/lohenry/cmtuser/Urania_v2r4/PhysFit/V0hhFitter/KShh/temp/SystI-Gaussian-"+selectionType+"-"+ss.str()+".root").c_str());
		  if (!temp)
			  break;
		  temp->Close();
  ...
  V0hhFit::FitterPulls fPulls(files, true);
  ...
  fPulls.plotSystI("KShh/temp/plots/","Gaussian",2);
