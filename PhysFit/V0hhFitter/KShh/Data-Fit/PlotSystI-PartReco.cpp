...
int main(int argc, char* argv[])
{
  ...
  while (i < 50) //Cap
	  {
		  std::stringstream ss;
		  ss << i;
		  temp = TFile::Open(("/afs/cern.ch/user/l/lohenry/cmtuser/Urania_v2r4/PhysFit/V0hhFitter/KShh/temp/SystI-PartReco-"+option+"-"+selectionType+"-"+combModel+"-"+ss.str()+".root").c_str());
		  if (!temp)
			  break;
		  temp->Close();
  ...
  V0hhFit::FitterPulls fPulls(files, "SystI", "dataFit", "SystI-PartReco-"+selectionType+"-"+combModel+"-"+option+".txt");
  ...
  fPulls.plotSystI(("KShh/temp/plots/PartReco/"+selectionType+"/"+combModel+"/").c_str(),"PartReco-"+option,2);
  //  fPulls.plotConstant(("KShh/temp/plots/PartReco/"+selectionType+"/").c_str(),"PartReco",2);
