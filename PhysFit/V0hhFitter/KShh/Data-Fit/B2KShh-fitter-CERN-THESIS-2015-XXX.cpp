    ...
    if (vm.count("integral"))
    {
       ...
       for(std::vector<std::string>::iterator it_year       = year.begin();    it_year    != year.end();    ++it_year) {
		...
		for(std::map<std::string,Double_t>::iterator it_cross2 = var_map_cross.begin(); it_cross2 != var_map_cross.end(); ++it_cross2){
		      ...
		      std::cout << "Bs2KSKpiDD " << yearComp << sigma << std::endl; 
		      ...
		      std::cout << "Bs2KSKpiLL " << yearComp << sigma << std::endl; 
                      ...
                      sigma =  sigma_input*sigma_DD_Kpi_r*sigmaDD;
		      std::cout << "Bs2KSpiKDD " << yearComp << sigma << std::endl; 
                      ...
                      sigma =  sigma_input*sigma_LL_Kpi_r*sigmaLL;
		      std::cout << "Bs2KSpiKLL " << yearComp << sigma << std::endl; 
		   ...
		   /*
		   TString auxName_pdf = "";
		   auxName_pdf = (it_sig->first).c_str();
		   if (auxName_pdf.Contains("KSpiK")){
		     auxName_pdf.ReplaceAll("KSpiK","KSKpi");
		   }
		   */
		   ...
		   //norm_sig_aux = fitter.getWS()->pdf(auxName_pdf)->createIntegral( *B_M_int, RooFit::NormSet(*B_M_int), RooFit::Range("sigma")) ; 
                   //norm_bkg_aux = fitter.getWS()->pdf((it_bkg->first).c_str())->createIntegral( *B_M_int, RooFit::NormSet(*B_M_int), RooFit::Range("sigma")) ; 
		   ...
		   std::cout << "Signal     : [norm_sig_aux] : " << (it_sig->first).c_str() << " " << norm_sig_aux->getVal() << " " << (it_sig->second) << " " << norm_sig <<  std::endl;
		   std::cout << "Background : [norm_bkg_aux] : " << (it_bkg->first).c_str() << " " << norm_bkg_aux->getVal() << " " << (it_bkg->second) << " " << norm_bkg <<  std::endl;
		   ...
		   std::string fileDP = fileName_aux_DP.Data(); 
                   ...
                   fitter.saveDPInfo( norm_sig, norm_bkg, norm_cross, fileDP.c_str(), mode.c_str(), minRange, maxRange); 
                } else {
                   continue;
                }
		}
