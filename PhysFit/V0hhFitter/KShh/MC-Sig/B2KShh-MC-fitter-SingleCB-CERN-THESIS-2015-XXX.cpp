    ...
    for(std::vector<std::string>::iterator it_year     = year.begin();   it_year  != year.end();   ++it_year) {
       for(std::vector<std::string>::iterator it_modes = modes.begin();  it_modes != modes.end();  ++it_modes) {
          for(std::vector<std::string>::iterator it_KS = KS_cat.begin(); it_KS    != KS_cat.end(); ++it_KS) {
             ...
             } else if (sig_shape == "SingleCB"){
		std::cout << "I am here" << std::endl;
		if ((*it_modes).find("Bd") != std::string::npos) { 
                   fitter.makeSingleCB(((*it_modes)+(*it_KS)+"_"+(*it_year)+"_pdf").c_str(), 5279, 5269, 5389, 20, 0, 25, 2, 0, 5, 1, 0, 10);
		} else {
                   fitter.makeSingleCB(((*it_modes)+(*it_KS)+"_"+(*it_year)+"_pdf").c_str(), 5366, 5356, 5376, 20, 0, 25, 2, 0, 5, 1, 0, 10);
		}
             } else {
...
//       params.push_back("coreFrac");
       ...
       // Run toyMC's
       toy.runToyStudy("fitResults", genYields.Data(), parFix.Data());
       // Plot Toy MC results
