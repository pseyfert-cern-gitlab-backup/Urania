...
void sanityDDLL()
{
	...
	TFile* f = TFile::Open("~/cmtuser/Urania_v2r4/PhysFit/V0hhFitter/KShh/WS/Fit-WS-Standard-Louis-Tight-PolSlopes-Standard.root","READ");
		...
		{	
			...
			for (std::vector<std::string>::iterator itYear = year.begin() ; itYear != year.end() ; ++itYear)
				for (std::vector<std::string>::iterator itMeson = meson.begin() ; itMeson != meson.end() ; ++itMeson)
					{
						if (*itInvMass == "KK" && *itMeson == "Bs")
							continue;
						std::string DDName = *itMeson+"2KS"+(*itInvMass)+"DD_"+(*itYear)+"_pdf_KS"+(*itInvMass)+"DD_"+(*itYear)+"_Yield";
						std::string LLName = *itMeson+"2KS"+(*itInvMass)+"LL_"+(*itYear)+"_pdf_KS"+(*itInvMass)+"LL_"+(*itYear)+"_Yield";
						RooArgList finPars = fitRes->floatParsFinal();
						double valDD = ((RooRealVar*) &finPars[finPars.index(DDName.c_str())])->getVal();
						double errDD = ((RooRealVar*) &finPars[finPars.index(DDName.c_str())])->getError();
						double valLL = ((RooRealVar*) &finPars[finPars.index(LLName.c_str())])->getVal();
						double errLL = ((RooRealVar*) &finPars[finPars.index(LLName.c_str())])->getError();
						std::pair<double,double> ZeeRatio = div(std::make_pair(valDD,errDD),std::make_pair(valLL,errLL));
						...
						n++;
						std::cout << *itYear << "," << *itMeson << ": " << ZeeRatio.first << " +/- " << ZeeRatio.second << std::endl;
					}
