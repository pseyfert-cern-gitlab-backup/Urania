...
//-----------------------------------------------------------------------------
V0hhFit::TDirectoryError::TDirectoryError(const std::string& func,
        const TFile& f,
...
//-----------------------------------------------------------------------------
V0hhFit::TDirectoryError::~TDirectoryError() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::TDirectoryError::what() const throw()
{
...
//-----------------------------------------------------------------------------
V0hhFit::WSImportCodeFailure::WSImportCodeFailure(const std::string& func,
...
//-----------------------------------------------------------------------------
V0hhFit::WSImportCodeFailure::~WSImportCodeFailure() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::WSImportCodeFailure::what() const throw()
{
    return m_fullMsg.c_str();
}
...
//-----------------------------------------------------------------------------
V0hhFit::WSImportFailure::WSImportFailure(const std::string& func,
...
//-----------------------------------------------------------------------------
V0hhFit::WSImportFailure::~WSImportFailure() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::WSImportFailure::what() const throw()
{
    return m_fullMsg.c_str();
}
...
//-----------------------------------------------------------------------------
V0hhFit::WSRetrievalFailure::WSRetrievalFailure(const std::string& func,
        const RooWorkspace& ws, const std::string& name,
...
//-----------------------------------------------------------------------------
V0hhFit::WSRetrievalFailure::~WSRetrievalFailure() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::WSRetrievalFailure::what() const throw()
{
    return m_fullMsg.c_str();
}

//-----------------------------------------------------------------------------
V0hhFit::RFRRetrievalFailure::RFRRetrievalFailure(const std::string& func,
												 const RooFitResult& fitRes, const std::string& name) throw()
{
    std::stringstream ss;
    ss << func << ":  " << " Unable to retrieve variable " 
	   << " with name " << name << " from RooFitResult "
	   << fitRes.GetName()
	   << "(";
	const RooArgList& args = fitRes.floatParsInit();
	TIterator *it = args.createIterator();
	TNamed* named = NULL;
	while ((named = (TNamed*) it->Next()) != NULL)
		ss << named->GetName() << ",";
	ss << ")";
    m_fullMsg=ss.str();
	delete it;
...
//-----------------------------------------------------------------------------
V0hhFit::RFRRetrievalFailure::~RFRRetrievalFailure() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::RFRRetrievalFailure::what() const throw()
{
    return m_fullMsg.c_str();
}

//-----------------------------------------------------------------------------
V0hhFit::IOFailure::IOFailure( const std::string& funcName,
...
//-----------------------------------------------------------------------------
V0hhFit::IOFailure::IOFailure(const std::string& func,
                                 const char* fname,
...
//-----------------------------------------------------------------------------
V0hhFit::IOFailure::~IOFailure() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::IOFailure::what() const throw()
{
    return m_fullMsg.c_str();
}

//-----------------------------------------------------------------------------
V0hhFit::GeneralException::GeneralException(const std::string& func,
...
//-----------------------------------------------------------------------------
V0hhFit::GeneralException::~GeneralException() throw()
{}

//-----------------------------------------------------------------------------
const char* V0hhFit::GeneralException::what() const throw()
{
    return m_fullMsg.c_str();
}
