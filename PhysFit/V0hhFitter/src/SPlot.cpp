	...
	TNamed(name, title)
{
	...
	while((arg=(RooAbsArg*)iter->Next())) {
		...
		if ((dynamic_cast<RooRealVar*>(arg))->isConstant()) {
			//			coutE(InputArguments) << "SPlot::SPlot(" << GetName() << ") input argument " 
			//								  << arg->GetName() << " is constant " << endl ;
			//			throw string(Form("SPlot::SPlot(%s) input argument %s is constant",GetName(),arg->GetName())) ;
			std::cout << "FATAL: Input argument " << arg->GetName() << " is constant. Please use the dedicated method." << std::endl;
			std::cout << "TEMP: This is a temporary error message until I can figure out how exceptions/string formation work." << std::endl;
			std::exit(1);
		}
	}
	delete iter ;
	...
	std::cout << "LHENRY: Adding sWeights " << std::endl;
	...
	TNamed(name, title)
{
	...
	delete iter ;
	//Check that fixed yields are in the allYields RooArgList
	iter = fixedYields.createIterator();
	while((arg=(RooAbsArg*)iter->Next()))
		if (!(fixedYields.contains(*arg)))
			{
				coutE(InputArguments) << "SPlot::SPlot(" << GetName() << ") input argument fixed yield " 
									  << arg->GetName() << " is not in the allYieldsList " << endl ;
				throw string(Form("SPlot::SPlot(%s) fixed yield %s is not in the allYieldsList",GetName(),arg->GetName())) ;
			}
	//Construct a new SPlot class,
	//calculate sWeights, and include them
	//in the RooDataSet of this class.   
...
//_________________________________________________________________
Double_t SPlot::GetSWeightCoef(const char* sVariable) const
{
...
//______________________________________________________________________
RooArgList SPlot::GetSWeightCoefs() const
{
   
  //Return a RooArgList containing the SWeights

  RooArgList Args = fSWeightCoefs;
  ...
  for(Int_t k=0; k<nspec; ++k) 
    {
	   ...
	   //C coefficients
	   wname = std::string(yieldvars[k]->GetName())+"_c";
	   var = new RooRealVar(wname.c_str(),wname.c_str(),0);
	   //Calculate the c right now
	   double cVal = yieldvalues[k];
	   for (Int_t n = 0 ; n<nspec; ++n)
		   cVal -= covMatrix[k][n];
	   var->setVal(cVal);
...
//____________________________________________________________________
void SPlot::AddSWeight( RooAbsPdf* pdf, const RooArgList &allYieldsList,  const RooArgList &fixedYields,
						const RooArgSet &projDeps, bool includeWeights) 
{  	
	//
	// Method which adds the sWeights to the dataset.
	...
	//Make a list of varying parameters

	TIterator *it = allYieldsList.createIterator();
	RooAbsArg* arg;
	unsigned int iArg(0);

	//Stores which indexes of the allYieldsList are the variable ones.
	...
	for (Int_t ievt = 0; ievt <numevents ; ievt++) 
		{
			...
			for(Int_t j=0; j<nVarSpec; ++j) norm[j] += pdfvalues[ievt][varIndexes[j]]/dnorm ;
		}	
	coutI(Contents) << "likelihood norms: "  ;
	...
	for(Int_t ievt = 0; ievt < numevents; ++ievt) 
		{			
			...
			// covariance weighted pdf for each specie
			for(Int_t n=0; n<nVarSpec; ++n) 
				{
					Double_t nsum(0) ;
