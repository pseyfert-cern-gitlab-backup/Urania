...
namespace RooStats{
  ...
  public:
	  ...
	  SPlot(const char* name, const char* title,RooDataSet& data, RooAbsPdf* pdf, 
			const RooArgList &allYieldsList,const RooArgList &fixedYieldList, const RooArgSet &projDeps=RooArgSet(), 
			bool includeWeights=kTRUE, bool copyDataSet = kFALSE, const char* newName = "");
	  ...
	  void AddSWeight(RooAbsPdf* pdf, const RooArgList &yieldsTmp, const RooArgList &fixedYields,
					  const RooArgSet &projDeps=RooArgSet(), bool includeWeights=kTRUE);
	  ...
	  RooArgList GetSWeightCoefs() const;
