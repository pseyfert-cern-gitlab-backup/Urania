###########################################################################################################################################
## Script for the CPT fit. The script is almost idetintical with the nominal fit for the phis.
## Differences : 1) paths for datasets
##               2) const. tagging calibration parameters
##               3) plus some other minor stuff
###########################

# parse command-line options
import argparse
parser = argparse.ArgumentParser()
parser.add_argument( '--JobNumber' ,'-J')
parser.add_argument( '--SystNumber', '-L')
parser.add_argument( '--jobName', '-N', default = 'Bs2JpsiKKFit' )
parser.add_argument( '--model', '-m', default = 'lamb_phi' )  # 'phi' / 'lamb_phi' / 'polarDep'
parser.add_argument( '--blind', '-b', default = False )#it was True dont forget
...
parser.add_argument( '--dataPath', '-d', default = '/project/bfys/georgm/workdir/Bs2Jpsiphi_me/data/Bs2Jpsiphi' )
parser.add_argument( '--workPath', '-w', default = '/project/bfys/georgm/workdir/Bs2Jpsiphi_me/data/Bs2Jpsiphi' )
parser.add_argument( '--dataSetFile', '-f', default = 'Bs2JpsiKK_data_CPT.root')
...
parser.add_argument( '--constAngAcc', '-q', default = True ) # dont forget it is True
...
if parFileIn == None or parFileIn == 'None' :
    parFileIn = dataPath + '20112012Reco14DataFitValues_6KKMassBins%s.par'\
...
if parFileOut == None or parFileOut == 'None' :
    if args.JobNumber    : path = '/project/bfys/georgm/workdir/Bs2Jpsiphi_me/ToyBs2JpsiphiCPT/ToyResults_f/'
    else: path = workPath
    parFileOut = path + '%s%s_%sLow_%sUp_%sTag.par'\
...
if resultFileOut == None or resultFileOut == 'None' :
    ...
    resultFileOut = path + '%s%s_%sLow_%sUp_%sTag%s.root'\
        % ( args.jobName + ( '_' if args.jobName else '' ), args.model, 'fix' if fixLowAcc else 'float'
            , 'fix' if fixUpAcc else 'float', 'fix' if fixTagging else 'float',args.JobNumber if args.JobNumber else '')
elif resultFileOut :
...
if args.JobNumber:
    print ' Job Number : %s' % args.JobNumber
...
pdfConfig = PdfConfig( CPTviolation = True )
...
if args.JobNumber:
    dataSetFile_ = '/project/bfys/georgm/workdir/Bs2Jpsiphi_me/DataforToys/Data_f/genData_sig_lamb_phi_%s.root' % (args.JobNumber)
else:
    dataSetFile_ = dataSetFile

dataSet = readData(filePath = dataSetFile_ ,dataSetName = args.dataSetName ,NTuple = False)
...
#set tagg calib constant. Todo: change C++ object.
for i in ['wTagDelP0OS','wTagDelP0SS','wTagDelP1OS','wTagDelP1SS', 'wTagP0OS','wTagP0SS','wTagP1OS','wTagP1SS']:
    ws[i].setConstant(True)
...
#fitData = correctWeights(dataSet,'KKMassCat')
...
if pdfConfig['lambdaCPParam'] == 'observables_CPVDecay' :
    ...
    from P2VV.Imports import parNames, parValues_SMEf
print 120 * '-'
print 'parameter values:'
fitResult.PrintSpecial( text = True, LaTeX = True, normal = True, ParNames = parNames, ParValues = parValues_SMEf )
print 120 * '-'
...
#if parFileOut: #and args.JobNumber == None:
