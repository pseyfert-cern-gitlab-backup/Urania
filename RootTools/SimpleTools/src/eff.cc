...
int main(int argc, char *argv[]){
	...
	if(nFiles!=2){
		cout << "Error: wrong number of ntuples specified in " << filelistName <<  " (" << nFiles << ")" << endl;
		cout << "You must only specify two ntuples, first the denominator, then the numerator" << endl;
		return EXIT_FAILURE;
	}
	...
	while(nVarsDone<nVars){
		...
		for(UInt_t v = 0; v<ppp; v++){
			if(nVarsDone<nVars){
				...
				//TLegend *legend = new TLegend(0.15,0.80,0.25,0.70);
				cout << "pad = " << v << "	Variable = " <<  Vars[nVarsDone] << endl;

				TH1F *denominator = new TH1F("temp",Vars[nVarsDone]+"_"+LegendNames[0],VarBins[nVarsDone],VarLower[nVarsDone],VarUpper[nVarsDone]);
				//                               Trees[0]->Draw(Vars[nVarsDone]+">>"+"temp");

				Trees[0]->Draw(Vars[nVarsDone]+">>"+"temp",sweightvar[0]);
				denominator->SetTitle(LegendNames[0]);
				denominator->SetName(Vars[nVarsDone]+"_"+LegendNames[0]);
				denominator->Sumw2();

				TH1F *numerator = new TH1F("temp",Vars[nVarsDone]+"_"+LegendNames[1],VarBins[nVarsDone],VarLower[nVarsDone],VarUpper[nVarsDone]);
				//                                Trees[1]->Draw(Vars[nVarsDone]+">>"+"temp");
				Trees[1]->Draw(Vars[nVarsDone]+">>"+"temp",sweightvar[1]);
				numerator->SetTitle(LegendNames[1]);
				numerator->SetName(Vars[nVarsDone]+"_"+LegendNames[1]);
				numerator->Sumw2();

				TH1F *eff = dynamic_cast<TH1F*>(numerator->Clone(Vars[nVarsDone]+" Efficiency"));
				eff->Divide(eff,denominator,1,1,"B");

				eff->SetLineColor(Colors[0]);
				eff->SetMarkerColor(Colors[0]);
				eff->SetMarkerStyle(20);
				//eff->SetLineWidth(lw[0]);
				eff->SetName(Vars[nVarsDone]);
				eff->SetTitle(Vars[nVarsDone]);
				eff->Draw("E1");
				eff->GetXaxis()->SetTitle(Units[nVarsDone]);
				eff->GetYaxis()->SetTitle("#varepsilon");
				c->Update();
				...
				/*
				   ...
				   Trees[f]->Draw(Vars[nVarsDone]+">>"+"temp");
